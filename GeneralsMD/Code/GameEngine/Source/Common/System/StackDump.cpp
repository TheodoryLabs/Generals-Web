...
void StackDumpFromContext(DWORD eip, DWORD esp, DWORD ebp,
                          void (*callback)(const char *)) {}
void FillStackAddresses(void **addresses, unsigned int count,
                        unsigned int skip) {
  if (addresses) {
...
//*****************************************************************************
BOOL InitSymbolInfo();
void MakeStackTrace(DWORD myeip, DWORD myesp, DWORD myebp, int skipFrames,
                    void (*callback)(const char *));
void GetFunctionDetails(void *pointer, char *name, char *filename,
                        unsigned int *linenumber, unsigned int *address);
...
//*****************************************************************************
//*****************************************************************************
void StackDumpDefaultHandler(const char *line) { DEBUG_LOG((line)); }
...
BOOL InitSymbolInfo() {
  ...
  if (DbgHelpLoader::symInitialize(process, pathname, FALSE)) {
    ...
    ::GetModuleFileName(nullptr, pathname, _MAX_PATH);
    if (DbgHelpLoader::symLoadModule(process, nullptr, pathname, nullptr, 0,
                                     0)) {
...
void MakeStackTrace(DWORD myeip, DWORD myesp, DWORD myebp, int skipFrames,
  ...
  stack_frame.AddrFrame.Offset = myebp;
  {
    ...
    */
    ...
    while (b_ret && skip) {
      b_ret = DbgHelpLoader::stackWalk(
          IMAGE_FILE_MACHINE_I386, process, thread, &stack_frame,
          nullptr, //&gsContext,
          nullptr, DbgHelpLoader::symFunctionTableAccess,
          DbgHelpLoader::symGetModuleBase, nullptr);
      skip--;
    }
...
void GetFunctionDetails(void *pointer, char *name, char *filename,
  ...
  if (DbgHelpLoader::symGetSymFromAddr(process, (DWORD)pointer, &displacement,
                                       psymbol)) {
    if (name) {
    ...
    if (DbgHelpLoader::symGetLineFromAddr(process, (DWORD)pointer,
                                          &displacement, &line)) {
      if (filename) {
...

  {
    ...
    */
    ...
    while (stillgoing && skip) {
      stillgoing = DbgHelpLoader::stackWalk(
                       IMAGE_FILE_MACHINE_I386, process, thread, &stack_frame,
                       nullptr, //&gsContext,
                       nullptr, DbgHelpLoader::symFunctionTableAccess,
                       DbgHelpLoader::symGetModuleBase, nullptr) != 0;
      skip--;
    }
...
void WriteStackLine(void *address, void (*callback)(const char *)) {
  ...
  sprintf(line, "  %s(%d) : %s 0x%08p", filename, linenumber, function_name,
          address);
...
void DumpExceptionInfo(unsigned int u, EXCEPTION_POINTERS *e_info) {
  ...
  */
  ...
  static const unsigned int _codes[] = {EXCEPTION_ACCESS_VIOLATION,
  ...
  */
  static char const *_code_txt[] = {
      "Error code: EXCEPTION_ACCESS_VIOLATION\nDescription: The thread tried "
      "to read from or write to a virtual address for which it does not have "
      "the appropriate access.",
      "Error code: EXCEPTION_ARRAY_BOUNDS_EXCEEDED\nDescription: The thread "
      "tried to access an array element that is out of bounds and the "
      "underlying hardware supports bounds checking.",
      "Error code: EXCEPTION_BREAKPOINT\nDescription: A breakpoint was "
      "encountered.",
      "Error code: EXCEPTION_DATATYPE_MISALIGNMENT\nDescription: The thread "
      "tried to read or write data that is misaligned on hardware that does "
      "not provide alignment. For example, 16-bit values must be aligned on "
      ...
      "operands in a floating-point operation is denormal. A denormal value is "
      "one that is too small to represent as a standard floating-point value.",
      "Error code: EXCEPTION_FLT_DIVIDE_BY_ZERO\nDescription: The thread tried "
      "to divide a floating-point value by a floating-point divisor of zero.",
      "Error code: EXCEPTION_FLT_INEXACT_RESULT\nDescription: The result of a "
      "floating-point operation cannot be represented exactly as a decimal "
      "fraction.",
      "Error code: EXCEPTION_FLT_INVALID_OPERATION\nDescription: Some strange "
      "unknown floating point operation was attempted.",
      "Error code: EXCEPTION_FLT_OVERFLOW\nDescription: The exponent of a "
      "floating-point operation is greater than the magnitude allowed by the "
      "corresponding type.",
      ...
      "tried to access a page that was not present, and the system was unable "
      "to load the page. For example, this exception might occur if a network "
      "connection is lost while running a program over the network.",
      "Error code: EXCEPTION_INT_DIVIDE_BY_ZERO\nDescription: The thread tried "
      "to divide an integer value by an integer divisor of zero.",
      "Error code: EXCEPTION_INT_OVERFLOW\nDescription: The result of an "
      "integer operation caused a carry out of the most significant bit of the "
      "result.",
      ...
      "Programmers using a high-level language such as C should never "
      ...
      "to execute an instruction whose operation is not allowed in the current "
      "machine mode.",
      "Error code: EXCEPTION_SINGLE_STEP\nDescription: A trace trap or other "
      "single-instruction mechanism signaled that one instruction has been "
      "executed.",
      "Error code: EXCEPTION_STACK_OVERFLOW\nDescription: The thread used up "
      "its stack.",
...
#define DOUBLE_DEBUG(x)                                                        \
  ...
  if (e_info->ExceptionRecord->ExceptionCode == EXCEPTION_ACCESS_VIOLATION) {
    ...
    access_read_write =
        e_info->ExceptionRecord->ExceptionInformation[0]; // 0=read, 1=write
    access_address = e_info->ExceptionRecord->ExceptionInformation[1];
  } else {
    DOUBLE_DEBUG(
        ("Exception code is %x", e_info->ExceptionRecord->ExceptionCode));
  }
  ...
  */
  ...
  /** For access violations, print out the violation address and if it was read
   * or write.
   */
  if (e_info->ExceptionRecord->ExceptionCode == EXCEPTION_ACCESS_VIOLATION) {
  ...
  ** Dump the registers.
  */
  DOUBLE_DEBUG(("Eip:%08X\tEsp:%08X\tEbp:%08X", context->Eip, context->Esp,
                context->Ebp));
  DOUBLE_DEBUG(("Eax:%08X\tEbx:%08X\tEcx:%08X", context->Eax, context->Ebx,
                context->Ecx));
  DOUBLE_DEBUG(("Edx:%08X\tEsi:%08X\tEdi:%08X", context->Edx, context->Esi,
                context->Edi));
  ...
  /*
  ** Dump the bytes at EIP. This will make it easier to match the crash address
