...
///////////////////////////////////////////////////////////////////////////////////////////////////
#define GAME_STATE_BLOCK_STRING                                                \
  "CHUNK_GameState" // block of save game data with game info data
#define CAMPAIGN_BLOCK_STRING                                                  \
  "CHUNK_Campaign" // block of game data that has campaign info
...
// ------------------------------------------------------------------------------------------------
GameState::SnapshotBlock *GameState::findBlockInfoByToken(AsciiString token,
                                                          SnapshotType which) {
...
// TheSuperHackers @tweak Use the user's default locale instead of the system
// default to match Windows regional settings. This allows regional formats such
// as Europe (English) to use 24-hour and DD/MM/YYYY formats in-game.
...
#define DATE_BUFFER_SIZE 256
  ...
  if (GetVersionEx(&osvi)) { // check if we're running Win9x variant since they
                             // may need different characters
    if (osvi.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS) {
      char dateBuffer[DATE_BUFFER_SIZE];
      GetDateFormat(LOCALE_USER_DEFAULT, DATE_SHORTDATE, &timeVal, nullptr,
  ...
  GetDateFormatW(LOCALE_USER_DEFAULT, DATE_SHORTDATE, &timeVal, nullptr,
  ...
  UnicodeString displayDateBuffer;
  char dateBuffer[DATE_BUFFER_SIZE];
  snprintf(dateBuffer, sizeof(dateBuffer), "%02d/%02d/%04d", timeVal.wMonth,
           timeVal.wDay, timeVal.wYear);
  displayDateBuffer.translate(dateBuffer);
  return displayDateBuffer;
#endif
}
  ...
  if (GetVersionEx(&osvi)) { // check if we're running Win9x variant since they
    ...
    if (osvi.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS) {
      ...
      GetTimeFormat(LOCALE_USER_DEFAULT,
                    ...
                    &timeVal, nullptr, timeBuffer, sizeof(timeBuffer));
...
#define TIME_BUFFER_SIZE 256
  ...
  GetTimeFormatW(LOCALE_USER_DEFAULT, TIME_NOSECONDS, &timeVal, nullptr,
                 timeBuffer, ARRAY_SIZE(timeBuffer));
  displayTimeBuffer.set(timeBuffer);
#else
  char timeBuffer[256];
  snprintf(timeBuffer, sizeof(timeBuffer), "%02d:%02d", timeVal.wHour,
           timeVal.wMinute);
  displayTimeBuffer.translate(timeBuffer);
#endif
  return displayTimeBuffer;
...
void GameState::init() {
  ...
  addSnapshotBlock(CAMPAIGN_BLOCK_STRING, TheCampaignManager,
                   SNAPSHOT_SAVELOAD);
  ...
  addSnapshotBlock("CHUNK_ParticleSystem", TheParticleSystemManager,
                   SNAPSHOT_SAVELOAD);
  addSnapshotBlock("CHUNK_TerrainVisual", TheTerrainVisual, SNAPSHOT_SAVELOAD);
  addSnapshotBlock("CHUNK_GhostObject", TheGhostObjectManager,
                   SNAPSHOT_SAVELOAD);

  // add all the snapshot objects to our list of data blocks for deep CRCs of
  // logic
  addSnapshotBlock("CHUNK_TeamFactory", TheTeamFactory,
  ...
  addSnapshotBlock("CHUNK_ScriptEngine", TheScriptEngine,
                   SNAPSHOT_DEEPCRC_LOGICONLY);
  addSnapshotBlock("CHUNK_SidesList", TheSidesList, SNAPSHOT_DEEPCRC_LOGICONLY);
  addSnapshotBlock("CHUNK_Partition", ThePartitionManager,
                   SNAPSHOT_DEEPCRC_LOGICONLY);
...
static void findHighFileNumber(AsciiString filename, void *userData) {
  ...
  //
  // atoi will return zero if the string could not be converted, if the filename
  // is not literally "00000000.sav" then that means the conversion could not be
  // done so we should reject this filename from further processing
  //
  if (fileNumber == 0 && nameOnly.compare(ZERO_NAME_ONLY) != 0)
    return;

  // compare against the file number we're keeping tally of in the user data
  // parameter
...
// ------------------------------------------------------------------------------------------------
/** Given the save files on disk, find the "next" filename to use when saving a
 * game */
// ------------------------------------------------------------------------------------------------
...
#define COCKBEER_DONT_USE_NORMAL_FILE_NAMES_THANKS
...
#else
  //
  // This method has code to support two modes of finding the next filename, one
  // of them starts with a filename of 00000000.sav and counts up the number
  // looking for a file that doesn't exist and therefore a filename we can use
  // (LOWEST_NUMBER). The other method iterates all save files and returns a
  // filename that is one larger than the largest filename number encountered
  // (HIGHEST_NUMBER)
  //
  ...
  if (searchType == HIGHEST_NUMBER) {
    ...
    // iterate all the save files in the directory and find the highest file
    // number
    Int highFileNumber = -1;
    iterateSaveFiles(findHighFileNumber, &highFileNumber);

    // check for a filename that is too big (this is unlikely but theoretically
    // possible)
    ...
    DEBUG_CRASH(
        ("GameState::findNextSaveFilename - Unknown file search type '%d'",
         searchType));
    return AsciiString::TheEmptyString;
  }
#endif
...
SaveCode GameState::saveGame(AsciiString filename, UnicodeString desc,
  ...
  // if there is no filename, this is a new file being created, find an
  ...
  if (filename.isEmpty()) {
    ...
    DEBUG_CRASH(
        ("GameState::saveGame - Unable to find valid filename for save game"));
...
SaveCode GameState::missionSave() {
  ...
  UnicodeString desc;
  desc.format(format, TheGameText->fetch(campaign->m_campaignNameLabel).str(),
              missionNumber);
...
SaveCode GameState::loadGame(AvailableGameInfo gameInfo) {
  ...
  // clear game data just like loading from the debug map load screen for
  // mission saves
  ...
  //
  // clear the save directory of any temporary "scratch pad" maps that were
  // extracted from any previously loaded save game files
  //
  ...
  if (error == TRUE) {
    ...
    return SC_INVALID_DATA; // you can't use a naked "throw" outside of a catch
                            // statement!
  }

  //
  // when loading a mission save, we want to do as much normal loading stuff as
  // we can cause we don't have any real save game data to load other than the
  ...
  if (getSaveGameInfo()->saveFileType == SAVE_FILE_TYPE_MISSION) {
    ...
    // remove the mission save data, we've got all we need and have started the
    // load
    SaveGameInfo *gameInfo = getSaveGameInfo();
...
//-------------------------------------------------------------------------------------------------
AsciiString
GameState::getFilePathInSaveDirectory(const AsciiString &leaf) const {
...
AsciiString GameState::getMapLeafName(const AsciiString &in) const {
  ...
  if (p) {
    //
    // p points to the last '\' (if found), however, if a '\' was found there
    // better be another character beyond it, otherwise the map filename would
    // actually be a *directory*  Just move to the first character beyond it so
    // we are looking at the name only
    //
    ++p;
    DEBUG_ASSERTCRASH(p != nullptr && *p != 0,
                      ("GameState::xfer - Illegal map name encountered"));
    return p;
  } else {
    return in;
  }
...
// ------------------------------------------------------------------------------------------------
AsciiString
GameState::realMapPathToPortableMapPath(const AsciiString &in) const {
  AsciiString prefix;
  if (in.startsWithNoCase(getSaveDirectory())) {
    ...
    DEBUG_CRASH(("Map file was not found in any of the expected directories; "
                 "this is impossible"));
...
// ------------------------------------------------------------------------------------------------
AsciiString
GameState::portableMapPathToRealMapPath(const AsciiString &in) const {
  AsciiString prefix;
  ...
  if (in.startsWithNoCase(PORTABLE_SAVE)) {
    ...
    DEBUG_CRASH(("Map file was not found in any of the expected directories; "
                 "this is impossible"));
    // Empty string represents a failure, either caused by an invalid prefix or
    // a relative path leading outside the base path.
    return AsciiString::TheEmptyString;
  }

  if (!FileSystem::isPathInDirectory(prefix, containingBasePath)) {
    DEBUG_LOG(("Normalized file path for '%s' was outside the expected base "
               "path of '%s'.",
...
void GameState::getSaveGameInfoFromFile(AsciiString filename,
  ...
  //
  // disable post processing cause we're not really doing a load of game data
  // that needs post processing and we don't want to keep track of any snapshots
  // we loaded
  //
  ...
  while (done == FALSE) {
    ...
    if (token.compareNoCase(SAVE_FILE_EOF) == 0) {
      ...
      // we should never get here, if we did, we didn't find block of data we
      // needed
      DEBUG_CRASH(("GameState::getSaveGameInfoFromFile - Game info not found "
                   "in file '%s'",
                   filename.str()));
      done = TRUE;
      ...
      if (stricmp(token.str(), GAME_STATE_BLOCK_STRING) == 0) {
        ...
        try {
          ...
          DEBUG_CRASH(("GameState::getSaveGameInfoFromFile - Error loading "
                       "block '%s' in file '%s'",
...
static void addGameToAvailableList(AsciiString filename, void *userData) {
  ...
  // sanity
  DEBUG_ASSERTCRASH(listHead != nullptr,
                    ("addGameToAvailableList - Illegal parameters"));
  DEBUG_ASSERTCRASH(filename.isEmpty() == FALSE,
                    ("addGameToAvailableList - Illegal filename"));
  ...
  try {
    ...
    if (*listHead == nullptr)
      *listHead = newInfo;
      ...
      // insert this info so that the most recent games are always at the top of
      // this list
      for (curr = *listHead; curr != nullptr; curr = curr->next) {
        ...
        // check to see if curr is older than the new info, if so, put new info
        // just ahead of curr
...
// ------------------------------------------------------------------------------------------------
/** Populate the listbox passed in with a list of the save games present on the
 * hard drive */
// ------------------------------------------------------------------------------------------------
void GameState::populateSaveGameListbox(GameWindow *listbox,
  ...
  // setup the first entry of the listbox to be a new game when saving is
  // allowed
  ...
  for (info = m_availableGames; info; info = info->next, count++) {
    ...
    // add this available game info in the user data pointer of that listbox
    // item
...
// ------------------------------------------------------------------------------------------------
void GameState::iterateSaveFiles(IterateSaveFileCallback callback,
                                 void *userData) {
  ...
  if (!getcwd(currentDirectory, PATH_MAX))
    return;
  if (chdir(getSaveDirectory().str()) != 0)
    return;
  DIR *dir = opendir(".");
  if (dir) {
    struct dirent *ent;
    while ((ent = readdir(dir)) != nullptr) {
      Char *c = strrchr(ent->d_name, '.');
      if (c && stricmp(c, ".sav") == 0) {
        struct stat st;
        if (stat(ent->d_name, &st) == 0 && S_ISREG(st.st_mode)) {
          AsciiString filename;
          filename.set(ent->d_name);
          callback(filename, userData);
        }
      }
    }
...
void GameState::friend_xferSaveDataForCRC(Xfer *xfer, SnapshotType which) {
  DEBUG_LOG(
      ("GameState::friend_xferSaveDataForCRC() - SnapshotType %d", which));
  SaveGameInfo *gameInfo = getSaveGameInfo();
...
void GameState::xferSaveData(Xfer *xfer, SnapshotType which) {
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    for (it = m_snapshotBlockList[which].begin();
      ...
      //
      // for mission save files, we only save the game state block and campaign
      // manager because anything else is not needed.
      //
      if (getSaveGameInfo()->saveFileType != SAVE_FILE_TYPE_MISSION ||
        ...
        try {
          ...
          DEBUG_CRASH(("Error saving block '%s' in file '%s'", blockName.str(),
                       xfer->getIdentifier().str()));
          throw;
        }
      }
    }
    ...
    while (done == FALSE) {
      ...
      if (token.compareNoCase(SAVE_FILE_EOF) == 0) {
        ...
        if (blockInfo == nullptr) {
          ...
          //
          // block was not found, this could have been a block from an older
          // file format where the block was removed, skip the block data and
          // try to continue
          //
...
void GameState::addPostProcessSnapshot(Snapshot *snapshot) {
  ...
  /*
          ...
          // Please keep this code around tho, it can be useful in debugging
     save games
          //

          // verify the snapshot isn't in the list already
          SnapshotListIterator it;
          for( it = m_snapshotPostProcessList.begin(); it !=
     m_snapshotPostProcessList.end(); ++it )
          {

                  if( (*it) == snapshot )
                  {

                          DEBUG_CRASH(( "GameState::addPostProcessSnapshot -
     snapshot is already in list!" )); return;
...
void GameState::gameStatePostProcessLoad() {
  ...
  SnapshotListIterator it;
  Snapshot *snapshot;
  for (it = m_snapshotPostProcessList.begin();
       it != m_snapshotPostProcessList.end();
       /*emtpy*/) {
  ...
  // evil... must ensure this is updated prior to the script engine running the
  // first time.
 ...
 * Version Info:
 * 1: Initial version
 * 2: Added save file type and mission map name (regular save vs automatic
 * mission save) */
// ------------------------------------------------------------------------------------------------
void GameState::xfer(Xfer *xfer) {
  ...
  // if no label was found, we'll use the map name (just filename, no directory
  // info)
  if (exists == FALSE ||
      saveGameInfo->mapLabel == AsciiString::TheEmptyString) {
