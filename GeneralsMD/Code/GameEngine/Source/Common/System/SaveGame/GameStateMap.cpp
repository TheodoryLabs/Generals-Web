...
// Desc:   Chunk in the save game file that will hold a pristine version of the
// map file
///////////////////////////////////////////////////////////////////////////////////////////////////
...
static void embedPristineMap(AsciiString map, Xfer *xfer) {
  ...
  if (file == nullptr) {
    ...
    DEBUG_CRASH(
        ("embedPristineMap - Error opening source file '%s'", map.str()));
  ...
  DEBUG_ASSERTCRASH(xfer->getXferMode() == XFER_SAVE,
                    ("embedPristineMap - Unsupported xfer mode"));
  xfer->beginBlock();
...
// ------------------------------------------------------------------------------------------------
/** Embed an "in use" map into the xfer stream.  An "in use" map is one that has
 * already been pulled out of a save game file and parked in a temporary file in
 * the save directory */
// ------------------------------------------------------------------------------------------------
static void embedInUseMap(AsciiString map, Xfer *xfer) {
  ...
  if (buffer == nullptr) {
    ...
    DEBUG_CRASH(
        ("embedInUseMap - Unable to allocate buffer for file '%s'", map.str()));
    throw SC_INVALID_DATA;
  }
...
// ------------------------------------------------------------------------------------------------
/** Extract the map from the xfer stream and save as a file with filename
 * 'mapToSave' */
// ------------------------------------------------------------------------------------------------
static void extractAndSaveMap(AsciiString mapToSave, Xfer *xfer) {
  ...
  if (fp == nullptr) {
    ...
    DEBUG_CRASH(
        ("extractAndSaveMap - Unable to open file '%s'", mapToSave.str()));
    throw SC_INVALID_DATA;
  }
  ...
  if (fwrite(buffer, 1, dataSize, fp) != dataSize) {
    ...
    DEBUG_CRASH(
        ("extractAndSaveMap - Error writing to file '%s'", mapToSave.str()));
    throw SC_INVALID_DATA;
  }
 ...
 * Version Info:
 * 1: Initial version
 * 2: Now storing the game mode from logic. Storing that here cause
 * TheGameLogic->startNewGame needs to set up the player list based on it.
 */
// ------------------------------------------------------------------------------------------------
void GameStateMap::xfer(Xfer *xfer) {
  ...
  //
  // map filename, for purposes of saving we will always be saving a with a map
  // filename that refers to map in the save directory so we must always save a
  // filename into the file that is in the save directory
  //
  Bool firstSave =
      FALSE; // TRUE if we haven't yet saved a pristine load of a new map
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    //
    // write pristine map name which is already in the member 'pristineMapName'
    // from a previous load, or in the instance where we are saving for the
    // first time and the global data map name refers to a pristine map we will
    // copy it in there first
    //
    if (!TheGameState->isInSaveDirectory(TheGlobalData->m_mapName)) {
      ...
      //
      // this is also an indication that we are saving for the first time a
      // brand new map that has never been saved into this save file before (a
      // save is also considered to be a first save as long as we are writing
      // data to disk without having loaded this particular map from the save
      // file ... so if you load USA01 for the first time and save, that is a
      // first save ... then, without quitting, if you save again that is *also*
      // considered a first save).  First save just determines whether the map
      // file we embed in the save file is taken from the maps directory or from
      // the temporary map extracted to the save directory from a load
      //
      firstSave = TRUE;
    }
    ...
    if (!TheGameState->isInSaveDirectory(saveGameInfo->saveGameMapName)) {
      DEBUG_CRASH(("GameState::xfer - The map filename read from the file '%s' "
                   "is not in the SAVE directory, but should be",
    ...
    saveGameInfo->pristineMapName = TheGameState->portableMapPathToRealMapPath(
        saveGameInfo->pristineMapName);
  ...
  if (xfer->getXferMode() == XFER_SAVE) {
    ...
    //
    // if this is a first save from a pristine map load, we need to copy the
    // pristine map into the save game file
    //
    if (firstSave == TRUE) {
      ...
      // that was extracted from the save game file during the last load and
      // embedd that into the save game file
      //
    ...
    //
    // take the embedded map file out of the save file, and save as its own .map
    // file in the save directory temporarily
    //
    extractAndSaveMap(saveGameInfo->saveGameMapName, xfer);
  }

  //
  // it's important that early in the load process, we xfer the object ID
  // counter in the game logic ... this is necessary because there are flows of
  // code that create objects during the load of a save game that is *NOT FROM
  // THE OBJECT BLOCK* of code, such as loading bridges (which creates the
  // bridge and tower objects).  We are fancy and "do the right thing" in those
  // situations, but we don't want to run the risk of these objects being
  // created and having overlapping IDs of anything that we will load from the
  // save file
  //
  ...
  //
  // it is also equally important to xfer the drawable id counter early in the
  // load process because the act of creating objects also creates drawables, so
  // when it comes time to load the block of drawables we want to make sure that
  // newly created drawables at that time will never overlap IDs with any
  // drawable created as part of an object which then had its ID transferred in
  // from the save file
  //
  ...
  // Save the Game Info so the game can be started with the correct players on
  // load
  ...
  //
  // for loading, start a new game (flagged from a save game) ... this will load
  // the map and all the things in the map file that don't don't change
  // (terrain, triggers, teams, script definitions) etc
  //
  if (xfer->getXferMode() == XFER_LOAD) {
...
// ------------------------------------------------------------------------------------------------
/** Delete any scratch pad maps in the save directory.  Scratch pad maps are
 * maps that were embedded in previously loaded save game files and temporarily
 * written out as their own file so that those map files could be loaded as a
 * part of the load game process */
// ------------------------------------------------------------------------------------------------
void GameStateMap::clearScratchPadMaps() {
  ...
  while (done == FALSE) {
    ...
    //
    // find the next file before we delete this one, this is probably not
    // necessary to structure things this way so that the find next occurs
    // before the file delete, but it seems more correct to do so
    //
