...
void DeepCRCSanityCheck::reset() {
  ...
  AsciiString fname;
  fname.format("%sCRCAfter%dMaps.dat", TheGlobalData->getPath_UserData().str(),
               timesThrough);
...
static void updateWindowTitle() {
  // TheSuperHackers @tweak Now prints product and version information in the
  // Window title.
...
#endif
  UnicodeString gameTitle = TheGameText->FETCH_OR_SUBSTITUTE(
      "GUI:Command&ConquerGenerals", defaultGameTitle);
  ...
  if (!gameTitle.isEmpty()) {
    ...
    if (productString.isEmpty()) {
      ...
      UnicodeString gameTitleFormat =
          TheGameText->FETCH_OR_SUBSTITUTE("Version:GameTitle", L"for %ls");
...
GameEngine::~GameEngine() {
  ...
  // Reset all subsystems before deletion to prevent crashing due to cross
  // dependencies.
  reset();
...
Bool GameEngine::isTimeFrozen() {
  // TheSuperHackers @fix The time can no longer be frozen in Network games. It
...
void GameEngine::init() {
  try {
    ...
    if (TheVersion) {
      ...
      DEBUG_LOG(
          ("Build location: %s", TheVersion->getAsciiBuildLocation().str()));
      ...
      DEBUG_LOG(
          ("Build git version: %s", TheVersion->getAsciiGitTagOrHash().str()));
      DEBUG_LOG(("Build git commit time: %s",
                 TheVersion->getAsciiGitCommitTime().str()));
      DEBUG_LOG(
          ("Build git commit author: %s", Version::getGitCommitAuthorName()));
      DEBUG_LOG(("============================================================="
                 "==================="));
    }
#endif
    ...
    // not part of the subsystem list, because it should normally never be
    // reset!
    ...
    sprintf(Buf,
            "------------------------------------------------------------------"
            "----------After TheNameKeyGenerator  = %f seconds",
            ((double)(endTime64 - startTime64) / (double)(freq64)));
    startTime64 =
        endTime64; // Reset the clock
                   // ////////////////////////////////////////////////////////
    DEBUG_LOG((
        "%s",
        Buf)); ////////////////////////////////////////////////////////////////////////////
    ...
    initSubsystem(TheLocalFileSystem, "TheLocalFileSystem",
                  createLocalFileSystem(), nullptr);
    ...
    sprintf(Buf,
            "------------------------------------------------------------------"
            "----------After TheLocalFileSystem  = %f seconds",
            ((double)(endTime64 - startTime64) / (double)(freq64)));
    startTime64 =
        endTime64; // Reset the clock
                   // ////////////////////////////////////////////////////////
    ...
    DEBUG_LOG(("TRACE: BEFORE TheArchiveFileSystem"));
    initSubsystem(TheArchiveFileSystem, "TheArchiveFileSystem",
                  createArchiveFileSystem(),
                  nullptr); // this MUST come after TheLocalFileSystem creation
    DEBUG_LOG(("TRACE: AFTER TheArchiveFileSystem"));

#if defined(__EMSCRIPTEN__)
    // emscripten_sleep(0); // Yield to browser after file system init
#endif
    ...
    sprintf(Buf,
            "------------------------------------------------------------------"
            "----------After TheArchiveFileSystem  = %f seconds",
            ((double)(endTime64 - startTime64) / (double)(freq64)));
    startTime64 =
        endTime64; // Reset the clock
                   // ////////////////////////////////////////////////////////
    ...
    DEBUG_ASSERTCRASH(TheWritableGlobalData,
                      ("TheWritableGlobalData expected to be created"));
                      
    DEBUG_LOG(("TRACE: BEFORE TheWritableGlobalData initSubsystem"));
    initSubsystem(TheWritableGlobalData, "TheWritableGlobalData",
                  TheWritableGlobalData, &xferCRC,
                  "Data\\INI\\Default\\GameData", "Data\\INI\\GameData");
    DEBUG_LOG(("TRACE: AFTER TheWritableGlobalData initSubsystem"));
    
    DEBUG_LOG(("TRACE: BEFORE parseCustomDefinition"));
    TheWritableGlobalData->parseCustomDefinition();
    DEBUG_LOG(("TRACE: AFTER parseCustomDefinition"));
    ...
    sprintf(Buf,
            "------------------------------------------------------------------"
            "----------After  TheWritableGlobalData = %f seconds",
            ((double)(endTime64 - startTime64) / (double)(freq64)));
    startTime64 =
        endTime64; // Reset the clock
                   // ////////////////////////////////////////////////////////
    ...
    ini.loadFileDirectory("Data\\INI\\GameDataDebug", INI_LOAD_OVERWRITE,
                          nullptr);
#endif
    ...
    // after parsing the command line, we may want to perform dds stuff. Do that
    // here.
    ...
    // read the water settings from INI (must do prior to initing GameClient,
    // apparently)
    ini.loadFileDirectory("Data\\INI\\Default\\Water", INI_LOAD_OVERWRITE,
                          &xferCRC);
    ini.loadFileDirectory("Data\\INI\\Water", INI_LOAD_OVERWRITE, &xferCRC);
    ini.loadFileDirectory("Data\\INI\\Default\\Weather", INI_LOAD_OVERWRITE,
                          &xferCRC);
...
#ifdef DEBUG_CRC
    initSubsystem(TheDeepCRCSanityCheck, "TheDeepCRCSanityCheck",
                  MSGNEW("GameEngineSubystem") DeepCRCSanityCheck, nullptr);
#endif // DEBUG_CRC
    initSubsystem(TheGameText, "TheGameText", CreateGameTextInterface(),
                  nullptr);
    updateWindowTitle();

#if defined(__EMSCRIPTEN__)
    // emscripten_sleep(0); // Yield to browser after game text + data init
#endif
    ...
    initSubsystem(TheScienceStore, "TheScienceStore",
                  MSGNEW("GameEngineSubsystem") ScienceStore(), &xferCRC,
                  "Data\\INI\\Default\\Science", "Data\\INI\\Science");
    initSubsystem(TheMultiplayerSettings, "TheMultiplayerSettings",
                  MSGNEW("GameEngineSubsystem") MultiplayerSettings(), &xferCRC,
                  "Data\\INI\\Default\\Multiplayer", "Data\\INI\\Multiplayer");
    initSubsystem(TheTerrainTypes, "TheTerrainTypes",
                  MSGNEW("GameEngineSubsystem") TerrainTypeCollection(),
                  &xferCRC, "Data\\INI\\Default\\Terrain",
                  "Data\\INI\\Terrain");
    initSubsystem(TheTerrainRoads, "TheTerrainRoads",
                  MSGNEW("GameEngineSubsystem") TerrainRoadCollection(),
                  &xferCRC, "Data\\INI\\Default\\Roads", "Data\\INI\\Roads");
    initSubsystem(TheGlobalLanguageData, "TheGlobalLanguageData",
                  MSGNEW("GameEngineSubsystem") GlobalLanguage,
                  nullptr); // must be before the game text
    ...
    sprintf(Buf,
            "------------------------------------------------------------------"
            "----------After TheGlobalLanguageData = %f seconds",
            ((double)(endTime64 - startTime64) / (double)(freq64)));
    startTime64 =
        endTime64; // Reset the clock
                   // ////////////////////////////////////////////////////////
    DEBUG_LOG((
        ...
        Buf)); ////////////////////////////////////////////////////////////////////////////
#endif /////////////////////////////////////////////////////////////////////////////////////////////
    initSubsystem(TheAudio, "TheAudio",
                  TheGlobalData->m_headless ? NEW AudioManagerDummy
                                            : createAudioManager(),
                  nullptr);
    ...
    initSubsystem(TheFunctionLexicon, "TheFunctionLexicon",
                  createFunctionLexicon(), nullptr);
    initSubsystem(TheModuleFactory, "TheModuleFactory", createModuleFactory(),
                  nullptr);
    initSubsystem(TheMessageStream, "TheMessageStream", createMessageStream(),
                  nullptr);
    initSubsystem(TheSidesList, "TheSidesList",
                  MSGNEW("GameEngineSubsystem") SidesList(), nullptr);
    initSubsystem(TheCaveSystem, "TheCaveSystem",
                  MSGNEW("GameEngineSubsystem") CaveSystem(), nullptr);
    initSubsystem(TheRankInfoStore, "TheRankInfoStore",
                  MSGNEW("GameEngineSubsystem") RankInfoStore(), &xferCRC,
                  nullptr, "Data\\INI\\Rank");
    initSubsystem(ThePlayerTemplateStore, "ThePlayerTemplateStore",
                  MSGNEW("GameEngineSubsystem") PlayerTemplateStore(), &xferCRC,
                  "Data\\INI\\Default\\PlayerTemplate",
                  "Data\\INI\\PlayerTemplate");
    initSubsystem(TheParticleSystemManager, "TheParticleSystemManager",
                  createParticleSystemManager(), nullptr);
    ...
    sprintf(Buf,
            "------------------------------------------------------------------"
            "----------After TheParticleSystemManager = %f seconds",
            ((double)(endTime64 - startTime64) / (double)(freq64)));
    startTime64 =
        endTime64; // Reset the clock
                   // ////////////////////////////////////////////////////////
    ...
    initSubsystem(TheFXListStore, "TheFXListStore",
                  MSGNEW("GameEngineSubsystem") FXListStore(), &xferCRC,
                  "Data\\INI\\Default\\FXList", "Data\\INI\\FXList");
    initSubsystem(TheWeaponStore, "TheWeaponStore",
                  MSGNEW("GameEngineSubsystem") WeaponStore(), &xferCRC,
                  nullptr, "Data\\INI\\Weapon");
    initSubsystem(TheObjectCreationListStore, "TheObjectCreationListStore",
                  MSGNEW("GameEngineSubsystem") ObjectCreationListStore(),
                  &xferCRC, "Data\\INI\\Default\\ObjectCreationList",
                  "Data\\INI\\ObjectCreationList");
    initSubsystem(TheLocomotorStore, "TheLocomotorStore",
                  MSGNEW("GameEngineSubsystem") LocomotorStore(), &xferCRC,
                  nullptr, "Data\\INI\\Locomotor");
    initSubsystem(TheSpecialPowerStore, "TheSpecialPowerStore",
                  MSGNEW("GameEngineSubsystem") SpecialPowerStore(), &xferCRC,
                  "Data\\INI\\Default\\SpecialPower",
                  "Data\\INI\\SpecialPower");
    initSubsystem(TheDamageFXStore, "TheDamageFXStore",
                  MSGNEW("GameEngineSubsystem") DamageFXStore(), &xferCRC,
                  nullptr, "Data\\INI\\DamageFX");
    initSubsystem(TheArmorStore, "TheArmorStore",
                  MSGNEW("GameEngineSubsystem") ArmorStore(), &xferCRC, nullptr,
                  "Data\\INI\\Armor");
    initSubsystem(TheBuildAssistant, "TheBuildAssistant",
                  MSGNEW("GameEngineSubsystem") BuildAssistant, nullptr);

#if defined(__EMSCRIPTEN__)
    // emscripten_sleep(0); // Yield to browser before heavy Object/Client init
#endif
    ...
    initSubsystem(TheThingFactory, "TheThingFactory", createThingFactory(),
                  &xferCRC, "Data\\INI\\Default\\Object", "Data\\INI\\Object");

#if defined(__EMSCRIPTEN__)
    // emscripten_sleep(0); // Yield after Object INI loading (very heavy)
#endif
    ...
    initSubsystem(TheUpgradeCenter, "TheUpgradeCenter",
                  MSGNEW("GameEngineSubsystem") UpgradeCenter, &xferCRC,
                  "Data\\INI\\Default\\Upgrade", "Data\\INI\\Upgrade");
    initSubsystem(TheGameClient, "TheGameClient", createGameClient(), nullptr);

#if defined(__EMSCRIPTEN__)
    // emscripten_sleep(0); // Yield to browser after GameClient init (heavy UI/font work)
#endif
    ...
    initSubsystem(TheAI, "TheAI", MSGNEW("GameEngineSubsystem") AI(), &xferCRC,
                  "Data\\INI\\Default\\AIData", "Data\\INI\\AIData");
    initSubsystem(TheGameLogic, "TheGameLogic", createGameLogic(), nullptr);
    initSubsystem(TheTeamFactory, "TheTeamFactory",
                  MSGNEW("GameEngineSubsystem") TeamFactory(), nullptr);
    initSubsystem(TheCrateSystem, "TheCrateSystem",
                  MSGNEW("GameEngineSubsystem") CrateSystem(), &xferCRC,
                  "Data\\INI\\Default\\Crate", "Data\\INI\\Crate");
    initSubsystem(ThePlayerList, "ThePlayerList",
                  MSGNEW("GameEngineSubsystem") PlayerList(), nullptr);
    initSubsystem(TheRecorder, "TheRecorder", createRecorder(), nullptr);
    initSubsystem(TheRadar, "TheRadar",
                  TheGlobalData->m_headless ? NEW RadarDummy : createRadar(),
                  nullptr);
    initSubsystem(TheVictoryConditions, "TheVictoryConditions",
                  createVictoryConditions(), nullptr);
    ...
    sprintf(Buf,
            "------------------------------------------------------------------"
            "----------After TheVictoryConditions = %f seconds",
            ((double)(endTime64 - startTime64) / (double)(freq64)));
    startTime64 =
        endTime64; // Reset the clock
                   // ////////////////////////////////////////////////////////
    ...
    initSubsystem(TheMetaMap, "TheMetaMap",
                  MSGNEW("GameEngineSubsystem") MetaMap(), nullptr, fname.str(),
                  "Data\\INI\\CommandMap");

    TheMetaMap->generateMetaMap();

#if defined(RTS_DEBUG)
    ini.loadFileDirectory("Data\\INI\\CommandMapDebug", INI_LOAD_MULTIFILE,
                          nullptr);
#endif

#if defined(_ALLOW_DEBUG_CHEATS_IN_RELEASE)
    ini.loadFileDirectory("Data\\INI\\CommandMapDemo", INI_LOAD_MULTIFILE,
                          nullptr);
#endif

    initSubsystem(TheActionManager, "TheActionManager",
                  MSGNEW("GameEngineSubsystem") ActionManager(), nullptr);
    // initSubsystem((CComObject<WebBrowser>
    // *)TheWebBrowser,"(CComObject<WebBrowser> *)TheWebBrowser",
    // (CComObject<WebBrowser> *)createWebBrowser(), nullptr);
    initSubsystem(TheGameStateMap, "TheGameStateMap",
                  MSGNEW("GameEngineSubsystem") GameStateMap, nullptr);
    initSubsystem(TheGameState, "TheGameState",
                  MSGNEW("GameEngineSubsystem") GameState, nullptr);

    // Create the interface for sending game results
    initSubsystem(TheGameResultsQueue, "TheGameResultsQueue",
    ...
    sprintf(Buf,
            "------------------------------------------------------------------"
            "----------After TheGameResultsQueue = %f seconds",
            ((double)(endTime64 - startTime64) / (double)(freq64)));
    startTime64 =
        endTime64; // Reset the clock
                   // ////////////////////////////////////////////////////////
...
#if defined(__EMSCRIPTEN__)
    // emscripten_sleep(0); // Yield to browser after postProcessLoad
#endif

    TheFramePacer->setFramesPerSecondLimit(
        TheGlobalData->m_framesPerSecondLimit);

    TheAudio->setOn(TheGlobalData->m_audioOn && TheGlobalData->m_musicOn,
                    AudioAffect_Music);
    TheAudio->setOn(TheGlobalData->m_audioOn && TheGlobalData->m_soundsOn,
                    AudioAffect_Sound);
    TheAudio->setOn(TheGlobalData->m_audioOn && TheGlobalData->m_sounds3DOn,
                    AudioAffect_Sound3D);
    TheAudio->setOn(TheGlobalData->m_audioOn && TheGlobalData->m_speechOn,
                    AudioAffect_Speech);
    ...
    // If we turn m_quitting to FALSE here, then we throw away any requests to
    // quit that took place during loading. :-\ - jkmcd If this really needs to
    // take place, please make sure that pressing cancel on the audio load music
    // dialog will still cause the game to quit. m_quitting = FALSE;
    ...
    sprintf(Buf,
            "------------------------------------------------------------------"
            "----------After TheMapCache->updateCache = %f seconds",
            ((double)(endTime64 - startTime64) / (double)(freq64)));
    startTime64 =
        endTime64; // Reset the clock
                   // ////////////////////////////////////////////////////////
    ...
    char buf[1024];
    snprintf(buf, sizeof(buf), "INIError exception during init: %d", (int)e);
    RELEASE_CRASH((buf));
  } catch (std::exception& e) {
    char buf[1024];
    snprintf(buf, sizeof(buf), "std::exception during initialization: %s", e.what());
    RELEASE_CRASH((buf));
  } catch (const char* msg) {
    char buf[1024];
    snprintf(buf, sizeof(buf), "const char* exception during initialization: %s", msg);
    RELEASE_CRASH((buf));
  } catch (XferStatus xs) {
    char buf[1024];
    snprintf(buf, sizeof(buf), "XferStatus exception during init: %d", (int)xs);
    RELEASE_CRASH((buf));
  } catch (int e) {
    char buf[1024];
    snprintf(buf, sizeof(buf), "int exception during init: %d", e);
    RELEASE_CRASH((buf));
  } catch (unsigned int e) {
    char buf[1024];
    snprintf(buf, sizeof(buf), "uint exception during init: %u", e);
    RELEASE_CRASH((buf));
  } catch (AsciiString& as) {
    char buf[1024];
    snprintf(buf, sizeof(buf), "AsciiString exception during init: %s", as.str());
    RELEASE_CRASH((buf));
  } catch (UnicodeString& us) {
    char buf[1024];
    snprintf(buf, sizeof(buf), "UnicodeString exception during init: (length %d)", us.getLength());
    DEBUG_LOG(("FATAL: %s", buf));
    RELEASE_CRASH((buf));
  } catch (...) {
    DEBUG_LOG(("FATAL: MY CUSTOM CRASH GameEngine.cpp fell to catch-all."));
    RELEASE_CRASH(("MY CUSTOM CRASH GameEngine.cpp fell to catch-all."));
  }
...
/** -----------------------------------------------------------------------------------------------
 * Reset all necessary parts of the game engine to be ready to accept new game
 * data
 */
void GameEngine::reset() {

  WindowLayout *background =
      TheWindowManager->winCreateLayout("Menus/BlankWindow.wnd");
...
void GameEngine::resetSubsystems() {
  // TheSuperHackers @fix xezon 09/06/2025 Reset GameLogic first to purge all
  // world objects early. This avoids potentially catastrophic issues when
  // objects and subsystems have cross dependencies.
  TheGameLogic->reset();
...
Bool GameEngine::canUpdateRegularGameLogic() {
  ...
  const Bool useFastMode =
      TheGlobalData->m_TiVOFastMode && TheGameLogic->isInReplayGame();
#endif

  if (useFastMode || !enabled || logicTimeScaleFps >= maxRenderFps) {
    // Logic time scale is uncapped or larger equal Render FPS. Update straight
    // away.
    return true;
  } else {
    ...
    m_logicTimeAccumulator +=
        min(TheFramePacer->getUpdateTime(), targetFrameTime);
...
void GameEngine::update() {
  USE_PERF_TIMER(GameEngine_update) {
    {
      // VERIFY CRC needs to be in this code block.  Please to not pull
      // TheGameLogic->update() inside this block.
      VERIFY_CRC
...

    {
...

      {
        try {
          ...
          // Release CRASH doesn't return, so don't worry about executing
          // additional code.
          if (e.mFailureMessage)
            RELEASE_CRASH((e.mFailureMessage));
          else
          ...
          try {
            if (TheRecorder &&
                TheRecorder->getMode() == RECORDERMODETYPE_RECORD &&
...
//-------------------------------------------------------------------------------------------------
Bool GameEngine::isMultiplayerSession() { return TheRecorder->isMultiplayer(); }

//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
#define CONVERT_EXEC1                                                          \
  "..\\Build\\nvdxt -list buildDDS.txt -dxt5 -full -outdir Art\\Textures > "   \
  "buildDDS.out"

void updateTGAtoDDS() {
  // Here's the scoop. We're going to traverse through all of the files in the
  // Art\Textures folder and determine if there are any .tga files that are
  // newer than associated .dds files. If there are, then we will re-run the
  // compression tool on them.

  File *fp = TheLocalFileSystem->openFile(
      "buildDDS.txt", File::WRITE | File::CREATE | File::TRUNCATE | File::TEXT);
  if (!fp) {
    return;
  }

  FilenameList files;
  TheLocalFileSystem->getFileListInDirectory("Art\\Textures\\", "", "*.tga",
                                             files, TRUE);
...
// If we're using the Wide character version of MessageBox, then there's no
// additional processing necessary. Please note that this is a sleazy way to get
// this information, but pending a better one, this'll have to do.
#ifndef __EMSCRIPTEN__
extern const Bool TheSystemIsUnicode =
    (((void *)(::MessageBox)) == ((void *)(::MessageBoxW)));
#else
