...
Bool g_verifyClientCRC =
    FALSE; // verify that GameLogic CRC doesn't change from client
...
Int parseReplay(char *args[], int num) {
  if (num > 1) {
    ...
    // Make replay playback possible while other clients (possible retail) are
    // running
...
Int parseJobs(char *args[], int num) {
  if (num > 1) {
    ...
    if (TheGlobalData->m_simulateReplayJobs < SIMULATE_REPLAYS_SEQUENTIAL ||
        TheGlobalData->m_simulateReplayJobs == 0) {
...
Int parseNoShaders(char *args[], int) {
  TheWritableGlobalData->m_chipSetType =
      1; // force to a voodoo card which uses least amount of features.
...
Int parseMod(char *args[], Int num) {
  if (num > 1) {
    ...
    if (strchr(modPath.str(), ':') || modPath.startsWith("/") ||
        modPath.startsWith("\\")) {
...
static CommandLineParam paramsForStartup[] = {
    ...
    // This runs the game without a window, graphics, input and audio. You can
    // combine this with -replay
    ...
    // You can also include wildcards. The file must be in the replay folder or
    // in a subfolder.
    {"-replay", parseReplay},

    // TheSuperHackers @feature helmutbuhler 23/05/2025
    // Simulate each replay in a separate process and use 1..N processes at the
    // same time. (If you have 4 cores, call it with -jobs 4) If you do not call
    // this, all replays will be simulated in sequence in the same process.
    {"-jobs", parseJobs},
};
...
static CommandLineParam paramsForEngineInit[] = {
    {"-nologo",
     parseNoLogo}, // TheSuperHackers @tweak Is now available in Release builds.
    {"-noshellmap", parseNoShellMap},
    {"-noShellAnim", parseNoWindowAnimation}, // TheSuperHackers @tweak Is now
    ...
    // TheSuperHackers @feature xezon 03/08/2025 Force full viewport for
    // 'Control Bar Pro' Addons like GenTool did it.
    ...
    // Note that you need to have a debug or internal configuration build in
    // order to use this. Release configuration also works if
    // RELEASE_DEBUG_LOGGING is defined in Debug.h Also note that all players
    // need to play in the same configuration, otherwise mismatch will occur
    // almost immediately. Try this if you want to play the game and have useful
    // debug information in case mismatch occurs: -ignoreAsserts
    // -DebugCRCFromFrame 0 -VerifyClientCRC -LogObjectCRCs -NetCRCInterval 1
    // After mismatch occurs, you can examine the logfile and also reproduce the
    // crc from the replay with this (and diff that with the log):
    // -ignoreAsserts -DebugCRCFromFrame xxx -LogObjectCRCs
    // -SaveDebugCRCPerFrame crc

    // After which frame to log crc logging. Call with 0 to log all frames and
    // with -1 to log none (default).
    {"-DebugCRCFromFrame", parseDebugCRCFromFrame},

    // Last frame to log
    {"-DebugCRCUntilFrame", parseDebugCRCUntilFrame},

    // Save data involving CRC calculation to a binary file. (This isn't that
    // useful.)
    ...
    // Note that the passed folder is deleted if it already exists for every
    // started game.
    ...
    // Write out binary crc data pre and post client update to "clientPre.crc"
    // and "clientPost.crc"
    ...
    // Number of frames between each CRC check between all players in
    // multiplayer games (if not all crcs are equal, mismatch occurs).
    {"-NetCRCInterval", parseNetCRCInterval},

    // Number of frames between each CRC that is written to replay files in
    // singleplayer games.
...
static void parseCommandLine(const CommandLineParam *params, int numParams) {
...
#else
  std::string cmdLine = "generals"; // dummy command line
#endif
  ...
  // look at the next argument(s), to accommodate multi-arg parameters, e.g. "-p
  // 1234".
  while (arg < argc) {
    // Look at arg #i
    Bool found = false;
...
void CommandLine::parseCommandLineForStartup() {
  ...
  // Note that this function is potentially called multiple times and only
  // initializes the first time.
  createGlobalData();

  if (TheGlobalData->m_commandLineData.m_hasParsedCommandLineForStartup)
    return;
  TheWritableGlobalData->m_commandLineData.m_hasParsedCommandLineForStartup =
      true;
...
void CommandLine::parseCommandLineForEngineInit() {
  ...
  DEBUG_ASSERTCRASH(
      ...
      ("parseCommandLineForStartup is expected to be called before "
