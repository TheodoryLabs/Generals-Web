...
TransportContainModuleData::TransportContainModuleData() {
  ...
  //
  // by default we say that transports can have infantry inside them, this will
  // be totally overwritten by any data provided from the INI entry tho
  //
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
void TransportContainModuleData::parseInitialPayload(
...
void TransportContainModuleData::buildFieldParse(MultiIniFieldParse &p) {
  ...
  static const FieldParse dataFieldParse[] = {
      {"Slots", INI::parseInt, nullptr,
       offsetof(TransportContainModuleData, m_slotCapacity)},
      {"ScatterNearbyOnExit", INI::parseBool, nullptr,
       offsetof(TransportContainModuleData, m_scatterNearbyOnExit)},
      {"OrientLikeContainerOnExit", INI::parseBool, nullptr,
       offsetof(TransportContainModuleData, m_orientLikeContainerOnExit)},
      {"KeepContainerVelocityOnExit", INI::parseBool, nullptr,
       offsetof(TransportContainModuleData, m_keepContainerVelocityOnExit)},
      {"GoAggressiveOnExit", INI::parseBool, nullptr,
       offsetof(TransportContainModuleData, m_goAggressiveOnExit)},
      {"ResetMoodCheckTimeOnExit", INI::parseBool, nullptr,
       offsetof(TransportContainModuleData, m_resetMoodCheckTimeOnExit)},
      {"DestroyRidersWhoAreNotFreeToExit", INI::parseBool, nullptr,
       offsetof(TransportContainModuleData,
                m_destroyRidersWhoAreNotFreeToExit)},
      {"ExitBone", INI::parseAsciiString, nullptr,
       offsetof(TransportContainModuleData, m_exitBone)},
      {"ExitPitchRate", INI::parseAngularVelocityReal, nullptr,
       offsetof(TransportContainModuleData, m_exitPitchRate)},
      {"InitialPayload", parseInitialPayload, nullptr, 0},
      {"HealthRegen%PerSec", INI::parseReal, nullptr,
       offsetof(TransportContainModuleData, m_healthRegen)},
      {"ExitDelay", INI::parseDurationUnsignedInt, nullptr,
       offsetof(TransportContainModuleData, m_exitDelay)},
      {"ArmedRidersUpgradeMyWeaponSet", INI::parseBool, nullptr,
       offsetof(TransportContainModuleData, m_armedRidersUpgradeWeaponSet)},
      {"DelayExitInAir", INI::parseBool, nullptr,
       offsetof(TransportContainModuleData, m_isDelayExitInAir)},
...
Bool TransportContain::isValidContainerFor(const Object *rider,
  ...
  // The point of this new code is to determine when something is a "fake"
  // container, to look at the object inside of it to use that as the valid
  // check. There is a case, when a paratrooper (an infantry contained in a
  // parachute). In this case, when we pass this object to contain in a
  // transport plane, we want to check the infantry, not the parachute.
  if (rider->getContain() &&
      rider->getContain()->isSpecialZeroSlotContainer()) {
    // Report the first thing inside it!
    const ContainedItemsList *items =
        rider->getContain()->getContainedItemsList();
    if (items && !items->empty()) {
      if (items->front()) {
        // Replace the object we are checking with the *first* object contained
        // within it.
        rider = items->front();
      }
    }
  ...
  //	// order matters: we want to know if I consider it to be an ally, not
  //vice versa 	if (getObject()->getRelationship(rider) != ALLIES) 		return false;
  ...
  if (checkCapacity) {
    ...
    return (m_extraSlotsInUse + containCount + transportSlotCount <=
            containMax);
    ...
    while (it != riderList->end()) {
      ...
      if (rider) {
        ...
        for (Int w = PRIMARY_WEAPON; w < WEAPONSLOT_COUNT; ++w) {
          ...
          if (weapon) {
            if (weapon->getTemplate()->isContactWeapon() == FALSE &&
                weapon->isDamageWeapon() ==
                    TRUE) // THIS MAY NEED TO CHECK MORE WEAPON ATTRIBUTES TO
                          // WORK BEST
            {
...
void TransportContain::onContaining(Object *rider, Bool wasSelected) {
  ...
  DEBUG_ASSERTCRASH(transportSlotCount > 0,
                    ("Hmm, this object isnt transportable"));
  m_extraSlotsInUse += transportSlotCount - 1;
  DEBUG_ASSERTCRASH(m_extraSlotsInUse >= 0 &&
                        m_extraSlotsInUse + getContainCount() <=
                            getContainMax(),
                    ("Hmm, bad slot count"));

  //
  // when we go from holding nothing to holding something we have a model
  // condition to visually show the change
  //
  ...
  // Force Jarmen Kell to transfer weapon timer for snipe to and from the combat
  // bike.
  if (getObject()->isKindOf(KINDOF_CLIFF_JUMPER) &&
      rider->isKindOf(KINDOF_HERO) && rider->isKindOf(KINDOF_SALVAGER)) {
    // Admittedly brutal hack, but considering the state of the game
    // (post-ship), this is a particularly surgical fix.
...
void TransportContain::onRemoving(Object *rider) {
  ...
  if (!d->m_exitBone.isEmpty()) {
    ...
    if (draw) {
      ...
      if (draw->getPristineBonePositions(d->m_exitBone.str(), 0, &bonePos,
                                         nullptr, 1) == 1) {
        getObject()->convertBonePosToWorldPos(&bonePos, nullptr, &worldPos,
                                              nullptr);
  ...
  // AI might need help using this transport in a good way.  Make the passengers
  // aggressive.
  // There is no computer player check since Aggressive only means something for
  // computer players anyway
...
void TransportContain::createPayload() {
  TransportContainModuleData *self =
      (TransportContainModuleData *)getTransportContainModuleData();

  Int count = self->m_initialPayload.count;
  const ThingTemplate *payloadTemplate =
      TheThingFactory->findTemplate(self->m_initialPayload.name);
  ...
  if (contain) {
    ...
    for (int i = 0; i < count; i++) {
      // We are creating a transport that comes with a initial payload, so add
      // it now!
      Object *payload = TheThingFactory->newObject(
          payloadTemplate, object->getControllingPlayer()->getDefaultTeam());
      if (contain->isValidContainerFor(payload, true)) {
        contain->addToContain(payload);
      } else {
        DEBUG_CRASH(("DeliverPayload: PutInContainer %s is full, or not valid "
                     "for the payload %s!",
                     object->getName().str(),
...
UpdateSleepTime TransportContain::update() {
  const TransportContainModuleData *moduleData =
  ...
  if (moduleData && moduleData->m_healthRegen) {
    ...
    if (contain) {
      // This transport has a health regeneration value, so go through and heal
      // all inside.
      ...
      if (items) {
        ...
        while (it != items->end()) {
          ...
          if (body->getHealth() < body->getMaxHealth()) {
            ...
            Real regen = body->getMaxHealth() * moduleData->m_healthRegen /
                         100.0f * SECONDS_PER_LOGICFRAME_REAL;

            // Perform the actual healing for this frame.
            //						DamageInfo damageInfo;
            //						damageInfo.in.m_damageType
            //= DAMAGE_HEALING; 						damageInfo.in.m_deathType = DEATH_NONE;
            //						damageInfo.in.m_sourceID
            //= getObject()->getID(); 						damageInfo.in.m_amount = regen;
...
void TransportContain::killRidersWhoAreNotFreeToExit() {
  ...
  while (it != getContainList().end()) {
    // get the object
    Object *obj = *it;
    // increment the iterator, since death will pull this guy from the list
    // somewhere else
    ++it;

    if (!isSpecificRiderFreeToExit(obj)) // only TransportContain has a
                                         // meaningful failure for isFreeToExit
    {
      // If we cannot exit this guy legally, kill the bastard before
      // removeAllContained forces him out.
...
Bool TransportContain::isSpecificRiderFreeToExit(Object *specificObject) {
  ...
  // this is present solely for some transports to override, so that they can
  // land before allowing people to exit...
  ...
  const Locomotor *hisLocomotor =
      specificObject->getAIUpdateInterface()->getCurLocomotor();
  if (hisLocomotor == nullptr)
    return FALSE;

  // He can't get to this spot naturally, so I can't force him there.  (amphib
  // transport)
  if (!TheAI->pathfinder()->validMovementTerrain(me->getLayer(), hisLocomotor,
                                                 myPosition))
    return FALSE;
...
Bool TransportContain::isPassengerAllowedToFire(ObjectID id) const {
  ...
  // This code detects the case of whether the contained passenger is in a
  // bunker riding on an overlord, inside a helix! Oh  my  God.
  const Object *heWhoContainsMe = getObject()->getContainedBy();
  if (heWhoContainsMe) {
    ContainModuleInterface *hisContain = heWhoContainsMe->getContain();
    DEBUG_ASSERTCRASH(hisContain,
                      ("TransportContain::isPassengerAllowedToFire()... "
                       "CONTAINER WITHOUT A CONTAIN! AARRGH!"));
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
ExitDoorType TransportContain::reserveDoorForExit(const ThingTemplate *objType,
                                                  Object *specificObject) {
  return isSpecificRiderFreeToExit(specificObject) ? DOOR_1
                                                   : DOOR_NONE_AVAILABLE;
}

// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
Bool TransportContain::isExitBusy()
    const ///< Contain style exiters are getting the ability to space out exits,
          ///< so ask this before reserveDoor as a kind of no-commitment check.
{
...
void TransportContain::onCapture(Player *oldOwner, Player *newOwner) {
  if (oldOwner != newOwner) {
    if (getObject()->isDisabledByType(DISABLED_UNMANNED)) {
      // If this vehicle was sniped, then instantly eject everyone (otherwise,
      // the next guy to eject will recapture it).
      removeAllContained();
    } else {
      // Use standard
