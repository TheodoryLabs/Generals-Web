...
static void initGadgetsFullQuit() {
  ...
  buttonSaveLoad =
      TheNameKeyGenerator->nameToKey("QuitMenu.wnd:ButtonSaveLoad");

  buttonRestartWin =
      TheWindowManager->winGetWindowFromId(nullptr, buttonRestart);
  buttonSaveLoadWin =
      TheWindowManager->winGetWindowFromId(nullptr, buttonSaveLoad);
  buttonOptionsWin =
      TheWindowManager->winGetWindowFromId(nullptr, buttonOptions);
...
static void initGadgetsNoSaveQuit() {
  ...
  buttonRestart =
      TheNameKeyGenerator->nameToKey("QuitNoSave.wnd:ButtonRestart");
  buttonReturn = TheNameKeyGenerator->nameToKey("QuitNoSave.wnd:ButtonReturn");
  buttonOptions =
      TheNameKeyGenerator->nameToKey("QuitNoSave.wnd:ButtonOptions");
...
static void exitQuitMenu() {
  ...
  if (TheGameLogic->isInMultiplayerGame() &&
      !TheGameLogic->isInSkirmishGame() && !TheGameInfo->isSandbox()) {
    GameMessage *msg =
        TheMessageStream->appendMessage(GameMessage::MSG_SELF_DESTRUCT);
...
static void noExitQuitMenu() { quitConfirmationWindow = nullptr; }
...
static void restartMissionMenu() {
  ...
  // TheSuperHackers @bugfix Caball009 07/02/2026 Reuse the previous seed value
  // for the new skirmish match to prevent mismatches. Campaign, challenge, and
  // skirmish single-player scenarios all use GAME_SINGLE_PLAYER and are
  // expected to use 0 as seed value.
  DEBUG_ASSERTCRASH((TheSkirmishGameInfo != nullptr) ==
                        (gameMode == GAME_SKIRMISH),
                    ("Unexpected game mode on map / mission restart"));
  ...
  Int rankPointsStartedWith =
      TheGameLogic
          ->getRankPointsToAddAtGameStart(); // must write down before reset
...
//-------------------------------------------------------------------------------------------------
void HideQuitMenu() {
  // Note: This is called as a safety a lot, without checking for the presence
  // of the quit menu. So don't do anything that counts on that menu actually
  // being here.
  if (!isVisible)
    return;
...
void ToggleQuitMenu() {
  ...
  // BGC- If we are currently in the disconnect screen, don't let the quit menu
  // come up.
  ...
  // Basically we're seeing if either the save/load window or the options window
  // is up and if one of them is, we quit out of them rather than toggle the
  // quit menu.
  if (TheShell->getOptionsLayout(FALSE) != nullptr) {
    ...
    DEBUG_ASSERTCRASH(optionsParent != nullptr,
                      ("Not able to get the options layout parent window"));
    GameWindow *optionsBack = TheWindowManager->winGetWindowFromId(
        optionsParent,
        TheNameKeyGenerator->nameToKey("OptionsMenu.wnd:ButtonBack"));
    DEBUG_ASSERTCRASH(
        optionsBack != nullptr,
        ("Not able to get the back button window from the options menu"));
    TheWindowManager->winSendSystemMsg(optLayout->getFirstWindow(),
                                       GBM_SELECTED, (WindowMsgData)optionsBack,
                                       0);
    return;
  }
  if ((saveLoadMenuLayout != nullptr) &&
      (saveLoadMenuLayout->isHidden() == FALSE)) {
    GameWindow *saveLoadParent = saveLoadMenuLayout->getFirstWindow();
    DEBUG_ASSERTCRASH(saveLoadParent != nullptr,
                      ("Not able to get the save/load layout parent window"));
    GameWindow *saveLoadBack = TheWindowManager->winGetWindowFromId(
        saveLoadParent,
        TheNameKeyGenerator->nameToKey("PopupSaveLoad.wnd:ButtonBack"));
    DEBUG_ASSERTCRASH(
        saveLoadBack != nullptr,
        ("Not able to get the back button window from the save/load menu"));
    TheWindowManager->winSendSystemMsg(saveLoadMenuLayout->getFirstWindow(),
                                       GBM_SELECTED,
                                       (WindowMsgData)saveLoadBack, 0);
  ...
  if (isVisible && quitMenuLayout) {
    ...
    if (TheGameLogic->isInMultiplayerGame() || TheGameLogic->isInReplayGame()) {
      ...
      if (!noSaveLoadQuitMenuLayout)
        noSaveLoadQuitMenuLayout =
            TheWindowManager->winCreateLayout("Menus/QuitNoSave.wnd");
      ...
      if (!fullQuitMenuLayout)
        fullQuitMenuLayout =
            TheWindowManager->winCreateLayout("Menus/QuitMenu.wnd");
    ...
    if (TheGameLogic->isInMultiplayerGame() ||
      ...
      if (TheGameLogic->isInSkirmishGame() == FALSE) {
        GadgetButtonSetText(buttonRestartWin,
                            TheGameText->fetch("GUI:Surrender"));
      }
      ...
      if ((!ThePlayerList->getLocalPlayer()->isPlayerActive() ||
           TheVictoryConditions->isLocalAlliedVictory()) &&
      ...
      if (!TheGameLogic->isInReplayGame()) {
        GadgetButtonSetText(buttonRestartWin,
                            TheGameText->fetch("GUI:RestartMission"));
        GadgetButtonSetText(buttonExitWin,
                            TheGameText->fetch("GUI:ExitMission"));
      }
...
WindowMsgHandledType QuitMenuSystem(GameWindow *window, UnsignedInt msg,
  ...
  case GBM_SELECTED: {
    ...
    if (controlID == buttonSaveLoad) {
      ...
      //
      // these commented lines (12-11-2002) will allow access to load only when
      // were viewing an in-game cinema ... but it's brittle, so I'm disableing
      // it for now and just using the grey button for the whole save/load
      // button for now during a cinema
      //
      ...
      //
      // if input is disabled we are in an in-game cinematic and can load only,
      // since we are in the quit menu we are paused and therefore input is
      // disabled all the time ... in order to figure out if the *game* has
      // input disabled we need to query for the input enabled memory in the
      // game logic which represents the input enabled status of the game if we
      // were not currently paused
      //
      //				if(
      //TheGameLogic->getInputEnabledMemory() == FALSE ) 					layoutType =
      //SLLT_LOAD_ONLY;
      ...
      quitConfirmationWindow =
          QuitMessageBoxYesNo(TheGameText->fetch("GUI:QuitPopupTitle"),
                              TheGameText->fetch("GUI:QuitPopupMessage"),
                              /*quitCallback*/ exitQuitMenu, noExitQuitMenu);
    ...
    //			{
    //				quitConfirmationWindow =
    //MessageBoxYesNo(TheGameText->fetch("GUI:QuitPopupTitle"),
    //TheGameText->fetch("GUI:QuitToDesktopConf"),/*quitCallback*/quitToDesktopQuitMenu,noExitQuitMenu);
    //
    //			}  // end else if
    else if (controlID == buttonRestart) {
      if (TheGameLogic->isInMultiplayerGame()) {
        // we really want to surrender
        quitConfirmationWindow = MessageBoxYesNo(
            TheGameText->fetch("GUI:SurrenderConfirmationTitle"),
            TheGameText->fetch("GUI:SurrenderConfirmation"),
            /*quitCallback*/ surrenderQuitMenu, noExitQuitMenu);
      } else {
        // we really want to restart
        quitConfirmationWindow = MessageBoxYesNo(
            TheGameText->fetch("GUI:RestartConfirmationTitle"),
