...
struct LayoutScriptParse {
  ...
  const char *name;
  Bool (*parse)(const char *token, char *buffer, UnsignedInt version,
...
const char *const WindowStatusNames[] = {
    ...
    "BORDER",        "SMOOTH_TEXT",
    "ONE_LINE",      "NO_FLUSH",
    "SEE_THRU",      "RIGHT_CLICK",
    "WRAP_CENTERED", "CHECK_LIKE",
    "HOTKEY_TEXT",   "USE_OVERLAY_STATES",
    "NOT_READY",     "FLASHING",
    "ALWAYS_COLOR",  "ON_MOUSE_DOWN", /*"SHORTCUT_BUTTON",*/
    nullptr};

const char *const WindowStyleNames[] = {
    "PUSHBUTTON",    "RADIOBUTTON", "CHECKBOX",   "VERTSLIDER",  "HORZSLIDER",
    "SCROLLLISTBOX", "ENTRYFIELD",  "STATICTEXT", "PROGRESSBAR", "USER",
    "MOUSETRACK",    "ANIMATED",    "TABSTOP",    "TABCONTROL",  "TABPANE",
    "COMBOBOX",      nullptr};
...
WinDrawData
    enabledUpButtonDrawData[MAX_DRAW_DATA]; ///< for list boxes and combo boxes
WinDrawData
    disabledUpButtonDrawData[MAX_DRAW_DATA]; ///< for list boxes and combo boxes
WinDrawData
    hiliteUpButtonDrawData[MAX_DRAW_DATA]; ///< for list boxes and combo boxes
WinDrawData enabledDownButtonDrawData[MAX_DRAW_DATA];  ///< for list boxes and
                                                       ///< combo boxes
WinDrawData disabledDownButtonDrawData[MAX_DRAW_DATA]; ///< for list boxes and
                                                       ///< combo boxes
WinDrawData
    hiliteDownButtonDrawData[MAX_DRAW_DATA]; ///< for list boxes and combo boxes
WinDrawData
    enabledSliderDrawData[MAX_DRAW_DATA]; ///< for list boxes and combo boxes
WinDrawData
    disabledSliderDrawData[MAX_DRAW_DATA]; ///< for list boxes and combo boxes
WinDrawData
    hiliteSliderDrawData[MAX_DRAW_DATA]; ///< for list boxes and combo boxes
WinDrawData
    enabledSliderThumbDrawData[MAX_DRAW_DATA]; ///< for sliders and list boxes
                                               ///< and combo boxes
WinDrawData
    disabledSliderThumbDrawData[MAX_DRAW_DATA]; ///< for sliders and list boxes
                                                ///< and combo boxes
WinDrawData hiliteSliderThumbDrawData[MAX_DRAW_DATA]; ///< for sliders and list
...
//=============================================================================
static void parseBitString(const char *inBuffer, UnsignedInt *bits,
                           ConstCharPtrArray flagList) {
  char buffer[256];
  char *tok;
...
static void readUntilSemicolon(File *fp, char *buffer, int maxBufLen) {
  ...
  DEBUG_LOG(
      ("ReadUntilSemicolon: ERROR - Read buffer overflow - input truncated."));
...
static Int scanInt(const char *source, Int &val) {
  Int ret = sscanf(
      source, "%d",
      &val); // not strictly necessary to wrap this, but it's more consistent
 ...
 * if present */
//=============================================================================
static Bool parseScreenRect(const char *token, char *buffer, Int *x, Int *y,
...
static Bool parseFont(const char *token, WinInstanceData *instData,
  ...
  if (TheFontLibrary) {
    GameFont *font =
        TheFontLibrary->getFont(AsciiString(fontName), fontSize, fontBold);
    if (font)
...
static Bool parseName(const char *token, WinInstanceData *instData,
  ...
  if (TheNameKeyGenerator)
    instData->m_id =
        (Int)TheNameKeyGenerator->nameToKey(instData->m_decoratedNameString);
...
static Bool parseSystemCallback(const char *token, WinInstanceData *instData,
  ...
  DEBUG_ASSERTCRASH(TheNameKeyGenerator && TheFunctionLexicon,
                    ("Invalid singletons"));
  theSystemString = c;
...
static Bool parseTooltipText(const char *token, WinInstanceData *instData,
  ...
  if (strlen(c) >= MAX_TEXT_LABEL) {
    ...
    DEBUG_LOG(
        ("TextTooltip label '%s' is too long, max is '%d'", c, MAX_TEXT_LABEL));
    assert(0);
    return FALSE;
  }
...
static Bool parseStaticTextData(const char *token, WinInstanceData *instData,
  ...
  // These are initialized here because any TextData constructor would never get
  // called. The behavior with these defaults is the same as it was before these
  // members were added.
...
static Bool parseDrawData(const char *token, WinInstanceData *instData,
  ...
  for (i = 0; i < MAX_DRAW_DATA; i++) {
    ...
    if (strcmp(c, "NoImage") != 0)
      drawData->image =
          TheMappedImageCollection->findImageByName(AsciiString(c));
    else
...
// <Max number of entries> <Height of an entry> <AutoScroll> <AutoPurge>
// <ScrollBar> <MultiSelect> <ForceSelect>
//
...
// <Max text length> <entry box width> <SecretText> <1 = NumericalOnly, 2 =
// AlphaNumericOnly>
//
...
//=============================================================================
static GameWindow *createGadget(char *type, GameWindow *parent, Int status,
                                Int x, Int y, Int width, Int height,
                                WinInstanceData *instData, void *data) {
  ...
  if (strcmp(type, "PUSHBUTTON") == 0) {
    ...
    window = TheWindowManager->gogoGadgetPushButton(
        parent, status, x, y, width, height, instData, instData->m_font, FALSE);
    ...
    strlcpy(filename, instData->m_decoratedNameString.str(),
            ARRAY_SIZE(filename));
    c = strchr(filename, ':');
    if (c)
    ...
    window = TheWindowManager->gogoGadgetRadioButton(
        parent, status, x, y, width, height, instData, rData, instData->m_font,
        FALSE);

  } else if (strcmp(type, "CHECKBOX") == 0) {

    instData->m_style |= GWS_CHECK_BOX;
    window = TheWindowManager->gogoGadgetCheckbox(
        parent, status, x, y, width, height, instData, instData->m_font, FALSE);

  } else if (strcmp(type, "TABCONTROL") == 0) {
    TabControlData *tcData = (TabControlData *)data;
    instData->m_style |= GWS_TAB_CONTROL;
    window = TheWindowManager->gogoGadgetTabControl(parent, status, x, y, width,
                                                    height, instData, tcData,
                                                    instData->m_font, FALSE);
  } else if (strcmp(type, "VERTSLIDER") == 0) {
    SliderData *sData = (SliderData *)data;

    instData->m_style |= GWS_VERT_SLIDER;
    window = TheWindowManager->gogoGadgetSlider(parent, status, x, y, width,
                                                height, instData, sData,
                                                instData->m_font, FALSE);
    ...
    window = TheWindowManager->gogoGadgetListBox(parent, status, x, y, width,
                                                 height, instData, lData,
                                                 instData->m_font, FALSE);
    ...
    window = TheWindowManager->gogoGadgetComboBox(parent, status, x, y, width,
                                                  height, instData, cData,
                                                  instData->m_font, FALSE);
    ...
    window = TheWindowManager->gogoGadgetTextEntry(parent, status, x, y, width,
                                                   height, instData, eData,
                                                   instData->m_font, FALSE);
    ...
    window = TheWindowManager->gogoGadgetStaticText(parent, status, x, y, width,
                                                    height, instData, tData,
                                                    instData->m_font, FALSE);

  } else if (strcmp(type, "PROGRESSBAR") == 0) {

    instData->m_style |= GWS_PROGRESS_BAR;
    window = TheWindowManager->gogoGadgetProgressBar(
        parent, status, x, y, width, height, instData, instData->m_font, FALSE);
  }
...
//=============================================================================
static GameWindow *
createWindow(char *type, Int id, Int status, Int x, Int y, Int width,
             Int height, WinInstanceData *instData, void *data,
             GameWinSystemFunc system, GameWinInputFunc input,
             GameWinTooltipFunc tooltip, GameWinDrawFunc draw) {
  ...
  if (strcmp(type, "USER") == 0) {
    ...
    window = TheWindowManager->winCreate(parent, status, x, y, width, height,
                                         system);
    if (window) {
    ...
    window =
        createGadget(type, parent, status, x, y, width, height, instData, data);
    if (window) {
  ...
  // assign the callbacks if they are not empty/nullptr, that means they were
  // read in and parsed from the window definition file
  if (window) {
...
//=============================================================================
static Bool parseChildWindows(GameWindow *window, File *inFile, char *buffer) {
  GameWindow *lastWindow;
  AsciiString asciibuf;

  // The gadget with children needs to delete its default created children in
  // favor of the ones from the script file.  So kill them before reading.
  if (BitIsSet(window->winGetStyle(), GWS_TAB_CONTROL)) {
    GameWindow *nextWindow = nullptr;
    for (GameWindow *myChild = window->winGetChild(); myChild;
         myChild = nextWindow) {
  ...
  if (lastWindow != window) {
    ...
    DEBUG_LOG(("parseChildWindows: unmatched window on stack.  Corrupt stack "
               "or bad source"));
    return FALSE;
  }
  ...
  if (BitIsSet(window->winGetStyle(), GWS_TAB_CONTROL))
    GadgetTabControlFixupSubPaneList(
        window); // all children created, so re-fill SubPane array with children
  ...
  while (TRUE) {
    ...
    if (parse->parse == nullptr) {
      ...
      if (asciibuf.compare("DATA") == 0) {
        ...
        // Check to see if we have a header template, if so, set the font equal
        // to that.
        if (TheHeaderTemplateManager->getFontFromTemplate(
                instData.m_headerTemplateName))
          instData.m_font = TheHeaderTemplateManager->getFontFromTemplate(
              instData.m_headerTemplateName);

        // Create a window using the current description
        if (window == nullptr)
          window = createWindow(type, instData.m_id, instData.getStatus(), x, y,
                                width, height, &instData, data, systemFunc,
                                inputFunc, tooltipFunc, drawFunc);
...
//-------------------------------------------------------------------------------------------------
Bool parseInit(const char *token, char *buffer, UnsignedInt version,
               WindowLayoutInfo *info) {
  char *c;
  const char *seps = " \n\r\t";
  ...
  info->init = TheFunctionLexicon->winLayoutInitFunc(
      TheNameKeyGenerator->nameToKey(info->initNameString));
...
//-------------------------------------------------------------------------------------------------
Bool parseUpdate(const char *token, char *buffer, UnsignedInt version,
                 WindowLayoutInfo *info) {
  char *c;
  const char *seps = " \n\r\t";
  ...
  info->update = TheFunctionLexicon->winLayoutUpdateFunc(
      TheNameKeyGenerator->nameToKey(info->updateNameString));
...
//-------------------------------------------------------------------------------------------------
Bool parseShutdown(const char *token, char *buffer, UnsignedInt version,
                   WindowLayoutInfo *info) {
  char *c;
  const char *seps = " \n\r\t";
  ...
  info->shutdown = TheFunctionLexicon->winLayoutShutdownFunc(
      TheNameKeyGenerator->nameToKey(info->shutdownNameString));
...
//-------------------------------------------------------------------------------------------------
Bool parseLayoutBlock(File *inFile, char *buffer, UnsignedInt version,
                      WindowLayoutInfo *info) {
...
WindowLayoutInfo::WindowLayoutInfo()
    : version(0), init(nullptr), update(nullptr), shutdown(nullptr),
...
GameWindow *GameWindowManager::winCreateFromScript(AsciiString filenameString,
  ...
  // zero info struct
  // memset( &scriptInfo, 0, sizeof( WindowLayoutInfo ) ); // it's a class - use
  // a constructor
