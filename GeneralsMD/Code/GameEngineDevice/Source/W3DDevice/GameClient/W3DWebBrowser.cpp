...
Bool W3DWebBrowser::createBrowserWindow(const char *tag, GameWindow *win) {
  ...
  if (url == nullptr) {
    DEBUG_LOG(
        ("W3DWebBrowser::createBrowserWindow - couldn't find URL for page %s",
         tag));
    return FALSE;
  }
  ...
  DX8WebBrowser::CreateBrowser(
      windowName.str(), url->m_url.str(), x, y, w, h, 0,
      BROWSEROPTION_SCROLLBARS | BROWSEROPTION_3DBORDER, (LPDISPATCH)this);
