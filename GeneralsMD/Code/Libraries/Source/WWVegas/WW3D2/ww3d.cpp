 ...
 *                    $Revision:: 98 $*
 *                                                                                             *
 * 07/01/02 KM Scalable shader library integration * 08/05/02 KM Texture class
 *redesign
 *---------------------------------------------------------------------------------------------*
 * Functions: * WW3D::Init -- Initialize the WW3D Library * WW3D::Shutdown --
 *shutdown the WW3D Library                                               *
 ...
 *   WW3D::Get_Render_Device_Desc -- returns description of the current render
 *device          * WW3D::Get_Render_Device_Count -- returns the number of
 *render devices available           * WW3D::Get_Render_Device_Name -- returns
 *the name of the n-th render device                *
 *	  WW3D::Get_Render_Target_Resolution -- get the resolution and bitdepth
 *of the current target* WW3D::Get_Device_Resolution -- get the current
 *resolution and bitdepth                    * WW3D::Begin_Render -- mark the
 *start of rendering for a new frame                         * WW3D::Render --
 *Render a 3D Scene using the given camera                                  *
 *   WW3D::Render -- Render a single render object * WW3D::End_Render -- Mark
 *the completion of a frame                                        * WW3D::Sync
 *-- Time synchronization * WW3D::Set_Ext_Swap_Interval -- Sets the swap
 *interval the device should aim sync for.     * WW3D::Get_Ext_Swap_Interval --
 *Queries the swap interval the device is aiming sync for.   *
 *   WW3D::Get_Polygon_Mode -- returns the current rendering mode *
 *   WW3D::Set_Collision_Box_Display_Mask -- control rendering of collision
 *boxes              * WW3D::Get_Collision_Box_Display_Mask -- returns the
 *current display mask for collision bo * WW3D::Normalize_Coordinates -- Convert
 *pixel coords to normalized screen coords 0..1      *
 *   WW3D::Update_Render_Device_Description -- updates the description of the
 *current render d * WW3D::Make_Screen_Shot -- saves a screenshot with the given
 *base filename                 * WW3D::Start_Movie_Capture -- begins dumping
 *frames to a movie                             * WW3D::Stop_Movie_Capture --
 *ends dumping frames to a movie                                *
 *   WW3D::Toggle_Movie_Capture -- toggles movie capture... *
 *   WW3D::Start_Single_Frame_Movie_Capture -- starts capturing a single frame
 *movie           * WW3D::Capture_Next_Movie_Frame -- tells ww3d to grab another
 *frame for the movie          * WW3D::Pause_Movie -- pauses/unpauses movie
 *capturing                                      * WW3D::Is_Movie_Paused --
 *returns whether the movie capture system is paused               *
 *   WW3D::Is_Recording_Next_Frame -- returns whether the next frame will be
 *dumped to a movie * WW3D::Is_Movie_Ready -- returns whether the movie capture
 *system is ready                 * WW3D::Update_Movie_Capture -- dumps the
 *current frame into the movie                      *
 *   WW3D::Get_Movie_Capture_Frame_Rate -- returns the framerate at which the
 *movie is being c * WW3D::Set_Texture_Reduction -- sets the (hacky) texture
 *reduction factor                  * WW3D::Get_Texture_Reduction -- gets the
 *(hacky) texture reduction factor                  * WW3D::Flush_Texture_Cache
 *-- dump all textures from the texture cache                     *
 *   WW3D::Allocate_Debug_Resources -- allocates the debug resources
 ** WW3D::Release_Debug_Resources -- releases the debug resources
 ** WW3D::Get_Last_Frame_Poly_Count -- returns the number of polys submitted in
 *the previous  * WW3D::Flush -- Process all pending rendering tasks *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
#define DEFAULT_DEBUG_SHADER_BITS                                              \
  (SHADE_CNST(ShaderClass::PASS_LEQUAL, ShaderClass::DEPTH_WRITE_ENABLE,       \
              ShaderClass::COLOR_WRITE_ENABLE, ShaderClass::SRCBLEND_ONE,      \
              ShaderClass::DSTBLEND_ZERO, ShaderClass::FOG_DISABLE,            \
              ShaderClass::GRADIENT_MODULATE,                                  \
              ShaderClass::SECONDARY_GRADIENT_DISABLE,                         \
              ShaderClass::TEXTURING_DISABLE, ShaderClass::ALPHATEST_DISABLE,  \
              ShaderClass::CULL_MODE_ENABLE, ShaderClass::DETAILCOLOR_DISABLE, \
...
#define LIGHTMAP_DEBUG_SHADER_BITS                                             \
  (SHADE_CNST(ShaderClass::PASS_LEQUAL, ShaderClass::DEPTH_WRITE_ENABLE,       \
              ...
              ShaderClass::SECONDARY_GRADIENT_DISABLE,                         \
              ShaderClass::TEXTURING_ENABLE, ShaderClass::ALPHATEST_DISABLE,   \
              ShaderClass::CULL_MODE_ENABLE, ShaderClass::DETAILCOLOR_DISABLE, \
              ShaderClass::DETAILALPHA_DISABLE))
...
float WW3D::LogicFrameTimeMs =
    1000.0f / WWSyncPerSecond; // initialized to something to avoid division by
                               // zero on first use
...
WW3D::NPatchesGapFillingModeEnum WW3D::NPatchesGapFillingMode =
...
WW3DErrorType WW3D::Shutdown() {
  ...
  */
  ...
  /*
  ** Free the DazzleRenderObject class stuff. Whatever it is. ST - 6/11/2001
  *8:20PM
  */
  if (!Lite) {
 ...
 * HISTORY: * 3/24/98    GTH : Created. *
 *=============================================================================================*/
WW3DErrorType WW3D::Set_Render_Device(const char *dev_name, int width,
                                      int height, int bits, int windowed,
                                      bool resize_window) {
  bool success = DX8Wrapper::Set_Render_Device(dev_name, width, height, bits,
...
WW3DErrorType WW3D::Set_Render_Device(int dev, int width, int height, int bits,
  ...
  bool success = DX8Wrapper::Set_Render_Device(dev, width, height, bits,
                                               windowed, resize_window,
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 3/28/2001  pds : Created. *
 *=============================================================================================*/
void *WW3D::Get_Window() { return _Hwnd; }
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 1/26/2001  gth : Created. *
 *=============================================================================================*/
bool WW3D::Is_Windowed() { return DX8Wrapper::Is_Windowed(); }

/***********************************************************************************************
 * WW3D::Toggle_Windowed -- Toggle the current render device between
 *fullscreen and windowed	  * mode.  Note:  Its called '_Windowed' to be
 *consistent with the	  * other references inside WW3D, a more descriptive
 *name would		  * be Toggle_Fullscreen.
 **
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 3/24/98    GTH : Created. * 1/25/2001  gth : converted to dx8 *
 *=============================================================================================*/
int WW3D::Get_Render_Device() { return DX8Wrapper::Get_Render_Device(); }
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 3/26/98    GTH : Created. * 1/25/2001  gth : converted to dx8 *
 *=============================================================================================*/
...
/***********************************************************************************************
 * WW3D::Get_Render_Device_Count -- returns the number of render devices
 *available             *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 5/19/99    GTH : Created. * 1/25/2001  gth : converted to DX8 *
 *=============================================================================================*/
 ...
 * HISTORY: * 3/24/98    GTH : Created. *
 *=============================================================================================*/
WW3DErrorType WW3D::Set_Device_Resolution(int width, int height, int bits,
                                          int windowed, bool resize_window) {
  bool success = DX8Wrapper::Set_Device_Resolution(width, height, bits,
                                                   windowed, resize_window);
...
/***********************************************************************************************
 * WW3D::Get_Render_Target_Resolution -- get the resolution and bitdepth of the
 *current target *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 3/24/98    GTH : Created. * 1/25/2001  gth : converted to dx8 *
 *=============================================================================================*/
void WW3D::Get_Render_Target_Resolution(int &set_w, int &set_h, int &set_bits,
                                        bool &set_windowed) {
  DX8Wrapper::Get_Render_Target_Resolution(set_w, set_h, set_bits,
                                           set_windowed);
}
 ...
 * HISTORY: * 3/24/98    GTH : Created. * 1/25/2001  gth : converted to dx8 *
 *=============================================================================================*/
void WW3D::Get_Device_Resolution(int &set_w, int &set_h, int &set_bits,
                                 bool &set_windowed) {
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 12/3/98    BMG : Created. * 1/25/2001  gth : converted to dx8 *
 *=============================================================================================*/
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 12/3/98    BMG : Created. *
 *=============================================================================================*/
WW3DErrorType WW3D::Registry_Save_Render_Device(const char *sub_key, int device,
                                                int width, int height,
                                                int depth, bool windowed,
                                                int texture_depth) {
  bool success = DX8Wrapper::Registry_Save_Render_Device(
      sub_key, device, width, height, depth, windowed, texture_depth);
  if (success) {
    return WW3D_ERROR_OK;
  } else {
    return WW3D_ERROR_INITIALIZATION_FAILED;
...
WW3DErrorType WW3D::Registry_Load_Render_Device(const char *sub_key,
                                                bool resize_window) {
  bool success =
      DX8Wrapper::Registry_Load_Render_Device(sub_key, resize_window);
  if (success) {
    return WW3D_ERROR_OK;
  } else {
    return WW3D_ERROR_INITIALIZATION_FAILED;
...
bool WW3D::Registry_Load_Render_Device(const char *sub_key, char *device,
                                       int device_len, int &width, int &height,
                                       int &depth, int &windowed,
                                       int &texture_depth) {
...
void WW3D::_Invalidate_Mesh_Cache() { TheDX8MeshRenderer.Invalidate(); }
...
void WW3D::Set_Texture_Filter(int texture_filter) {
  if (texture_filter < 0)
    texture_filter = 0;
  if (texture_filter > TextureFilterClass::TEXTURE_FILTER_ANISOTROPIC)
    texture_filter = TextureFilterClass::TEXTURE_FILTER_ANISOTROPIC;
...
WW3DErrorType WW3D::Begin_Render(bool clear, bool clearz, const Vector3 &color,
                                 float dest_alpha, void (*network_callback)()) {
  if (!IsInitted) {
    return (WW3D_ERROR_OK);
  }
  ...
  // If we want to clear the screen, we need to set the viewport to include the
  // entire screen:
  if (clear || clearz) {
    D3DVIEWPORT8 vp;
...
WW3DErrorType WW3D::Render(const LayerClass &Layer) {
  ...
  WWASSERT(IsRendering);
  return Render(Layer.Scene, Layer.Camera, Layer.Clear, Layer.ClearZ,
                Layer.ClearColor);
}
...
WW3DErrorType WW3D::Render(SceneClass *scene, CameraClass *cam, bool clear,
  ...
  // Set the global ambient light value here.  If the scene is using the
  // LightEnvironment system this setting will get overridden.
 ...
 *                                                                                             *
 *    NOTE: This normally happens AUTOMATICALLY. The user should almost *NEVER*
 *have to call   * this function.  Anyway, this function causes all of the
 *deferred rendering systems to    * actually perform all of their rendering
 *tasks.  This includes the DX8MeshRenderer and    * the sorting system. *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 * OUTPUT: *
 *                                                                                             *
 * WARNINGS: * Don't call this unless you know what you're doing *
 *                                                                                             *
 * HISTORY: * 4/17/2001  gth : Created. * 07/01/02 KM Scalable shader library
 *integration				                               *
 *=============================================================================================*/
...
WW3DErrorType WW3D::End_Render(bool flip_frame) {
  ...
  // If sorting renderer flush isn't called from within any of the render
  // functions the sorting arrays will overflow!
  ...
  // (gth) I've found some cases where its not safe to rely on our "shadow" copy
  // (of matrices for example) across multiple frames.  So even though this is
  // slightly less "optimal", lets just reset the caches each frame.
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 6/20/01    DEL : Created. *
 *=============================================================================================*/
void WW3D::Flip_To_Primary() { DX8Wrapper::Flip_To_Primary(); }
...
/***********************************************************************************************
 * WW3D::Set_Ext_Swap_Interval -- Sets the swap interval the device should aim
 *sync for.       *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * WW3D::Get_Ext_Swap_Interval -- Queries the swap interval the device is aiming
 *sync for.     *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 5/07/98    NH : Created. *
 *=============================================================================================*/
long WW3D::Get_Ext_Swap_Interval() { return DX8Wrapper::Get_Swap_Interval(); }
...
/***********************************************************************************************
 * WW3D::Get_Collision_Box_Display_Mask -- returns the current display mask for
 *collision box  *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * WW3D::Normalize_Coordinates -- Convert pixel coords to normalized screen
 *coords 0..1        *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
...
void WW3D::Normalize_Coordinates(int x, int y, float &fx, float &fy) {
  ...
  // now that the coordinates are clipped convert them to their normalized
  // values.
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 5/19/99    GTH : Created. * 2/26/2001  hy : Updated to DX8 *
 *=============================================================================================*/
void WW3D::Make_Screen_Shot(const char *filename_base, const float gamma,
                            const ScreenShotFormatEnum format) {
  ...
  // TheSuperHackers @bugfix xezon 21/05/2025 Get the back buffer and create a
  // copy of the surface. Originally this code took the front buffer and tried
  // to lock it. This does not work when the render view clips outside the
  // desktop boundaries. It crashed the game.
  ...
  SurfaceClass *surfaceCopy =
      NEW_REF(SurfaceClass,
              (DX8Wrapper::_Create_DX8_Surface(
                  surfaceDesc.Width, surfaceDesc.Height, surfaceDesc.Format)));
  DX8Wrapper::_Copy_DX8_Rects(surface->Peek_D3D_Surface(), nullptr, 0,
                              surfaceCopy->Peek_D3D_Surface(), nullptr);
  ...
  for (y = 0; y < height; y++) {
    for (x = 0; x < width; x++) {
      ...
      image[index + 1] =
          gamma_lut[*((unsigned char *)lrect.pBits + index2 + 1)];
      image[index + 2] =
          gamma_lut[*((unsigned char *)lrect.pBits + index2 + 0)];
    }
  }
...
#ifndef __EMSCRIPTEN__
  {
    ...
    if (file) {
      ...
      for (y = 0; y < (int)height; y++) {
        memcpy(&temp[0], &image[3 * width * (height - y - 1)],
               3 * width * sizeof(char));
        for (x = 0; x < width; x++) {
          char t2 = temp[3 * x];
          temp[3 * x] = temp[3 * x + 2];
...
/***********************************************************************************************
 * WW3D::Capture_Next_Movie_Frame -- tells ww3d to grab another frame for the
 *movie            *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 5/19/99    GTH : Created. *
 *=============================================================================================*/
bool WW3D::Is_Movie_Ready() { return Movie != nullptr; }
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 11/25/99    TSS : Created. *
 *=============================================================================================*/
int WW3D::Get_Texture_Reduction() { return _TextureReduction; }

/***********************************************************************************************
 * WW3D::Get_Texture_Min_Mip_Levels -- gets the minimum number of mip levels
 *permitted		   *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * WW3D::Peek_Default_Debug_Material -- returns a pointer to the default debug
 *mtl				  *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 * HISTORY: * 7/21/99    GTH : Created. *
 *=============================================================================================*/
ShaderClass WW3D::Peek_Default_Debug_Shader() { return DefaultDebugShader; }
 ...
 * HISTORY: * 7/21/99    GTH : Created. *
 *=============================================================================================*/
ShaderClass WW3D::Peek_Lightmap_Debug_Shader() { return LightmapDebugShader; }
...
WW3DErrorType WW3D::On_Activate_App() { return WW3D_ERROR_OK; }
...
int WW3D::Get_Texture_Bitdepth() { return DX8Wrapper::Get_Texture_Bitdepth(); }
...
void WW3D::Render_And_Clear_Static_Sort_Lists(RenderInfoClass &rinfo) {
  // The ststic sort lists need to be disabled while we are rendering from them
  // otherwise the Render() function will just dump the objects right back on
  // the same lists.
...
void WW3D::Set_Gamma(float gamma, float bright, float contrast,
                     bool calibrate) {
