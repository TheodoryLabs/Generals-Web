 ...
 *                    $Revision:: 170 $*
 *                                                                                             *
 * 06/26/02 KM Matrix name change to avoid MAX conflicts * 06/27/02 KM Render to
 *shadow buffer texture support
 ** 06/27/02 KM Shader system updates
 ** 08/05/02 KM Texture class redesign
 *---------------------------------------------------------------------------------------------*
 * Functions: * DX8Wrapper::_Update_Texture -- Copies a texture from system
...
void Log_DX8_ErrorCode(unsigned res) {
  ...
  HRESULT new_res = D3DXGetErrorStringA(res, tmp, sizeof(tmp));
...
void Log_DX8_ErrorCode(unsigned res, const char *file, int line) {
  ...
  HRESULT new_res = D3DXGetErrorStringA(res, tmp, sizeof(tmp));
  if (new_res == D3D_OK) {
    WWDEBUG_SAY(("DX8 Error: %s Code %u at %s:%d", tmp, res, file, line));
  }
  WWASSERT(0);
}
...
// Helper function that moves x and y such that the inner rect fits into the
// outer rect. If the inner rect already is in the outer rect, then this does
// nothing. If the inner rect is larger than the outer rect, then the inner rect
// will be aligned to the top left of the outer rect.
void MoveRectIntoOtherRect(const RECT &inner, const RECT &outer, int *x,
                           int *y) {
  int dx = 0;
...
extern "C" IDirect3D8 *Direct3DCreate8(UINT SDKVersion) {
    static Emscripten_IDirect3D8 s_d3d8;
    return &s_d3d8;
}
#endif

bool DX8Wrapper::Init(void *hwnd, bool lite) {
  fprintf(stderr, "DEBUG_CRAZY: DX8Wrapper::Init called with lite=%d\n", lite);
  WWASSERT(!IsInitted);
  ...
  memset(Vertex_Shader_Constants, 0,
         sizeof(Vector4) * MAX_VERTEX_SHADER_CONSTANTS);
  memset(Pixel_Shader_Constants, 0,
         sizeof(Vector4) * MAX_PIXEL_SHADER_CONSTANTS);
  ...
  */
  ...
  if (!lite) {
    ...
    StringClass device_name("Emscripten GLES3 Device", true);
    ...
    // Enumerate devices (even if dummy, to fill tables)
    Enumerate_Devices();
    WWDEBUG_SAY(("DX8Wrapper Init completed (Emscripten)"));
    return true;
#else
    fprintf(stderr, "DEBUG_CRAZY: About to call LoadLibrary D3D8.DLL\n");
    D3D8Lib = LoadLibrary("D3D8.DLL");

    if (D3D8Lib == nullptr) {
      fprintf(stderr, "DEBUG_CRAZY: LoadLibrary returned nullptr, returning false\n");
      return false; // Return false at this point if init failed
    }

    Direct3DCreate8Ptr =
        (Direct3DCreate8Type)GetProcAddress(D3D8Lib, "Direct3DCreate8");
    ...
    */
    ...
    {
      // TheSuperHackers @bugfix xezon 13/06/2025 Front load the system
      // dbghelp.dll to prevent the graphics driver from potentially loading the
      // old game dbghelp.dll and then crashing the game process.
      DbgHelpGuard dbgHelpGuard;

      D3DInterface =
          Direct3DCreate8Ptr(D3D_SDK_VERSION); // TODO: handle failure cases...
    }
    if (D3DInterface == nullptr) {
      return (false);
    }
...
void DX8Wrapper::Shutdown() {
  ...
  _RenderDeviceNameTable
      .Clear(); // note - Delete_All() resizes the vector, causing a
                // reallocation.  Clear is better. jba.
...
void DX8Wrapper::Do_Onetime_Device_Dependent_Inits() {
  ...
  */
  fprintf(stderr, "DEBUG_CRAZY: Do_Onetime: Compute_Caps\n");
  Compute_Caps(D3DFormat_To_WW3DFormat(DisplayFormat));

  /*
   ** Initialize any other subsystems inside of WW3D
   */
  fprintf(stderr, "DEBUG_CRAZY: Do_Onetime: MissingTexture::_Init\n");
  MissingTexture::_Init();
  fprintf(stderr, "DEBUG_CRAZY: Do_Onetime: TextureFilterClass::_Init_Filters\n");
  TextureFilterClass::_Init_Filters(
      (TextureFilterClass::TextureFilterMode)WW3D::Get_Texture_Filter());
  fprintf(stderr, "DEBUG_CRAZY: Do_Onetime: TheDX8MeshRenderer.Init\n");
  TheDX8MeshRenderer.Init();
  fprintf(stderr, "DEBUG_CRAZY: Do_Onetime: SHD_INIT\n");
  SHD_INIT;
  fprintf(stderr, "DEBUG_CRAZY: Do_Onetime: BoxRenderObjClass::Init\n");
  BoxRenderObjClass::Init();
  fprintf(stderr, "DEBUG_CRAZY: Do_Onetime: VertexMaterialClass::Init\n");
  VertexMaterialClass::Init();
  fprintf(stderr, "DEBUG_CRAZY: Do_Onetime: PointGroupClass::_Init\n");
  PointGroupClass::_Init();
  fprintf(stderr, "DEBUG_CRAZY: Do_Onetime: ShatterSystem::Init\n");
  ShatterSystem::Init();
  fprintf(stderr, "DEBUG_CRAZY: Do_Onetime: TextureLoader::Init\n");
  TextureLoader::Init();
  fprintf(stderr, "DEBUG_CRAZY: Do_Onetime: Set_Default_Global_Render_States\n");
  Set_Default_Global_Render_States();
  fprintf(stderr, "DEBUG_CRAZY: Do_Onetime: ALL DONE\n");
}
...
void DX8Wrapper::Set_Default_Global_Render_States() {
  ...
  Set_DX8_Render_State(D3DRS_RANGEFOGENABLE,
                       (caps.RasterCaps & D3DPRASTERCAPS_FOGRANGE) ? TRUE
                                                                   : FALSE);
...
bool DX8Wrapper::Create_Device() {
  WWASSERT(
      D3DDevice ==
      nullptr); // for now, once you've created a device, you're stuck with it!

  D3DCAPS8 caps;
  if (FAILED(
          D3DInterface->GetDeviceCaps(CurRenderDevice, WW3D_DEVTYPE, &caps))) {
    return false;
  }

  ::ZeroMemory(&CurrentAdapterIdentifier, sizeof(D3DADAPTER_IDENTIFIER8));

  if (FAILED(D3DInterface->GetAdapterIdentifier(
          CurRenderDevice, D3DENUM_NO_WHQL_LEVEL, &CurrentAdapterIdentifier))) {
    return false;
  }
  ...
  // to prevent the graphics driver from potentially loading the old game
  // dbghelp.dll and then crashing the game process.
  DbgHelpGuard dbgHelpGuard;

  HRESULT hr = D3DInterface->CreateDevice(CurRenderDevice, WW3D_DEVTYPE, _Hwnd,
                                          Vertex_Processing_Behavior,
                                          &_PresentParameters, &D3DDevice);

  if (FAILED(hr)) {
    // The device selection may fail because the device lied that it supports 32
    // bit zbuffer with 16 bit display. This happens at least on Voodoo2.
    ...
    if ((_PresentParameters.BackBufferFormat == D3DFMT_R5G6B5 ||
      ...
      hr = D3DInterface->CreateDevice(CurRenderDevice, WW3D_DEVTYPE, _Hwnd,
                                      Vertex_Processing_Behavior,
                                      &_PresentParameters, &D3DDevice);
...
void DX8Wrapper::Release_Device() {
  if (D3DDevice) {
    ...
    for (int a = 0; a < MAX_TEXTURE_STAGES;
         ++a) { // release references to any textures that were used in last
                // rendering call
    ...
    DX8CALL(SetStreamSource(
        0, nullptr,
        0)); // release reference count on last rendered vertex buffer
    DX8CALL(SetIndices(
        nullptr, 0)); // release reference count on last rendered index buffer
...
void DX8Wrapper::Enumerate_Devices() {
  ...
  for (int adapter_index = 0; adapter_index < adapter_count; adapter_index++) {
    ...
    HRESULT res = D3DInterface->GetAdapterIdentifier(
        adapter_index, D3DENUM_NO_WHQL_LEVEL, &id);

    if (res == D3D_OK) {

      /*
      ** Set up the render device description
      ** TODO: Fill in more fields of the render device description?  (need some
      *lookup tables)
      */
      RenderDeviceDescClass desc;
      ...
      */
      ...
      for (int mode_index = 0; mode_index < mode_count; mode_index++) {
        ...
        if (res == D3D_OK) {
          ...
          ** TODO: should we handle more formats?  will any cards report more
          *than 24 or 16 bit?
          */
          if (bits != 0) {
      ...
      // IML: If the device has one or more valid resolutions add it to the
      // device list. NOTE: Testing has shown that there are drivers with zero
      // resolutions.
      if (desc.Enumerate_Resolutions().Count() > 0) {
        ...
        */
        ...
        _RenderDeviceNameTable.Add(device_name);
        _RenderDeviceShortNameTable.Add(
            device_name); // for now, just add the same name to the "pretty name
                          // table"
...
bool DX8Wrapper::Set_Any_Render_Device() {
  ...
  // Try windowed first
  for (dev_number = 0; dev_number < _RenderDeviceNameTable.Count();
       dev_number++) {
...
bool DX8Wrapper::Set_Render_Device(const char *dev_name, int width, int height,
                                   int bits, int windowed, bool resize_window) {
  for (int dev_number = 0; dev_number < _RenderDeviceNameTable.Count();
       dev_number++) {
    if (strcmp(dev_name, _RenderDeviceNameTable[dev_number]) == 0) {
      return Set_Render_Device(dev_number, width, height, bits, windowed,
                               resize_window);
    }
...
void DX8Wrapper::Get_Format_Name(unsigned int format, StringClass *tex_format) {
  *tex_format = "Unknown";
  ...
  case D3DFMT_A1R5G5B5:
    *tex_format = "D3DFMT_A1R5G5B5";
    break;
  case D3DFMT_R5G6B5:
    *tex_format = "D3DFMT_R5G6B5";
    break;
  case D3DFMT_L8:
    *tex_format = "D3DFMT_L8";
    break;
  case D3DFMT_A8:
    *tex_format = "D3DFMT_A8";
    break;
  case D3DFMT_P8:
    *tex_format = "D3DFMT_P8";
    break;
  case D3DFMT_X8R8G8B8:
  ...
  case D3DFMT_X1R5G5B5:
    *tex_format = "D3DFMT_X1R5G5B5";
    break;
  case D3DFMT_R3G3B2:
    *tex_format = "D3DFMT_R3G3B2";
    break;
  case D3DFMT_A8R3G3B2:
    *tex_format = "D3DFMT_A8R3G3B2";
    break;
  case D3DFMT_X4R4G4B4:
    *tex_format = "D3DFMT_X4R4G4B4";
    break;
  case D3DFMT_A8P8:
    *tex_format = "D3DFMT_A8P8";
    break;
  case D3DFMT_A8L8:
    *tex_format = "D3DFMT_A8L8";
    break;
  case D3DFMT_A4L4:
    *tex_format = "D3DFMT_A4L4";
    break;
  case D3DFMT_V8U8:
    *tex_format = "D3DFMT_V8U8";
    break;
  case D3DFMT_L6V5U5:
    *tex_format = "D3DFMT_L6V5U5";
    break;
  case D3DFMT_X8L8V8U8:
    *tex_format = "D3DFMT_X8L8V8U8";
    break;
  case D3DFMT_Q8W8V8U8:
    *tex_format = "D3DFMT_Q8W8V8U8";
    break;
  case D3DFMT_V16U16:
    *tex_format = "D3DFMT_V16U16";
    break;
  case D3DFMT_W11V11U10:
    *tex_format = "D3DFMT_W11V11U10";
    break;
  case D3DFMT_UYVY:
    *tex_format = "D3DFMT_UYVY";
    break;
  case D3DFMT_YUY2:
    *tex_format = "D3DFMT_YUY2";
    break;
  case D3DFMT_DXT1:
    *tex_format = "D3DFMT_DXT1";
    break;
  case D3DFMT_DXT2:
    *tex_format = "D3DFMT_DXT2";
    break;
  case D3DFMT_DXT3:
    *tex_format = "D3DFMT_DXT3";
    break;
  case D3DFMT_DXT4:
    *tex_format = "D3DFMT_DXT4";
    break;
  case D3DFMT_DXT5:
    *tex_format = "D3DFMT_DXT5";
    break;
  case D3DFMT_D16_LOCKABLE:
    *tex_format = "D3DFMT_D16_LOCKABLE";
    break;
  case D3DFMT_D32:
    *tex_format = "D3DFMT_D32";
    break;
  case D3DFMT_D15S1:
    *tex_format = "D3DFMT_D15S1";
    break;
  case D3DFMT_D24S8:
    *tex_format = "D3DFMT_D24S8";
    break;
  case D3DFMT_D16:
    *tex_format = "D3DFMT_D16";
    break;
  case D3DFMT_D24X8:
    *tex_format = "D3DFMT_D24X8";
    break;
  case D3DFMT_D24X4S4:
    *tex_format = "D3DFMT_D24X4S4";
    break;
  default:
    break;
  }
...
void DX8Wrapper::Resize_And_Position_Window() {
  ...
  if ((rect.right - rect.left) != ResolutionWidth ||
    ...
    if (!IsWindowed) {
      ...
      // TheSuperHackers @feature helmutbuhler 14/04/2025
      // Move the window to try fit it into the monitor area, if one of its
      // dimensions is larger than the work area. Otherwise align the window to
      // the top left edges, if it is even larger than the monitor area.
      RECT rectClient;
      ...
      DEBUG_LOG(("Window positioned to x:%d y:%d, resized to w:%d h:%d", left,
                 top, width, height));
    }
  }
}

bool DX8Wrapper::Set_Render_Device(int dev, int width, int height, int bits,
                                   int windowed, bool resize_window,
                                   bool reset_device, bool restore_assets) {
  WWASSERT(IsInitted);
  WWASSERT(dev >= -1);
  ...
  */
  ...
  WWDEBUG_SAY(
      ("Attempting Set_Render_Device: name: %s (%s:%s), width: %d, height: %d, "
       "windowed: %d",
       _RenderDeviceNameTable[CurRenderDevice].str(),
       _RenderDeviceDescriptionTable[CurRenderDevice].Get_Driver_Name(),
       _RenderDeviceDescriptionTable[CurRenderDevice].Get_Driver_Version(),
       ResolutionWidth, ResolutionHeight, (IsWindowed ? 1 : 0)));
  ...
  */
  ...
  // I changed this to discard all the time (even when full-screen) since that
  // the most efficient. 07-16-03 MW:
  _PresentParameters.SwapEffect =
      D3DSWAPEFFECT_DISCARD; // IsWindowed ? D3DSWAPEFFECT_DISCARD :
                             // D3DSWAPEFFECT_FLIP;		// Shouldn't
                             // this be D3DSWAPEFFECT_FLIP?
  _PresentParameters.hDeviceWindow = _Hwnd;
  _PresentParameters.Windowed = IsWindowed;

  _PresentParameters.EnableAutoDepthStencil =
      TRUE;                     // Driver will attempt to match Z-buffer depth
  ...
  */
  if (IsWindowed) {
    ...
    // In windowed mode, define the bitdepth from desktop mode (as it can't be
    // changed)
    ...
    if (BitDepth == 32 &&
        D3DInterface->CheckDeviceType(0, D3DDEVTYPE_HAL, desktop_mode.Format,
                                      D3DFMT_A8R8G8B8, TRUE) ==
            D3D_OK) { // promote 32-bit modes to include destination alpha
    ...
    */
    if (!Find_Z_Mode(DisplayFormat, _PresentParameters.BackBufferFormat,
                     &_PresentParameters.AutoDepthStencilFormat)) {
      // If opening 32 bit mode failed, try 16 bit, even if the desktop happens
      // to be 32 bit
      if (BitDepth == 32) {
        BitDepth = 16;
        _PresentParameters.BackBufferFormat = D3DFMT_R5G6B5;
        if (!Find_Z_Mode(_PresentParameters.BackBufferFormat,
  ...
  */
  ...
  WWDEBUG_SAY(("Using Display/BackBuffer Formats: %s/%s", displayFormat.str(),
               backbufferFormat.str()));
  ...
  if (reset_device) {
    ...
    ret =
        Reset_Device(restore_assets); // reset device without restoring data -
                                      // we're likely switching out of the app.
  } else
    ret = Create_Device();
...
bool DX8Wrapper::Toggle_Windowed() {
  ...
  if (IsInitted) {
    ...
    const DynamicVectorClass<ResolutionDescClass> &resolutions =
        render_device.Enumerate_Resolutions();
    ...
    int curr_res = -1;
    for (int res = 0; (res < resolutions.Count()) && (curr_res == -1); res++) {
    ...
    if (curr_res == -1) {
      ...
      return Set_Device_Resolution(resolutions[0].Width, resolutions[0].Height,
                                   resolutions[0].BitDepth, !IsWindowed, true);
    } else {
...
void DX8Wrapper::Get_Device_Resolution(int &set_w, int &set_h, int &set_bits,
                                       bool &set_windowed) {
  WWASSERT(IsInitted);
...
void DX8Wrapper::Get_Render_Target_Resolution(int &set_w, int &set_h,
                                              int &set_bits,
                                              bool &set_windowed) {
  WWASSERT(IsInitted);
  ...
  if (CurrentRenderTarget != nullptr) {
    ...
    set_windowed = IsWindowed; // this doesn't really make sense for render
                               // targets (shouldn't matter)...
...
bool DX8Wrapper::Registry_Save_Render_Device(const char *sub_key) {
  ...
  return Registry_Save_Render_Device(sub_key, CurRenderDevice, ResolutionWidth,
                                     ResolutionHeight, BitDepth, IsWindowed,
                                     TextureBitDepth);
}

bool DX8Wrapper::Registry_Save_Render_Device(const char *sub_key, int device,
...
bool DX8Wrapper::Registry_Load_Render_Device(const char *sub_key,
  ...
  if (Registry_Load_Render_Device(sub_key, name, sizeof(name), width, height,
                                  depth, windowed, TextureBitDepth) &&
      (*name != 0)) {
    WWDEBUG_SAY(("Device %s (%d X %d) %d bit windowed:%d", name, width, height,
                 depth, windowed));
    ...
    if (TextureBitDepth == 16 || TextureBitDepth == 32) {
      ...
      WWDEBUG_SAY(
          ("Invalid texture depth %d, switching to 16 bits", TextureBitDepth));
      TextureBitDepth = 16;
    }
    ...
    if (Set_Render_Device(name, width, height, depth, windowed,
      ...
      if (depth == 16)
        ...
        depth = 16;
      // we'll test resolutions down, so if start is 640, increase to begin
      // with...
      if (width == 640) {
        width = 1024;
        height = 768;
...
bool DX8Wrapper::Registry_Load_Render_Device(const char *sub_key, char *device,
                                             int device_len, int &width,
  ...
  if (registry.Is_Valid()) {
    registry.Get_String(VALUE_NAME_RENDER_DEVICE_NAME, device, device_len);
    ...
    texture_depth =
        registry.Get_Int(VALUE_NAME_RENDER_DEVICE_TEXTURE_DEPTH, -1);
    return true;
  }
  *device = 0;
  width = -1;
...
bool DX8Wrapper::Find_Color_And_Z_Mode(int resx, int resy, int bitdepth,
                                       D3DFORMAT *set_colorbuffer,
                                       D3DFORMAT *set_backbuffer,
                                       D3DFORMAT *set_zmode) {
#ifdef __EMSCRIPTEN__
  *set_colorbuffer = D3DFMT_A8R8G8B8;
  *set_backbuffer = D3DFMT_A8R8G8B8;
  *set_zmode = D3DFMT_D24S8;
  return true;
#endif

  static D3DFORMAT _formats16[] = {D3DFMT_R5G6B5, D3DFMT_X1R5G5B5,
                                   D3DFMT_A1R5G5B5};
  ...
  /*
  ** Select the table that we're going to use to search for a valid backbuffer
  *format
  */
  ...
  */
  ...
  if (bitdepth == 32 && *set_colorbuffer == D3DFMT_X8R8G8B8 &&
      D3DInterface->CheckDeviceType(0, D3DDEVTYPE_HAL, *set_colorbuffer,
                                    D3DFMT_A8R8G8B8, TRUE) ==
          D3D_OK) { // promote 32-bit modes to include destination alpha when
                    // supported
    *set_backbuffer = D3DFMT_A8R8G8B8;
  }
...
// refresh rate
bool DX8Wrapper::Find_Color_Mode(D3DFORMAT colorbuffer, int resx, int resy,
                                 UINT *mode) {
  UINT i, j, modemax;
  UINT rx, ry;
  ...
  while (i < modemax && !found) {
    ...
    if (dmode.Width == rx && dmode.Height == ry &&
        dmode.Format == colorbuffer) {
      WWDEBUG_SAY(
...
bool DX8Wrapper::Find_Z_Mode(D3DFORMAT colorbuffer, D3DFORMAT backbuffer,
                             D3DFORMAT *zmode) {
...
bool DX8Wrapper::Test_Z_Mode(D3DFORMAT colorbuffer, D3DFORMAT backbuffer,
                             D3DFORMAT zmode) {
  // See if we have this mode first
  if (FAILED(D3DInterface->CheckDeviceFormat(D3DADAPTER_DEFAULT, WW3D_DEVTYPE,
                                             colorbuffer, D3DUSAGE_DEPTHSTENCIL,
                                             D3DRTYPE_SURFACE, zmode))) {
    WWDEBUG_SAY(("CheckDeviceFormat failed.  Colorbuffer format = %d  "
                 "Zbufferformat = %d",
                 colorbuffer, zmode));
    return false;
  }
  ...
  if (FAILED(D3DInterface->CheckDepthStencilMatch(
          D3DADAPTER_DEFAULT, WW3D_DEVTYPE, colorbuffer, backbuffer, zmode))) {
    WWDEBUG_SAY(("CheckDepthStencilMatch failed.  Colorbuffer format = %d  "
                 "Backbuffer format = %d Zbufferformat = %d",
...
void DX8Wrapper::End_Scene(bool flip_frames) {
  ...
  if (flip_frames) {
    ...
    // If the device was lost we need to check for cooperative level and
    // possibly reset the device
    if (hr == D3DERR_DEVICELOST) {
...
void DX8Wrapper::Clear(bool clear_color, bool clear_z_stencil,
  ...
  // If we try to clear a stencil buffer which is not there, the entire call
  // will fail KJM fixed this to get format from back buffer (incase render to
  // texture is used)
  /*bool has_stencil = (	_PresentParameters.AutoDepthStencilFormat ==
     D3DFMT_D15S1 || _PresentParameters.AutoDepthStencilFormat == D3DFMT_D24S8
     || _PresentParameters.AutoDepthStencilFormat == D3DFMT_D24X4S4);*/
  ...
  if (depthbuffer) {
    ...
    has_stencil = (desc.Format == D3DFMT_D15S1 || desc.Format == D3DFMT_D24S8 ||
  ...
  if (flags) {
    DX8CALL(
        Clear(0, nullptr, flags, Convert_Color(color, dest_alpha), z, stencil));
  }
}
...
void DX8Wrapper::Set_Vertex_Buffer(const VertexBufferClass *vb,
  ...
  REF_PTR_SET(render_state.vertex_buffers[stream],
...
void DX8Wrapper::Set_Vertex_Buffer(const DynamicVBAccessClass &vba_) {
  ...
  if (render_state.vertex_buffers[0])
    render_state.vertex_buffers[0]->Release_Engine_Ref();
  ...
  render_state_changed |=
      INDEX_BUFFER_CHANGED; // vba_offset changes so index buffer needs to be
                            // reset as well.
}
...
void DX8Wrapper::Set_Index_Buffer(const DynamicIBAccessClass &iba_,
                                  unsigned short index_base_offset) {
  if (render_state.index_buffer)
    render_state.index_buffer->Release_Engine_Ref();
...
void DX8Wrapper::Draw_Sorting_IB_VB(unsigned primitive_type,
  ...
  WWASSERT(render_state.vertex_buffer_types[0] == BUFFER_TYPE_SORTING ||
  ...
  DynamicVBAccessClass dyn_vb_access(BUFFER_TYPE_DYNAMIC_DX8, dynamic_fvf_type,
                                     vertex_count);
  {
    ...
    src += render_state.vba_offset + render_state.index_base_offset +
           min_vertex_index;
    unsigned size = dyn_vb_access.FVF_Info().Get_FVF_Size() * vertex_count /
                    sizeof(unsigned);
  ...
  // If using FVF format VB, set the FVF as vertex shader (may not be needed
  // here KM)
  ...
  default:
    WWASSERT(0);
    break; // Unsupported primitive type
  }
  ...
  DynamicIBAccessClass dyn_ib_access(BUFFER_TYPE_DYNAMIC_DX8, index_count);
  {
    ...
    src = static_cast<SortingIndexBufferClass *>(render_state.index_buffer)
              ->index_buffer;
  ...
  DX8CALL(
      SetIndices(static_cast<DX8IndexBufferClass *>(dyn_ib_access.IndexBuffer)
                     ->Get_DX8_Index_Buffer(),
                 dyn_vb_access.VertexBufferOffset));
  DX8_RECORD_INDEX_BUFFER_CHANGE();

  DX8_RECORD_DRAW_CALLS();
  DX8CALL(DrawIndexedPrimitive(D3DPT_TRIANGLELIST,
                               0, // start vertex
                               vertex_count, dyn_ib_access.IndexBufferOffset,
                               polygon_count));
...
void DX8Wrapper::Draw(unsigned primitive_type, unsigned short start_index,
  ...
  SNAPSHOT_SAY(
      ("DX8 - draw %d polygons (%d vertices)", polygon_count, vertex_count));
  ...
  case BUFFER_TYPE_DYNAMIC_DX8:
    ...
    case BUFFER_TYPE_DYNAMIC_DX8: {
      /*				if
         ((start_index+render_state.iba_offset+polygon_count*3) >
         render_state.index_buffer->Get_Index_Count()) {
         WWASSERT_PRINT(0,"OVERFLOWING INDEX BUFFER");
                                              ///@todo: MUST FIND OUT WHY THIS
         HAPPENS WITH LOTS OF PARTICLES ON BIG FIGHT!  -MW break;
                                      }*/
      DX8_RECORD_RENDER(polygon_count, vertex_count, render_state.shader);
      DX8_RECORD_DRAW_CALLS();
      DX8CALL(DrawIndexedPrimitive(
          (D3DPRIMITIVETYPE)primitive_type, min_vertex_index, vertex_count,
  ...
  case BUFFER_TYPE_DYNAMIC_SORTING:
    ...
    case BUFFER_TYPE_SORTING:
    case BUFFER_TYPE_DYNAMIC_SORTING:
      Draw_Sorting_IB_VB(primitive_type, start_index, polygon_count,
...
void DX8Wrapper::Draw_Strip(unsigned short start_index,
                            ...
                            unsigned short min_vertex_index,
                            unsigned short vertex_count) {
  Draw(D3DPT_TRIANGLESTRIP, start_index, polygon_count, min_vertex_index,
       vertex_count);
}
      ...
      if (render_state_changed & mask) {
        ...
        if (render_state.LightEnable[index]) {
          ...
          if (WW3D::Is_Snapshot_Activated()) {
            ...
            static const char *_light_types[] = {"Unknown", "Point", "Spot",
                                                 "Directional"};
            WWASSERT((light->Type >= 0) && (light->Type <= 3));

            SNAPSHOT_SAY((" type = %s amb = %4.2f,%4.2f,%4.2f  diff = "
                          "%4.2f,%4.2f,%4.2f spec = %4.2f, %4.2f, %4.2f",
                          _light_types[light->Type], light->Ambient.r,
                          light->Ambient.g, light->Ambient.b, light->Diffuse.r,
                          light->Diffuse.g, light->Diffuse.b, light->Specular.r,
                          light->Specular.g, light->Specular.b));
            SNAPSHOT_SAY((" pos = %f, %f, %f  dir = %f, %f, %f",
                          light->Position.x, light->Position.y,
                          light->Position.z, light->Direction.x,
...
IDirect3DTexture8 *
DX8Wrapper::_Create_DX8_Texture(unsigned int width, unsigned int height,
                                WW3DFormat format, MipCountType mip_level_count,
                                D3DPOOL pool, bool rendertarget) {
  DX8_THREAD_ASSERT();
  DX8_Assert();
  ...
  // NOTE: If 'format' is not supported as a texture format, this function will
  // find the closest format that is supported and use that instead.

  // Render target may return NOTAVAILABLE, in
  // which case we return null.
  if (rendertarget) {
    unsigned ret = D3DXCreateTexture(
        DX8Wrapper::_Get_D3D_Device8(), width, height, mip_level_count,
    ...
    if (ret == D3DERR_OUTOFVIDEOMEMORY) {
      ...
      ret = D3DXCreateTexture(DX8Wrapper::_Get_D3D_Device8(), width, height,
                              mip_level_count, D3DUSAGE_RENDERTARGET,
  ...
  // We should never run out of video memory when allocating a non-rendertarget
  // texture. However, it seems to happen sometimes when there are a lot of
  // textures in memory and so if it happens we'll release assets and try again
  // (anything is better than crashing).
  unsigned ret = D3DXCreateTexture(
      DX8Wrapper::_Get_D3D_Device8(), width, height, mip_level_count, 0,
      WW3DFormat_To_D3DFormat(format), pool, &texture);

  // If ran out of texture ram, try invalidating some textures and mesh cache.
  if (ret == D3DERR_OUTOFVIDEOMEMORY) {
    WWDEBUG_SAY(("Error: Out of memory while creating texture. Trying to "
                 "release assets..."));
    // Free all textures that haven't been used in the last 5 seconds
    TextureClass::Invalidate_Old_Unused_Textures(5000);
    ...
    ret = D3DXCreateTexture(DX8Wrapper::_Get_D3D_Device8(), width, height,
                            mip_level_count, 0, WW3DFormat_To_D3DFormat(format),
                            pool, &texture);
    if (SUCCEEDED(ret)) {
...
DX8Wrapper::_Create_DX8_Texture(const char *filename,
  ...
  // NOTE: If the original image format is not supported as a texture format, it
  // will automatically be converted to an appropriate format. NOTE: It is
  // possible to get the size and format of the original image file from this
  // function as well, so if we later want to second-guess D3DX's format
  // conversion decisions we can do so after this function is called..
  unsigned result = D3DXCreateTextureFromFileExA(
      _Get_D3D_Device8(), filename, D3DX_DEFAULT, D3DX_DEFAULT,
...
IDirect3DTexture8 *
DX8Wrapper::_Create_DX8_Texture(IDirect3DSurface8 *surface,
                                MipCountType mip_level_count) {
  DX8_THREAD_ASSERT();
  DX8_Assert();
  IDirect3DTexture8 *texture = nullptr;
  ...
  // This function will create a texture with a different (but similar) format
  // if the surface is not in a supported texture format.
  WW3DFormat format = D3DFormat_To_WW3DFormat(surface_desc.Format);
  texture = _Create_DX8_Texture(surface_desc.Width, surface_desc.Height, format,
                                mip_level_count);

  // Copy the surface to the texture
  IDirect3DSurface8 *tex_surface = nullptr;
  texture->GetSurfaceLevel(0, &tex_surface);
  DX8_ErrorCode(D3DXLoadSurfaceFromSurface(tex_surface, nullptr, nullptr,
 ...
 */
IDirect3DTexture8 *
DX8Wrapper::_Create_DX8_ZTexture(unsigned int width, unsigned int height,
                                 WW3DZFormat zformat,
                                 MipCountType mip_level_count, D3DPOOL pool) {
  DX8_THREAD_ASSERT();
  DX8_Assert();
  IDirect3DTexture8 *texture = nullptr;
  ...
  unsigned ret = DX8Wrapper::_Get_D3D_Device8()->CreateTexture(
      width, height, mip_level_count, D3DUSAGE_DEPTHSTENCIL, zfmt, pool,
      &texture);
  ...
  // If ran out of texture ram, try invalidating some textures and mesh cache.
  if (ret == D3DERR_OUTOFVIDEOMEMORY) {
    WWDEBUG_SAY(("Error: Out of memory while creating render target. Trying to "
                 "release assets..."));
    // Free all textures that haven't been used in the last 5 seconds
    TextureClass::Invalidate_Old_Unused_Textures(5000);
...
IDirect3DCubeTexture8 *DX8Wrapper::_Create_DX8_Cube_Texture(
    ...
    MipCountType mip_level_count, D3DPOOL pool, bool rendertarget) {
  ...
  if (rendertarget) {
    unsigned ret = D3DXCreateCubeTexture(
        DX8Wrapper::_Get_D3D_Device8(), width, mip_level_count,
        D3DUSAGE_RENDERTARGET, WW3DFormat_To_D3DFormat(format), pool, &texture);
    ...
    if (ret == D3DERR_OUTOFVIDEOMEMORY) {
      ...
      ret = D3DXCreateCubeTexture(DX8Wrapper::_Get_D3D_Device8(), width,
                                  mip_level_count, D3DUSAGE_RENDERTARGET,
                                  WW3DFormat_To_D3DFormat(format), pool,
                                  &texture);
  ...
  // (anything is better than crashing).
  unsigned ret = D3DXCreateCubeTexture(
      DX8Wrapper::_Get_D3D_Device8(), width, mip_level_count, 0,
      WW3DFormat_To_D3DFormat(format), pool, &texture);
...
IDirect3DVolumeTexture8 *DX8Wrapper::_Create_DX8_Volume_Texture(
    ...
    WW3DFormat format, MipCountType mip_level_count, D3DPOOL pool) {
  DX8_THREAD_ASSERT();
  DX8_Assert();
  ...
  unsigned ret = D3DXCreateVolumeTexture(
      DX8Wrapper::_Get_D3D_Device8(), width, height, depth, mip_level_count, 0,
      WW3DFormat_To_D3DFormat(format), pool, &texture);
  ...
  if (ret == D3DERR_OUTOFVIDEOMEMORY) {
    ...
    ret = D3DXCreateVolumeTexture(
        DX8Wrapper::_Get_D3D_Device8(), width, height, depth, mip_level_count,
...
IDirect3DSurface8 *DX8Wrapper::_Create_DX8_Surface(unsigned int width,
  ...
  DX8CALL(CreateImageSurface(width, height, WW3DFormat_To_D3DFormat(format),
                             &surface));
...
IDirect3DSurface8 *DX8Wrapper::_Create_DX8_Surface(const char *filename_) {
  ...
  // Note: Since there is no "D3DXCreateSurfaceFromFile" and no
  // "GetSurfaceInfoFromFile" (the latter is supposed to be added to D3DX in a
  // future version), we create a texture from the file (w/o mipmaps), check
  // that its surface is equal to the original file data (which it will not be
  // if the file is not in a texture-supported format or size). If so, copy its
  // surface (we might be able to just get its surface and add a ref to it but
  // I'm not sure so I'm not going to risk it) and release the texture. If not,
  // create a surface according to the file data and use
  // D3DXLoadSurfaceFromFile. This is a horrible hack, but it saves us having to
  // write file loaders. Will fix this when D3DX provides us with the right
  // functions. Create a surface the size of the file image data
  IDirect3DSurface8 *surface = nullptr;

  {
    ...
    if (!myfile->Is_Available()) {
      ...
      if (ext && (strlen(ext) == 4) && ((ext[1] == 't') || (ext[1] == 'T')) &&
          ((ext[2] == 'g') || (ext[2] == 'G')) &&
          ((ext[3] == 'a') || (ext[3] == 'A'))) {
        ext[1] = 'd';
        ext[2] = 'd';
  ...
  surface = TextureLoader::Load_Surface_Immediate(filename_string,
                                                  WW3D_FORMAT_UNKNOWN, true);
  return surface;
}

/***********************************************************************************************
 * DX8Wrapper::_Update_Texture -- Copies a texture from system memory to video
 *memory          *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
...
void DX8Wrapper::_Update_Texture(TextureClass *system, TextureClass *video) {
  ...
  DX8CALL(UpdateTexture(system->Peek_D3D_Base_Texture(),
                        video->Peek_D3D_Base_Texture()));
}
...
void DX8Wrapper::Compute_Caps(WW3DFormat display_format) {
  ...
  DX8_Assert();
  delete CurrentCaps;
  CurrentCaps = new DX8Caps(_Get_D3D8(), D3DDevice, display_format,
...
void DX8Wrapper::Set_Light(unsigned index, const LightClass &light) {
  ...
  if (fabs(a - b) < 1e-5)
    ...
    // this will cause the light to drop to half intensity at the first far
    // attenuation
...
void DX8Wrapper::Set_Light_Environment(LightEnvironmentClass *light_env) {
  ...
  // *
  //	if (Light_Environment && light_env &&
  //(*Light_Environment)==(*light_env)) return;
  ...
  if (light_env) {
    ...
    unsigned int color =
        Convert_Color(light_env->Get_Equivalent_Ambient(), 0.0f);
    if (RenderStates[D3DRS_AMBIENT] != color) {
      Set_DX8_Render_State(D3DRS_AMBIENT, color);
// buggy Radeon 9700 driver doesn't apply new ambient unless the material also
// changes.
#if 1
    ...
    for (; l < light_count; ++l) {
      ...
      // (gth) TODO: put specular into LightEnvironment?  Much work to be done
      // on lights :-)'
      if (l == 0) {
...
//(gth) CNC3 Generals code for the attenuation factors is causing the lights to
// over-brighten I'm changing the Attenuation0 parameter to 1.0 to avoid this
// problem.
#if 0
        ...
        if (fabs(a - b) < 1e-5)
          ...
          // this will cause the light to drop to half intensity at the first
          // far attenuation
...
TextureClass *DX8Wrapper::Create_Render_Target(int width, int height,
  ...
  //
  //	Note: We're going to force the width and height to be powers of two and
  // equal
  //
  ...
  //
  TextureClass *tex =
      NEW_REF(TextureClass, (width, height, format, MIP_LEVELS_1,
...
/*! KJM
 */
void DX8Wrapper::Create_Render_Target(int width, int height, WW3DFormat format,
                                      WW3DZFormat zformat,
                                      TextureClass **target,
  ...
  *depth_buffer = NEW_REF(ZTextureClass, (width, height, zformat, MIP_LEVELS_1,
...
// Contrast - controls the difference between the maximum and the minimum of the
// curve
void DX8Wrapper::Set_Gamma(float gamma, float bright, float contrast,
  ...
  // IML: I'm not really sure what the intent of the 'limit' variable is. It
  // does not produce useful results for my purposes.
  if (uselimit) {
...
void DX8Wrapper::Apply_Default_State() {
  ...
  for (i = 0; i < CurrentCaps->Get_Max_Textures_Per_Pass(); i++) {
    ...
    //		Set_DX8_Texture_Stage_State(i, D3DTSS_MAGFILTER,
    // D3DTEXF_LINEAR); 		Set_DX8_Texture_Stage_State(i,
    ...
    Set_DX8_Texture_Stage_State(i, D3DTSS_TEXTURETRANSFORMFLAGS,
                                D3DTTFF_DISABLE);
    Set_Texture(i, nullptr);
  }
...
const char *
DX8Wrapper::Get_DX8_Texture_Stage_State_Name(D3DTEXTURESTAGESTATETYPE state) {
  switch (state) {
  case D3DTSS_COLOROP:
...
void DX8Wrapper::Get_DX8_Texture_Stage_State_Value_Name(
    StringClass &name, D3DTEXTURESTAGESTATETYPE state, unsigned value) {
  switch (state) {
  case D3DTSS_COLOROP:
  case D3DTSS_ALPHAOP:
