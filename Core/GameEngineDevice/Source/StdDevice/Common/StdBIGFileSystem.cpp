...
ArchiveFile *StdBIGFileSystem::openArchiveFile(const Char *filename) {
  ...
  ArchiveFile *archiveFile =
      NEW StdBIGFile(filename, AsciiString::TheEmptyString);

  DEBUG_LOG(
      ("StdBIGFileSystem::openArchiveFile - opening BIG file %s", filename));
  ...
  DEBUG_LOG(
      ("StdBIGFileSystem::openArchiveFile - size of archive file is %d bytes",
       archiveFileSize));

  //	char t;

  // read in the number of files contained in this BIG file.
  // change the order of the bytes cause the file size is in reverse byte order
  // for some reason.
  ...
  for (Int i = 0; i < numLittleFiles; ++i) {
    ...
    while ((filenameIndex >= 0) && (buffer[filenameIndex] != '\\') &&
           (buffer[filenameIndex] != '/')) {
      --filenameIndex;
    }
    ...
    //		DEBUG_LOG(("StdBIGFileSystem::openArchiveFile - adding file %s
...
void StdBIGFileSystem::closeArchiveFile(const Char *filename) {
  ...
  DEBUG_ASSERTCRASH(stricmp(filename, MUSIC_BIG) == 0,
                    ("Attempting to close Archive file '%s', need to add code "
                     "to handle its shutdown correctly.",
                     filename));
...
Bool StdBIGFileSystem::loadBigFilesFromDirectory(AsciiString dir,
  ...
  while (it != filenameList.end()) {
    ...
    if (archiveFile != nullptr) {
      DEBUG_LOG(("StdBIGFileSystem::loadBigFilesFromDirectory - loading %s "
                 "into the directory tree.",
                 (*it).str()));
      loadIntoDirectoryTree(archiveFile, overwrite);
      m_archiveFileMap[(*it)] = archiveFile;
      DEBUG_LOG(("StdBIGFileSystem::loadBigFilesFromDirectory - %s inserted "
                 "into the archive file map.",
                 (*it).str()));
      actuallyAdded = TRUE;
    }
