...
class WWMath {
public:
  // Initialization and Shutdown.  Other math sub-systems which require
  // initialization and shutdown processing will be handled in these functions
  static void Init();
  static void Shutdown();

  // These are meant to be a collection of small math utility functions to be
  ...
  static WWINLINE float
  Inv_Sqrt(float a); // Some 30% faster inverse square root than regular C++
                     // compiled, from Intel's math library
  ...
  static WWINLINE double Clamp(double val, double min = 0.0f,
                               double max = 1.0f);
  ...
  static WWINLINE unsigned char Unit_Float_To_Byte(float f) {
    return (unsigned char)(f * 255.0f);
  }
  static WWINLINE float Byte_To_Unit_Float(unsigned char byte) {
  ...
  static WWINLINE float
  Normalize_Angle(float angle); // Normalizes the angle to the range -PI..PI
};
...
WWINLINE float WWMath::Clamp(float val, float min /*= 0.0f*/,
                             float max /*= 1.0f*/) {
  if (val < min)
    return min;
  if (val > max)
...
WWINLINE double WWMath::Clamp(double val, double min /*= 0.0f*/,
                              double max /*= 1.0f*/) {
  if (val < min)
    return min;
  if (val > max)
...
WWINLINE float WWMath::Wrap(float val, float min /*= 0.0f*/,
                            float max /*= 1.0f*/) {
...
WWINLINE double WWMath::Wrap(double val, double min /*= 0.0f*/,
                             double max /*= 1.0f*/) {
  // Implemented as an if rather than a while, to long loops
  if (val >= max)
    val -= (max - min);
...
WWINLINE int WWMath::Float_To_Int_Chop(const float &f) {
  int a = *reinterpret_cast<const int *>(
      &f); // take bit pattern of float into a register
  int sign =
      (a >>
       31); // sign = 0xFFFFFFFF if original value is negative, 0 if positive
  int mantissa = (a & ((1 << 23) - 1)) |
                 (1 << 23); // extract mantissa and add the hidden bit
  int exponent = ((a & 0x7fffffff) >> 23) - 127; // extract the exponent
  int r = ((unsigned int)(mantissa) << 8) >>
          (31 - exponent); // ((1<<exponent)*mantissa)>>24 -- (we know that
                           // mantissa > (1<<24))
  return ((r ^ (sign)) - sign) &
         ~(exponent >> 31); // add original sign. If exponent was negative, make
                            // return value 0.
}
...
WWINLINE int WWMath::Float_To_Int_Floor(const float &f) {
  ...
  int expsign =
      ~(exponent >> 31); // 0xFFFFFFFF if exponent is positive, 0 otherwise
  int imask = ((1 << (31 - (exponent)))) - 1; // mask for true integer values
  int mantissa =
      (a & ((1 << 23) - 1)); // extract mantissa (without the hidden bit)
  int r = ((unsigned int)(mantissa | (1 << 23)) << 8) >>
          (31 - exponent); // ((1<<exponent)*(mantissa|hidden bit))>>24 -- (we
                           // know that mantissa > (1<<24))

  r = ((r & expsign) ^ (sign)) +
      (((!((mantissa << 8) & imask)) & (expsign ^ ((a - 1) >> 31))) &
       sign); // if (fabs(value)<1.0) value = 0; copy sign; if (value < 0 &&
...
#else
WWINLINE float WWMath::Inv_Sqrt(float val) { return 1.0f / (float)sqrt(val); }
#endif
