...
#define MulDiv(nNumber, nNumerator, nDenominator)                              \
  (int)(((long long)(nNumber) * (nNumerator) + ((nDenominator) / 2)) /         \
        (nDenominator))
#include "../WW3D2/emscripten_compat/windows.h"
#endif
#include <stdlib.h>
#include <sys/stat.h>
...
//
// Kick off a download - copys the parameters for the download to the
// appropriate data members, then sets the status flag to start the download
...
HRESULT CDownload::DownloadFile(LPCSTR server, LPCSTR username, LPCSTR password,
  ...
  if ((m_Status != DOWNLOADSTATUS_DONE) &&
  ...
  if ((server == nullptr) || (username == nullptr) || (password == nullptr) ||
      (file == nullptr) || (localfile == nullptr) || (regkey == nullptr)) {
  ...
  /*********
          ...
          else if(( m_StartPosition = m_Ftp->FileRecoveryPosition( m_LocalFile,
  m_RegKey ) ) != 0 )
          {
...
//
// PumpMessages() does all the work - checks for possible resumption of a
// previous download, connects to the server, finds the file and downloads it.
//
...
HRESULT CDownload::PumpMessages() {
  ...
  if (m_Status == DOWNLOADSTATUS_GO) {
    ...
    /***********
                    ...
                    else if(	( m_StartPosition = m_Ftp->FileRecoveryPosition(
    m_LocalFile, m_RegKey ) ) != 0 )
                    {
                            if( Listener->OnQueryResume() ==
    DOWNLOADEVENT_DONOTRESUME )
                            {
                                    m_Ftp->RestartFrom( 0 );
                                    m_StartPosition = 0;
                            }
  ...
  if ((m_Status == DOWNLOADSTATUS_FINDINGFILE) && (strlen(m_File))) {
    ...
    if (m_FileSize > 0) {
      // First check to see if we already have the complete file in the proper
      // dest location...
      //
      // Note, we will only skip the download if the file is a patch.  Patch
      // files should
  ...
  if (m_Status == DOWNLOADSTATUS_DOWNLOADING) {
    ...
    if ((m_BytesRead - m_StartPosition) > 0) {
      ...
      m_predictionTimes[predictionIndex] =
          MulDiv(timetaken, (m_FileSize - m_BytesRead),
                 (m_BytesRead - m_StartPosition));
      ...
      // m_predictionTimes[ predictionIndex ] = ( ( m_FileSize - m_BytesRead )
      // * timetaken ) / ( m_BytesRead - m_StartPosition ); m_predictionTimes[
      // predictionIndex ] = ( ( m_FileSize - m_BytesRead ) / ( m_BytesRead -
      // m_StartPosition ) * timetaken );

      DEBUG_LOG(("About to OnProgressUpdate() - m_FileSize=%d, m_BytesRead=%d, "
                 "timetaken=%d, numerator=%d",
                 m_FileSize, m_BytesRead, timetaken,
                 ((m_FileSize - m_BytesRead) * timetaken)));
      DEBUG_LOG((", m_startPosition=%d, denominator=%d, predictionTime=%d",
                 m_StartPosition, (m_BytesRead - m_StartPosition),
                 predictionIndex));
      DEBUG_LOG(("vals are %d %d %d %d %d %d %d %d", m_predictionTimes[0],
                 m_predictionTimes[1], m_predictionTimes[2],
                 m_predictionTimes[3], m_predictionTimes[4],
                 m_predictionTimes[5], m_predictionTimes[6],
                 m_predictionTimes[7]));
    ...
    Listener->OnProgressUpdate(m_BytesRead, m_FileSize, timetaken,
                               averagetimepredicted + 1);
  }
  ...
  if (m_Status == DOWNLOADSTATUS_FINISHING) {
    if (m_Ftp->m_iCommandSocket)
      ...
      m_Status =
          DOWNLOADSTATUS_DONE; // command channel closed, connect again...
