...
#include <process.h>
#else
#include "../WW3D2/emscripten_compat/windows.h" // For Emscripten compat
#endif
#endif
#include <sys/stat.h>
#ifndef __EMSCRIPTEN__
...
#define gethostbynameA gethostbyname
#endif
#include <errno.h>
#include <time.h>
...
// Some firewalls break the TCP stack so non-blocking sockets don't work
// right...
//
static bool Use_Non_Blocking_Mode() {
  ...
  regRetval = RegOpenKeyEx(HKEY_LOCAL_MACHINE,
                           "SOFTWARE\\Westwood\\Earth And Beyond Beta 2", 0,
                           KEY_READ, &regKey);
  ...
  bufsiz = sizeof(value);
  type = REG_DWORD;
  regRetval = RegQueryValueEx(regKey, "UseNonBlockingFTP", nullptr, &type,
                              (BYTE *)&value, &bufsiz);
...
int Cftp::AsyncGetHostByName(char *szName, struct sockaddr_in &address) {
  ...
  if (stat == 0) {
    ...
    if (CreateThread(nullptr, 0, gethostbynameA, szName, 0, &threadid) ==
        nullptr) {
      return (FTP_FAILED);
    }
    stat = 1;
 ...
 *	Optionally, the name of the server to connect to.  This must be
 *specified in the first call to the function.
 *
 * $_ReturnValue :
...
HRESULT Cftp::ConnectToServer(LPCSTR szServerName) {
  ...
  if (m_iStatus == FTPSTAT_INIT) {
    ...
    if (serverIP == INADDR_NONE) {
      ...
      ///////memcpy( (char *)&(m_CommandSockAddr.sin_addr), he.h_addr,
      /// he.h_length );
    ...
    if (connect(m_iCommandSocket, (struct sockaddr *)&m_CommandSockAddr,
 ...
 *
 *	<- int * piSize
 *	A pointer to an integer in which to return the size of the file, in
 *bytes.
 *
 * $_ReturnValue :
 ...
 *
 * $_Warnings    :
 *	This function will continue to return FTP_TRYING if the file does not
 *exist on the server.
 *
 ***************************************************************************/
...
HRESULT Cftp::FindFile(LPCSTR szRemoteFileName, int *piSize) {
  ...
  _splitpath(szRemoteFileName, nullptr,
             m_szRemoteFilePath + strlen(m_szRemoteFilePath),
...
/*$_Function****************************************************************
 * $_Name        :
 *	HRESULT  Cftp::RecvReply( LPCSTR pReplyBuffer, int iSize, int *
 *piRetCode )
 *
 * $_Description :
 ...
 *
 *	<- piRetCode
 *	A pointer to an integer to store the numeric return code for the message
 *received.
 *
 * $_ReturnValue :
...
HRESULT Cftp::RecvReply(LPCSTR pReplyBuffer, int iSize, int *piRetCode) {
  ...
  while (1) {
    // Preview whats on the socket so we can make the decision if there is
    // enough there to use.
    ...
    // Is this an unrecognized command?  It's probably in response to our
    // BOGUSCOMMAND, ignore it. If we got this for some other reason then this
    // FTP server doesn't support our required command set. If this were a
    // general purpose FTP library we probably couldn't get away with this
 ...
 * $_Parameters  :
 *	-> int sockfd
 *	An optional socket descriptor - if this is given and the socket is
 *connected then the answer will guaranteed to be on the same interface as the
 *connected socket.
 *
 * $_ReturnValue :
...
int Cftp::SendNewPort() {
  ...
  if (m_sendNewPortStatus == 0) {
    ...
    m_DataSockAddr.sin_port =
        0; // Winsock fills in an ephemeral port during bind...

    if (bind(m_iDataSocket, (struct sockaddr *)&m_DataSockAddr,
             sizeof(m_DataSockAddr)) < 0) {
      return (FTP_FAILED);
    }
  ...
  if (m_sendNewPortStatus == 1) {
    ...
    sprintf(command, "PORT %d,%d,%d,%d,%d,%d\r\n", i & 0xFF, (i >> 8) & 0xFF,
            (i >> 16) & 0xFF, (i >> 24) & 0xFF, m_DataSockAddr.sin_port & 0xFF,
...
/*$_Function****************************************************************
 * $_Name        :
 *	HRESULT  Cftp::GetNextFileBlock( LPCSTR szLocalFileName, int *
 *piTotalRead )
 *
 * $_Description :
 ...
 *
 *	The file is initally written to a temporary file and then copied to the
 *local file name when the transfer is complete.  If the transfer is halted, the
 *	temporary file will be used as the starting point for subsequent
 *retries.
 *
 * $_Parameters  :
 *	-> LPCSTR szLocalFileName
 *	A pointer to a string containing the local file name for the transferred
 *file.
 *
 *	<- int * piTotalRead
 ...
 *
 * $_ReturnValue :
 *	FTP_TRYING while the transfer is in progress, FTP_SUCCEEDED on
 *completion, FTP_FAILED on error.
 *
 ***************************************************************************/
...
HRESULT Cftp::GetNextFileBlock(LPCSTR szLocalFileName, int *piTotalRead) {
    ...
    do {
      ...
      if (read > 0) {
        ...
        totread += read;
             ...
             (totread < (max_data_len * 20))); // Don't read too much -
  ...
  if (m_iStatus == FTPSTAT_FILEDATACLOSED) {
    ...
    if (rename(downloadfilename, m_szLocalFileName) != 0) {
      ...
      _chmod(m_szLocalFileName,
             _S_IWRITE | _S_IREAD); // make sure it's not readonly
      DEBUG_LOG(
          ("_chmod of %s failed with errno of %d", m_szLocalFileName, errno));
      remove(m_szLocalFileName);
      DEBUG_LOG(
          ("remove of %s failed with errno of %d", m_szLocalFileName, errno));
    ...
    // Send a bogus command.  If the firewall has closed our ftp command channel
    // connection this will allow us to detect it.  (Obviously the data channel
    // is closed or we wouldn't be here) There are cases (firewall) where the
    // connection will go away without having a close command but writing to it
    // will allow us to detect this case. We will ignore the response because we
    // don't know in which order the server will respond
...
//
// Do we have this file in the download directory?  If so then it's a partial
// download.
//
//
HRESULT Cftp::FileRecoveryPosition(LPCSTR szLocalFileName,
...
/*************8
                ...
                0, KEY_ALL_ACCESS, &hkey ) != ERROR_SUCCESS )
        {
                ...
                RegSetValueEx( hkey, (LPCTSTR)"File", 0, REG_SZ, (const unsigned
char *)szLocalFileName, strlen( szLocalFileName ) + 1 ); RegCloseKey(hkey);
        ...
        if( RegQueryValueEx( hkey, (LPCTSTR)"File", 0, &t1, regfilename, &t2 )
!= ERROR_SUCCESS )
        {
                RegSetValueEx( hkey, (LPCTSTR)"File", 0, REG_SZ, (const unsigned
char *)szLocalFileName, strlen( szLocalFileName ) + 1 ); RegCloseKey(hkey);
        ...
        RegSetValueEx( hkey, (LPCTSTR)"File", 0, REG_SZ, (const unsigned char
*)szLocalFileName, strlen( szLocalFileName ) + 1 ); RegCloseKey(hkey);
...
void Cftp::GetDownloadFilename(const char *localname, char *downloadname,
  ...
  snprintf(downloadname, downloadname_size, "download\\%s_%d.tmp", name,
           m_iFileSize);
  free(name);
  /*
  Wstring name;
