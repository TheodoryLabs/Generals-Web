...
// typedefs removed for emscripten
...
public:
  WWProfileHierarchyNodeClass(const char *name,
  ...
  WWProfileHierarchyNodeClass *
  Clone_Hierarchy(WWProfileHierarchyNodeClass *parent);
...
public:
  WWProfileHierarchyInfoClass(const char *name,
...
public:
  WWINLINE static void Enable_Profile(bool enable) {
    IsProfileEnabled = enable;
  }
  ...
  static void Load_Profile_Log(const char *filename,
                               WWProfileHierarchyInfoClass **&array,
                               unsigned &count);
...
public:
  WWProfileSampleClass(const char *name, bool is_root)
      : IsRoot(is_root), Enabled(WWProfileManager::Is_Profile_Enabled()) {
    if (Enabled) {
      if (IsRoot)
...
/*
** WWTimeIt is like WWProfile, but it doesn't save anything, it just times one
*routine, regardless of thread
*/
class WWTimeItClass {
public:
...
/*
** WWMeasureItClass is like WWTimeItClass, but it pokes the result into the
*given float,
...
#define WWLOG_PREPARE_TIME_AND_MEMORY(t)                                       \
  WWMemoryAndTimeLog memory_and_time_log(t)
