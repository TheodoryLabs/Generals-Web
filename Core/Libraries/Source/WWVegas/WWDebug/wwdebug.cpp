 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * WWDebug_Install_Message_Handler -- install function for handling
 *the debug messages       * WWDebug_Install_Assert_Handler -- Install a
 *function for handling the assert messages     *
 *   WWDebug_Install_Trigger_Handler -- install a trigger handler function *
 *   WWDebug_Printf -- Internal function for passing messages to installed
 *handler             * WWDebug_Assert_Fail -- Internal function for passing
 *assert messages to installed handler * WWDebug_Assert_Fail_Print -- Internal
 *function, passes assert message to handler          * WWDebug_Check_Trigger --
 *calls the user-installed debug trigger handler                   *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *- - - - - - - */
...
#ifndef _UNIX
  FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, nullptr, id, 0, buffer, buf_len,
                nullptr);
#endif
}
...
/***********************************************************************************************
 * WWDebug_Install_Message_Handler -- install function for handling the debug
 *messages         *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * WWDebug_Install_Assert_Handler -- Install a function for handling the assert
 *messages       *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * WWDebug_Printf_Warning -- Internal function for passing messages to installed
 *handler       *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * WWDebug_Printf_Error -- Internal function for passing messages to installed
 *handler         *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
/***********************************************************************************************
 * WWDebug_Assert_Fail -- Internal function for passing assert messages to
 *installed handler   *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
...
void WWDebug_Assert_Fail(const char *expr, const char *file, int line) {
  if (_CurAssertHandler != nullptr) {
    ...
    /*
    // If the exception handler is try to quit the game then don't show an
    assert.
    */
 ...
 * WARNINGS: None *
 *                                                                                             *
 * HISTORY: * 12/11/2001 3:56PM ST : Created *
 *=============================================================================================*/
...
/***********************************************************************************************
 * WWDebug_Assert_Fail_Print -- Internal function, passes assert message to
 *handler            *
 *                                                                                             *
 * INPUT: *
 *                                                                                             *
 ...
 *=============================================================================================*/
#ifdef WWDEBUG
void WWDebug_Assert_Fail_Print(const char *expr, const char *file, int line,
                               const char *string) {
  if (_CurAssertHandler != nullptr) {
...
void WWDebug_DBWin32_Message_Handler(const char *str) {
  ...
  hSharedFile = CreateFileMapping((HANDLE)-1, nullptr, PAGE_READWRITE, 0, 4096,
                                  "DBWIN_BUFFER");
  if (!hSharedFile) {
    // MessageBox(nullptr, "DebugTrace: Unable to create file mapping object
    // DBWIN_BUFFER", "Error", MB_OK);
    CloseHandle(heventDBWIN);
  ...
  if (!lpszSharedMem) {
    // MessageBox(nullptr, "DebugTrace: Unable to map shared memory", "Error",
    // MB_OK);
    CloseHandle(heventDBWIN);
    CloseHandle(heventData);
    return;
