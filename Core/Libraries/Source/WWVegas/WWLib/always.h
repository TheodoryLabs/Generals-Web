...
// TheSuperHackers @build feliwir 17/04/2025 include utility macros for
// cross-platform compatibility
#include <Utility/compat.h>
#include <Utility/stdint_adapter.h>

// Disable warning about exception handling not being enabled. It's used as part
// of STL - in a part of STL we don't use.
#pragma warning(disable : 4530)

/*
** Define for debug memory allocation to include __FILE__ and __LINE__ for every
*memory allocation.
...
// (size_t s, void *p) { return p; } inline void __cdecl operator delete
// (void *, void *p)		{ }
...
private:                                                                       \
  static void *getClassMemoryPool() {                                          \
    /*                                                                         \
            Note that this static variable will be initialized exactly once:   \
       the first time control flows over this section of code. This allows us  \
       to neatly resolve the order-of-execution problem for static variables,  \
       ensuring this is not executed prior to the initialization of            \
       TheMemoryPoolFactory.                                                   \
    */                                                                         \
    static void *The##ARGCLASS##Pool =                                         \
        createW3DMemPool(#ARGCLASS, sizeof(ARGCLASS));                         \
...
public:                                                                        \
  inline void *operator new(size_t s) {                                        \
    return allocateFromW3DMemPool(getClassMemoryPool(), s);                    \
  }                                                                            \
  inline void operator delete(void *p) {                                       \
    freeFromW3DMemPool(getClassMemoryPool(), p);                               \
  }                                                                            \
  inline void *operator new(size_t s, const char *msg, int unused) {           \
    return allocateFromW3DMemPool(getClassMemoryPool(), s, msg, unused);       \
  }                                                                            \
  inline void operator delete(void *p, const char *msg, int unused) {          \
    freeFromW3DMemPool(getClassMemoryPool(), p);                               \
  }
...
protected:
  // we never call this; it is present to cause compile errors in descendent
  // classes
...
// Jani: MSVC doesn't necessarily inline code with inline keyword. Using
// __forceinline results better inlining and also prints out a warning if
// inlining wasn't possible. __forceinline is MSVC specific.
#if defined(_MSC_VER)
...
** NOTE: Joe used to #include <minmax.h> in the various compiler header files.
*This
** header defines 'min' and 'max' macros which conflict with the surrender code
*so
** I'm replacing all occurrences of 'min' and 'max with 'MIN' and 'MAX'.  For
*code which
** is out of our domain (e.g. Max sdk) I'm declaring template functions for
*'min' and 'max'
*/
#ifndef NOMINMAX
#define NOMINMAX
...
// GeneralsX @build BenderAI 09/03/2026 MSVC __max/__min compat for
...
/*
**	This includes the minimum set of compiler defines and pragmas in order
*to bring the *	various compilers to a common behavior such that the C&C engine
...
#include "../../../../../GeneralsMD/Code/Libraries/Source/WWVegas/WW3D2/emscripten_compat/windows.h"
#endif
