...
// Templated strlen.
// Returns the number of characters until the first zero character or when
// maxlen is reached.
template <typename T> size_t strnlen_t(const T *str, size_t maxlen) {
  const T *begin = str;
...
#endif
#ifndef HAVE_STRLCAT
inline size_t strlcat(char *dst, const char *src, size_t dstsize) {
  return strlcat_t(dst, src, dstsize);
}
#endif
inline size_t wcslcpy(wchar_t *dst, const wchar_t *src, size_t dstsize) {
  return strlcpy_t(dst, src, dstsize);
}
inline size_t wcslcat(wchar_t *dst, const wchar_t *src, size_t dstsize) {
  return strlcat_t(dst, src, dstsize);
}
...
inline size_t strlmove(char *dst, const char *src, size_t dstsize) {
  return strlmove_t(dst, src, dstsize);
}
inline size_t strlmcat(char *dst, const char *src, size_t dstsize) {
  return strlmcat_t(dst, src, dstsize);
}
inline size_t wcslmove(wchar_t *dst, const wchar_t *src, size_t dstsize) {
  return strlmove_t(dst, src, dstsize);
}
inline size_t wcslmcat(wchar_t *dst, const wchar_t *src, size_t dstsize) {
  return strlmcat_t(dst, src, dstsize);
}

#if !(defined(_MSC_VER) && _MSC_VER < 1300)
template <size_t Size> size_t strlcpy_t(char (&dst)[Size], const char *src) {
  return strlcpy_t(dst, src, Size);
}
template <size_t Size> size_t strlcat_t(char (&dst)[Size], const char *src) {
  return strlcat_t(dst, src, Size);
}
template <size_t Size> size_t strlmove_t(char (&dst)[Size], const char *src) {
  return strlmove_t(dst, src, Size);
}
template <size_t Size> size_t strlmcat_t(char (&dst)[Size], const char *src) {
  return strlmcat_t(dst, src, Size);
}
#endif

// Templated strncmp.
// Compares up to maxcount chars or a null byte is encountered, whichever comes
// first. Returns < 0 if str1 is less than str2, 0 is str1 and str2 are equal
// and > 0 if str2 is greater than str1.
template <typename T>
...
inline wchar_t tolower_t(wchar_t c) { return (wchar_t)towlower(c); }

// Templated strnicmp.
// Case insensitively compares up to maxcount chars or a null byte is
// encountered, whichever comes first. Returns < 0 if str1 is less than str2, 0
// is str1 and str2 are equal and > 0 if str2 is greater than str1.
template <typename T>
