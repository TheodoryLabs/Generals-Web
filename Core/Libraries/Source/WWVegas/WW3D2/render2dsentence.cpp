...
Render2DSentenceClass::Render2DSentenceClass()
    : Font(nullptr), Location(0.0F, 0.0F), Cursor(0.0F, 0.0F),
      TextureOffset(0, 0), TextureStartX(0), CurSurface(nullptr),
      CurrTextureSize(0), MonoSpaced(false), IsClippedEnabled(false),
      ClipRect(0, 0, 0, 0), BaseLocation(0, 0), LockedPtr(nullptr),
      LockedStride(0), TextureSizeHint(0), WrapWidth(0), Centered(false),
      DrawExtents(0, 0, 0, 0), ParseHotKey(false), useHardWordWrap(false) {
...
void Render2DSentenceClass::Build_Textures() {
  ...
  for (int index = 0; index < PendingSurfaces.Count(); index++) {
    ...
    TextureClass *new_texture = W3DNEW TextureClass(
        desc.Width, desc.Width, WW3D_FORMAT_A4R4G4B4, MIP_LEVELS_1);
    ...
    //
    for (int renderer_index = 0;
         renderer_index < surface_info.Renderers.Count(); renderer_index++) {
...
void Render2DSentenceClass::Draw_Sentence(uint32 color) {
  ...
  for (int index = 0; index < SentenceData.Count(); index++) {
    ...
    if (data.Surface != curr_surface) {
      ...
      //
      bool found = false;
      for (int renderer_index = 0; renderer_index < Renderers.Count();
           renderer_index++) {
      ...
      if (found == false) {
        ...
        curr_renderer->Set_Coordinate_Range(
        ...
        //
        for (int surface_index = 0; surface_index < PendingSurfaces.Count();
             surface_index++) {
    ...
    if (IsClippedEnabled) {
      ...
      if (screen_rect.Right <= ClipRect.Left ||
        ...
        float percent =
            ((clipped_rect.Left - screen_rect.Left) / screen_rect.Width());
        clipped_uv_rect.Left = uv_rect.Left + (uv_rect.Width() * percent);

        percent =
            ((clipped_rect.Right - screen_rect.Left) / screen_rect.Width());
        ...
        percent =
            ((clipped_rect.Bottom - screen_rect.Top) / screen_rect.Height());
...
void Render2DSentenceClass::Allocate_New_Surface(const WCHAR *text,
  ...
  if (!justCalcExtents) {
    ...
    //
    CurSurface = NEW_REF(
        SurfaceClass, (CurrTextureSize, CurrTextureSize, WW3D_FORMAT_A4R4G4B4));
...
float FindStartingXPos(const WCHAR *text) { return 1; }
////////////////////////////////////////////////////////////////////////////////////
//
//	Build_Sentence_Centered
//
////////////////////////////////////////////////////////////////////////////////////
void Render2DSentenceClass::Build_Sentence_Centered(const WCHAR *text, int *hkX,
                                                    int *hkY) {
  float char_height = Font->Get_Char_Height();
  int wordWidth = 0;
  int notCenteredHotkeyX = 0;
  int notCenteredHotkeyY = 0;
  Vector2 extent = Build_Sentence_Not_Centered(
      text, &notCenteredHotkeyX, &notCenteredHotkeyY,
      TRUE); // Get_Formatted_Text_Extents(text);
  ...
  while (!end) {
    ...
    while (1) {
      ...
      while ((*word != 0) && (*word > L' ') && (*word != L'\n')) {
        if (ParseHotKey && (*word == L'&') && (*word + 1 != 0) &&
            (*word + 1 > L' ') && (*word + 1 != L'\n')) {
          int offset = 0;
          if (word_width != 0) {
      ...
      //
      // If this word is unworthy to be on the current line, decrement the space
      // and break
      //
      if (WrapWidth > 0 && (line_width + word_width >= WrapWidth)) {
        //
        // Take care of the case that the word is too big for the allocated
        // space... If that's the case, drop out and process the word anyway
        //
        if (wordCount == 0 || (charCount == 0 && word_width > WrapWidth)) {
          // A single word is longer than the WrapWidth! We have to break it
          // forcefully
          word = text;
          while (charCount > 0) {
            word++;
          ...
          wordCount = 0;
          while (1) {
            if (*word == 0) { // <--- ADDED: PREVENT INFINITE LOOP
                break;
            }
            charWidth = Font->Get_Char_Spacing(*word++);

            if (line_width + charWidth >= WrapWidth) {
              const WCHAR *word_back = word;
              word_back--; // fixed: removed useless *
              wordCount++;
              break;
            } else {
          ...
          charCount += wordCount; // <--- ADDED: MUST UPDATE charCount SO WE DON'T DRAW 1 CHAR AT A TIME
        } else {
            // normal processing when wordCount > 0
            if (charCount == 0) {
      ...
      //
      // if we reached the end of the text, set the values and break, also set
      // the end flag
      //
      if (*word == 0) {
        charCount += wordCount;
    ...
    //
    // we now hold the length of the line and it's width lets set our cursor
    // position to center it
    //
    ...
    for (int i = 0; i <= charCount; i++) {
      ...
      //
      if (ParseHotKey && (ch == L'&') && (*text != 0) && (*text > L' ') &&
          (*text != L'\n')) {
        ch = *text++;
        dontBlit = true;
      }
      ...
      bool exceeded_texture_width =
          ((TextureOffset.I + char_spacing) >= CurrTextureSize);
      bool encountered_break_char = (ch == L' ' || ch == L'\n' || ch == 0);

      //
      //	Do we need to record this portion of the sentence to its own
      //chunk?
      //
      ...
      if (ch != L'\n' && ch != L' ') {
        ...
        //
        if (!dontBlit)
          Font->Blit_Char(ch, LockedPtr, LockedStride, TextureOffset.I,
                          TextureOffset.J);
...
Vector2 Render2DSentenceClass::Build_Sentence_Not_Centered(
    const WCHAR *text, int *hkX, int *hkY, bool justCalcExtents) {
  ...
  while (text != nullptr) {
    ...
    if (ParseHotKey && (ch == L'&') && (*text != 0) && (*text > L' ') &&
      ...
      if (calcHotKeyX)
        ...
        hotKeyPosX =
            Cursor.X + TextureOffset.I - TextureStartX; // TextureOffset.I;
    ...
    bool exceeded_texture_width =
        ((TextureOffset.I + char_spacing) >= CurrTextureSize);
    bool encountered_break_char = (ch == L' ' || ch == L'\n' || ch == 0);
    bool wordBiggerThenLine = ((useHardWordWrap) && (WrapWidth != 0) &&
                               ((Cursor.X + TextureOffset.I - TextureStartX +
                                 char_spacing) >= WrapWidth));
    //
    ...
    if (exceeded_texture_width || encountered_break_char ||
      ...
      if (ch == L' ') {
        ...
        break;
      } else if (wordBiggerThenLine) { // we've entered this loop because we're
                                       // greater then the wordwrap so we need
                                       // to force a wordwrap
        Cursor.X = 0;
        Cursor.Y += char_height;
      }
...
//
////////////////////////////////////////////////////////////////////////////////////
void Render2DSentenceClass::Build_Sentence(const WCHAR *text, int *hkX,
                                           int *hkY) {
  if (text == nullptr) {
    return;
  }
...
FontCharsClass::FontCharsClass()
    : OldGDIFont(nullptr), OldGDIBitmap(nullptr), GDIFont(nullptr),
      GDIBitmap(nullptr), GDIBitmapBits(nullptr), MemDC(nullptr),
      CurrPixelOffset(0), PointSize(0), CharHeight(0),
      UnicodeCharArray(nullptr), FirstUnicodeChar(0xFFFF), LastUnicodeChar(0),
      IsBold(false) {
...
//
////////////////////////////////////////////////////////////////////////////////////
void FontCharsClass::Blit_Char(WCHAR ch, uint16 *dest_ptr, int dest_stride,
                               int x, int y) {
  const FontCharsClassCharDataStruct *data = Get_Char_Data(ch);
...
bool FontCharsClass::Create_GDI_Font(const char *font_name) {
  ...
  DWORD italic = 0;
  GDIFont =
      ::CreateFont(font_height, fontWidth, 0, 0, bold, italic, FALSE, FALSE,
                   DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
                   ANTIALIASED_QUALITY, VARIABLE_PITCH, font_name);
  ...
  //
  GDIBitmap =
      ::CreateDIBSection(screen_dc, (const BITMAPINFO *)&bitmap_info,
                         DIB_RGB_COLORS, (void **)&GDIBitmapBits, nullptr, 0L);
...
//
////////////////////////////////////////////////////////////////////////////////////
bool FontCharsClass::Initialize_GDI_Font(const char *font_name, int point_size,
                                         bool is_bold) {
  //
...
//
////////////////////////////////////////////////////////////////////////////////////
bool FontCharsClass::Is_Font(const char *font_name, int point_size,
                             bool is_bold) {
  bool retval = false;
  ...
  if ((GDIFontName.Compare_No_Case(font_name) == 0) &&
      (point_size == PointSize) && (is_bold == IsBold)) {
    retval = true;
  }
...
void FontCharsClass::Grow_Unicode_Array(WCHAR ch) {
  ...
  //
  FontCharsClassCharDataStruct **new_array =
  ...
  if (UnicodeCharArray != nullptr) {
    ...
    ::memcpy(&new_array[start_offset], UnicodeCharArray,
