 ...
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
 * Functions: * SurfaceClass::Clear -- Clears a surface to 0 *
 *   SurfaceClass::Copy -- Copies a region from one surface to another of the
 *same format      * SurfaceClass::FindBBAlpha -- Finds the bounding box of non
 *zero pixels in the region (x0, * SurfaceClass::Is_Transparent_Column -- Tests
 *to see if the column is transparent or not   * SurfaceClass::Copy -- Copies
 *from a byte array to the surface                             *
 *   SurfaceClass::CreateCopy -- Creates a byte array copy of the surface *
 *   SurfaceClass::DrawHLine -- draws a horizontal line *
 *   SurfaceClass::DrawPixel -- draws a pixel * SurfaceClass::Copy -- Copies a
 *block of system ram to the surface                         *
...
void Convert_Pixel(Vector3 &rgb, const SurfaceClass::SurfaceDescription &sd,
...
SurfaceClass::SurfaceClass(unsigned width, unsigned height, WW3DFormat format)
    : D3DSurface(nullptr), SurfaceFormat(format) {
  WWASSERT(width);
  WWASSERT(height);
  D3DSurface = DX8Wrapper::_Create_DX8_Surface(width, height, format);
}

SurfaceClass::SurfaceClass(const char *filename) : D3DSurface(nullptr) {
...
SurfaceClass::LockedSurfacePtr
SurfaceClass::Lock(int *pitch, const Vector2i &min, const Vector2i &max) {
  D3DLOCKED_RECT lock_rect;
  ::ZeroMemory(&lock_rect, sizeof(D3DLOCKED_RECT));
...
void SurfaceClass::Unlock() { DX8_ErrorCode(D3DSurface->UnlockRect()); }
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 5/2/2001   hy : Created. *
 *=============================================================================================*/
void SurfaceClass::Copy(const Vector2i &min, const Vector2i &max,
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 3/16/2001  hy : Created. *
 *=============================================================================================*/
unsigned char *SurfaceClass::CreateCopy(int *width, int *height, int *size,
                                        bool flip) {
  SurfaceDescription sd;
  Get_Description(sd);
  ...
  *size = mysize;
  ...
  unsigned char *other =
      W3DNEWARRAY unsigned char[sd.Height * sd.Width * mysize];
  ...
  for (i = 0; i < sd.Height; i++) {
    if (flip) {
      memcpy(&other[(sd.Height - i - 1) * sd.Width * mysize], mem,
             mysize * sd.Width);
    } else {
...
void SurfaceClass::Copy(unsigned int dstx, unsigned int dsty, unsigned int srcx,
                        unsigned int srcy, unsigned int width,
  ...
  if (sd.Format == osd.Format && sd.Width == osd.Width &&
      sd.Height == osd.Height) {
    POINT dst;
    dst.x = dstx;
    dst.y = dsty;
    ...
    DX8_ErrorCode(D3DXLoadSurfaceFromSurface(D3DSurface, nullptr, &dest,
                                             other->D3DSurface, nullptr, &src,
                                             D3DX_FILTER_NONE, 0));
  }
}
 ...
 * HISTORY: * 2/13/2001  hy : Created. *
 *=============================================================================================*/
void SurfaceClass::Stretch_Copy(unsigned int dstx, unsigned int dsty,
...
/***********************************************************************************************
 * SurfaceClass::FindBB -- Finds the bounding box of non zero pixels in the
 *region             *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
...
void SurfaceClass::FindBB(Vector2i *min, Vector2i *max) {
  ...
  for (y = min->J; y < max->J; y++) {
    for (x = min->I; x < max->I; x++) {
      ...
      unsigned char *alpha = (unsigned char *)((unsigned int)lock_rect.pBits +
...
/***********************************************************************************************
 * SurfaceClass::Is_Transparent_Column -- Tests to see if the column is
 *transparent or not     *
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
...
bool SurfaceClass::Is_Transparent_Column(unsigned int column) {
  ...
  for (y = 0; y < (int)sd.Height; y++) {
    // HY - this is not endian safe
    unsigned char *alpha =
        (unsigned char *)((unsigned int)lock_rect.pBits + y * lock_rect.Pitch);
    unsigned char myalpha = alpha[size - 1];
    myalpha = (myalpha >> (8 - alphabits)) & mask;
    if (myalpha) {
      DX8_ErrorCode(D3DSurface->UnlockRect());
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 2/13/2001  hy : Created. * 1/10/2025  TheSuperHackers : Added bits
 *and pitch to argument list for better performance *
 *=============================================================================================*/
void SurfaceClass::Get_Pixel(Vector3 &rgb, int x, int y, LockedSurfacePtr pBits,
                             int pitch) {
  SurfaceDescription sd;
  Get_Description(sd);
  ...
  unsigned char *dst =
      static_cast<unsigned char *>(pBits) + y * pitch + x * bytesPerPixel;
  Convert_Pixel(rgb, sd, dst);
}

/***********************************************************************************************
 * SurfaceClass::Attach -- Attaches a surface pointer to the object, releasing
 *the current ptr.*
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
...
/***********************************************************************************************
 * SurfaceClass::Detach -- Releases the reference on the internal surface ptr,
 *and NULLs it.	 .*
 *                                                                                             *
 *                                                                                             *
 *                                                                                             *
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 1/10/2025  TheSuperHackers : Added bits and pitch to argument list
 *for better performance *
 *=============================================================================================*/
void SurfaceClass::Draw_Pixel(const unsigned int x, const unsigned int y,
                              unsigned int color, unsigned int bytesPerPixel,
                              LockedSurfacePtr pBits, int pitch) {
  unsigned char *dst =
      static_cast<unsigned char *>(pBits) + y * pitch + x * bytesPerPixel;
 ...
 * WARNINGS: *
 *                                                                                             *
 * HISTORY: * 4/9/2001   hy : Created. * 1/10/2025  TheSuperHackers : Added bits
 *and pitch to argument list for better performance *
 *=============================================================================================*/
void SurfaceClass::Draw_H_Line(const unsigned int y, const unsigned int x1,
                               const unsigned int x2, unsigned int color,
...
bool SurfaceClass::Is_Monochrome() {
  ...
  if (is_compressed) {
    ...
    SurfaceClass *new_surf =
        NEW_REF(SurfaceClass, (sd.Width, sd.Height, new_format));
