...
TextureLoadTaskListClass::TextureLoadTaskListClass() : Root() {
...
public:
  ...
  LoaderThreadClass(const char *thread_name = "Texture loader thread")
...
IDirect3DTexture8 *Load_Compressed_Texture(const StringClass &filename,
  ...
  // If format isn't defined get the nearest valid texture format to the
  // compressed file format Note that the nearest valid format could be
  // anything, even uncompressed.
  ...
  IDirect3DTexture8 *d3d_texture = DX8Wrapper::_Create_DX8_Texture(
      width, height, dest_format, (MipCountType)mips);

  for (unsigned level = 0; level < mips; ++level) {
    IDirect3DSurface8 *d3d_surface = nullptr;
    WWASSERT(d3d_texture);
    DX8_ErrorCode(d3d_texture->GetSurfaceLevel(level /*-reduction_factor*/,
                                               &d3d_surface));
...
static bool Is_Format_Compressed(WW3DFormat texture_format,
                                 bool allow_compression) {
  // Verify that the user isn't requesting compressed texture without hardware
  // support
  ...
  // If hardware supports DXTC compression, load a compressed texture. Proceed
  // only if the texture format hasn't been defined as non-compressed.
  compressed |=
      (texture_format == WW3D_FORMAT_UNKNOWN &&
       DX8Wrapper::Get_Current_Caps()->Support_DXTC() && allow_compression);
...
bool TextureLoader::Is_DX8_Thread() {
  return (ThreadClass::_Get_Current_Thread_ID() ==
          DX8Wrapper::_Get_Main_Thread_ID());
}
...
void TextureLoader::Validate_Texture_Size(unsigned &width, unsigned &height,
  ...
  if (dx8caps.MaxTextureWidth > 0 && poweroftwowidth > dx8caps.MaxTextureWidth) {
    poweroftwowidth = dx8caps.MaxTextureWidth;
  }
  if (dx8caps.MaxTextureHeight > 0 && poweroftwoheight > dx8caps.MaxTextureHeight) {
    poweroftwoheight = dx8caps.MaxTextureHeight;
  }
  if (dx8caps.MaxVolumeExtent > 0 && poweroftwodepth > dx8caps.MaxVolumeExtent) {
...
IDirect3DTexture8 *TextureLoader::Load_Thumbnail(
    ...
    const Vector3 &hsv_shift) //,WW3DFormat texture_format)
{
  WWASSERT(Is_DX8_Thread());

  ThumbnailClass *thumb = nullptr;
  thumb =
      ThumbnailManagerClass::Peek_Thumbnail_Instance_From_Any_Manager(filename);
  ...
  if (texture_format == WW3D_FORMAT_UNKNOWN) {
    dest_format = Get_Valid_Texture_Format(
        WW3D_FORMAT_A4R4G4B4, false); // no compressed formats please
  } else {
    dest_format = Get_Valid_Texture_Format(
        texture_format, false); // no compressed formats please
    WWASSERT(dest_format == texture_format);
  }

  IDirect3DTexture8 *sysmem_texture = DX8Wrapper::_Create_DX8_Texture(
      thumb->Get_Width(), thumb->Get_Height(), dest_format, MIP_LEVELS_ALL,
  ...
  for (level = 0; level < sysmem_texture->GetLevelCount(); ++level) {
    DX8_ErrorCode(
        sysmem_texture->LockRect(level, &locked_rects[level], nullptr, 0));
  }
  ...
  for (level = 0; level < sysmem_texture->GetLevelCount() - 1; ++level) {
    BitmapHandlerClass::Copy_Image_Generate_Mipmap(
        width, height, (unsigned char *)locked_rects[level].pBits,
        locked_rects[level].Pitch, dest_format, src_surface, src_pitch,
        src_format,
        (unsigned char *)locked_rects[level + 1].pBits, // mipmap
        locked_rects[level + 1].Pitch, hsv);
    hsv = Vector3(0.0f, 0.0f, 0.0f); // Only do the shift for the first level,
                                     // as the mipmaps are based on it.
...
#else
  IDirect3DTexture8 *d3d_texture = DX8Wrapper::_Create_DX8_Texture(
      thumb->Get_Width(), thumb->Get_Height(), dest_format,
      TextureBaseClass::MIP_LEVELS_ALL, D3DPOOL_DEFAULT);
...
// targa format. If suitable format is not available, it selects closest
// matching format and performs color space conversion.
//
// ----------------------------------------------------------------------------
IDirect3DSurface8 *
TextureLoader::Load_Surface_Immediate(const StringClass &filename,
  ...
  if (compressed) {
    IDirect3DTexture8 *comp_tex =
        Load_Compressed_Texture(filename, 0, MIP_LEVELS_1, WW3D_FORMAT_UNKNOWN);
    if (comp_tex) {
      IDirect3DSurface8 *d3d_surface = nullptr;
  ...
  if (src_format == WW3D_FORMAT_A1R5G5B5 || src_format == WW3D_FORMAT_R5G6B5 ||
      src_format == WW3D_FORMAT_A4R4G4B4 || src_format == WW3D_FORMAT_P8 ||
      src_format == WW3D_FORMAT_L8 || src_width != width ||
      src_height != height) {
    ...
    BitmapHandlerClass::Copy_Image(converted_surface, width, height, width * 4,
                                   ...
                                   src_surface, src_width, src_height,
                                   src_width * src_bpp, src_format,
  ...
  IDirect3DSurface8 *d3d_surface =
      DX8Wrapper::_Create_DX8_Surface(width, height, dest_format);
  WWASSERT(d3d_surface);
  D3DLOCKED_RECT locked_rect;
  DX8_ErrorCode(d3d_surface->LockRect(&locked_rect, nullptr, 0));

  BitmapHandlerClass::Copy_Image(
      (unsigned char *)locked_rect.pBits, width, height, locked_rect.Pitch,
      dest_format, src_surface, src_width, src_height, src_pitch, src_format,
...
void TextureLoader::Request_Thumbnail(TextureBaseClass *tc) {
  ...
  if (Is_DX8_Thread()) {
    ...
    if (!task && (!load_task || load_task->Get_State() <
                                    TextureLoadTaskClass::STATE_LOAD_MIPMAP)) {

      // create a thumbnail load task and add to foreground queue.
      task =
          TextureLoadTaskClass::Create(tc, TextureLoadTaskClass::TASK_THUMBNAIL,
                                       TextureLoadTaskClass::PRIORITY_LOW);
...
void TextureLoader::Request_Background_Loading(TextureBaseClass *tc) {
  ...
  // threads.
  FastCriticalSectionClass::LockClass foreground_lock(
      _ForegroundCriticalSection);
  ...
  task = TextureLoadTaskClass::Create(tc, TextureLoadTaskClass::TASK_LOAD,
                                      TextureLoadTaskClass::PRIORITY_LOW);
...
void TextureLoader::Request_Foreground_Loading(TextureBaseClass *tc) {
  ...
  if (Is_DX8_Thread()) {
    ...
    if (task) {
      ...
      FastCriticalSectionClass::LockClass background_lock(
          _BackgroundCriticalSection);
      _ForegroundQueue.Remove(task);
      ...
      task = TextureLoadTaskClass::Create(tc, TextureLoadTaskClass::TASK_LOAD,
                                          TextureLoadTaskClass::PRIORITY_HIGH);
    }
    ...
    // leave it queued in the completed state so it will be destroyed by
    // Update().
    if (task_thumb) {
...
void TextureLoader::Update(void (*network_callback)()) {
  WWASSERT_PRINT(Is_DX8_Thread(),
                 "TextureLoader::Update must be called from the main thread!");
...
void TextureLoader::Suspend_Texture_Load() {
  WWASSERT_PRINT(Is_DX8_Thread(), "TextureLoader::Suspend_Texture_Load must be "
                                  "called from the main thread!");
  TextureLoadSuspended = true;
}

void TextureLoader::Continue_Texture_Load() {
  WWASSERT_PRINT(Is_DX8_Thread(), "TextureLoader::Continue_Texture_Load must "
...
void TextureLoader::Load_Thumbnail(TextureBaseClass *tc) {
  ...
  IDirect3DTexture8 *d3d_texture =
      Load_Thumbnail(tc->Get_Full_Path(), tc->Get_HSV_Shift());
...
void LoaderThreadClass::Thread_Function() {
  while (running) {
    // if there are no tasks on the background queue, no need to grab background
    // lock.
...
TextureLoadTaskClass::~TextureLoadTaskClass() { Deinit(); }
...
TextureLoadTaskClass *TextureLoadTaskClass::Create(TextureBaseClass *tc,
  ...
  case TextureBaseClass::TEX_REGULAR:
    task = _TexLoadFreeList.Pop_Front();
    break;
  case TextureBaseClass::TEX_CUBEMAP:
    task = _CubeTexLoadFreeList.Pop_Front();
    break;
  case TextureBaseClass::TEX_VOLUME:
    task = _VolTexLoadFreeList.Pop_Front();
    break;
  default:
    WWASSERT(0);
  };
  ...
  if (!task) {
    ...
    case TextureBaseClass::TEX_REGULAR:
      ...
      break;
    case TextureBaseClass::TEX_CUBEMAP:
      task = new CubeTextureLoadTaskClass;
      break;
    case TextureBaseClass::TEX_VOLUME:
      task = new VolumeTextureLoadTaskClass;
      break;
    default:
      WWASSERT(0);
    }
...
void TextureLoadTaskClass::Init(TextureBaseClass *tc, TaskType type,
                                PriorityType priority) {
  WWASSERT(tc);

  // NOTE: we must be in the main thread to avoid corrupting the texture's
  // refcount.
...
static bool Get_Texture_Information(const char *filename, unsigned &reduction,
  ...
  if (!thumb) {
    if (compressed) {
      ...
      while (curReduction < reqReduction && curWidth > minDim &&
             curHeight > minDim) {
    ...
    for (int i = targa.Header.Width, j = targa.Header.Height; i > 0 && j > 0;
         i >>= 1, j >>= 1)
      mip_count++;
  ...
  // d=thumb->Get_Original_Texture_Depth() >> reduction; // need to a volume
  // texture support to thumbnails...maybe
...
bool TextureLoadTaskClass::Begin_Compressed_Load() {
  ...
  if (!Get_Texture_Information(Texture->Get_Full_Path(), reduction, orig_w,
                               orig_h, orig_d, orig_format, orig_mip_count,
                               true)) {
    return false;
  }
  ...
  Height = height;
  Format =
      Get_Valid_Texture_Format(orig_format, Texture->Is_Compression_Allowed());
  Reduction = reduction;
  ...
  if (!Texture->Is_Reducible() || Texture->MipLevelCount == MIP_LEVELS_1)
    ...
    if (Texture->MipLevelCount != MIP_LEVELS_ALL &&
        (Texture->MipLevelCount - Reduction) < 1)
      Reduction = Texture->MipLevelCount - 1;

  // Another sanity check
  if (Reduction >= orig_mip_count)
    Reduction = 0; // should not be possible to get here, but check just in
                   // case.
  ...
  // If texture wants all mip levels, take as many as the file contains (not
  // necessarily all) Otherwise take as many mip levels as the texture wants,
  // not to exceed the count in file...
  if (!mip_level_count) {
    reducedWidth >>= Reduction;
    reducedHeight >>= Reduction;
    mip_level_count =
        orig_mip_count - Reduction; // dds_file.Get_Mip_Level_Count();
    if (mip_level_count < 1)
      mip_level_count = 1; // sanity check to make sure something gets loaded.
  } else {
    if (mip_level_count > orig_mip_count) { // dds_file.Get_Mip_Level_Count()) {
    ...
    if (Reduction) {
      reducedWidth >>= Reduction;
      reducedHeight >>= Reduction;
      mip_level_count -= Reduction; // reduced requested number by those
                                    // removed.
    }
  }

  // Once more, verify that the mip level count is correct (in case it was
  // changed here it might not match the size...well actually it doesn't have to
  // match but it can't be bigger than the size)
  ...
  D3DTexture = DX8Wrapper::_Create_DX8_Texture(
      reducedWidth, reducedHeight, Format, (MipCountType)mip_level_count,
#ifdef USE_MANAGED_TEXTURES
      D3DPOOL_MANAGED
#else
      D3DPOOL_SYSTEMMEM
...
bool TextureLoadTaskClass::Begin_Uncompressed_Load() {
  ...
  WW3DFormat orig_format;
  if (!Get_Texture_Information(Texture->Get_Full_Path(), reduction, width,
                               height, depth, orig_format, orig_mip_count,
                               false)) {
    return false;
  }
  ...
  dest_format = Get_Valid_Texture_Format(
      dest_format,
      false); // No compressed destination format if reading from targa...

  if (src_format != WW3D_FORMAT_A8R8G8B8 && src_format != WW3D_FORMAT_R8G8B8 &&
      src_format != WW3D_FORMAT_X8R8G8B8) {
    WWDEBUG_SAY(("Invalid TGA format used in %s - only 24 and 32 bit formats "
                 "should be used!",
  ...
  if (width != ow || height != oh) {
    WWDEBUG_SAY(("Invalid texture size, scaling required. Texture: %s, size: "
                 "%d x %d -> %d x %d",
  ...
  if (Reduction) { // we don't care about specific levels so reduce them if
                   // needed.
    reducedWidth >>= Reduction;
    reducedHeight >>= Reduction;
  ...
  D3DTexture = DX8Wrapper::_Create_DX8_Texture(
      reducedWidth, reducedHeight, Format, (MipCountType)reducedMipCount,
#ifdef USE_MANAGED_TEXTURES
      D3DPOOL_MANAGED
#else
      D3DPOOL_SYSTEMMEM
...
bool TextureLoadTaskClass::Begin_Compressed_Load()
{
        ...
        // Destination size will be the next power of two square from the larger
width and height... unsigned int width	= dds_file.Get_Width(0); unsigned int
height	= dds_file.Get_Height(0); TextureLoader::Validate_Texture_Size(width,
height);

        // If the size doesn't match, try and see if texture reduction would
help... (mainly for
        // cases where loaded texture is larger than hardware limit)
        if (width != dds_file.Get_Width(0) || height != dds_file.Get_Height(0))
{ for (unsigned int i = 1; i < dds_file.Get_Mip_Level_Count(); ++i) { unsigned
int w = dds_file.Get_Width(i); unsigned int h = dds_file.Get_Height(i);
                        TextureLoader::Validate_Texture_Size(w,h);

                        if (w == dds_file.Get_Width(i) && h ==
dds_file.Get_Height(i)) { Reduction	+= i; width			=
w; height		=	h; break;
                        }
                }
        }
        ...
        Width		= width;
        Height	= height;
        Format	= Get_Valid_Texture_Format(dds_file.Get_Format(),
Texture->Is_Compression_Allowed());

        unsigned int mip_level_count = Get_Mip_Level_Count();

        // If texture wants all mip levels, take as many as the file contains
(not necessarily all)
        // Otherwise take as many mip levels as the texture wants, not to exceed
the count in file... if (!mip_level_count) { mip_level_count =
dds_file.Get_Mip_Level_Count(); } else if (mip_level_count >
dds_file.Get_Mip_Level_Count()) { mip_level_count =
dds_file.Get_Mip_Level_Count();
        }

        // Once more, verify that the mip level count is correct (in case it was
changed here it might not
        // match the size...well actually it doesn't have to match but it can't
be bigger than the size) unsigned int max_mip_level_count = 1; unsigned int w =
4; unsigned int h = 4;
...
{
        Targa targa;
        if (TARGA_ERROR_HANDLER(targa.Open(Texture->Get_Full_Path(),
TGA_READMODE), Texture->Get_Full_Path())) { return false;
        }
        ...
        if (	src_format != WW3D_FORMAT_A8R8G8B8
                ...
                &&	src_format != WW3D_FORMAT_X8R8G8B8) {
                WWDEBUG_SAY(("Invalid TGA format used in %s - only 24 and 32 bit
formats should be used!", Texture->Get_Full_Path()));
        }

        // Destination size will be the next power of two square from the larger
width and height... unsigned width=targa.Header.Width,
height=targa.Header.Height; int ReductionFactor=Get_Reduction(); int
MipLevels=0;
        ...
        //Adjust the reduction factor to keep textures above some minimum
dimensions if (MipLevels <= WW3D::Get_Texture_Min_Mip_Levels())
                ReductionFactor=0;
        else
        ...
        if (width != ow || height != oh) {
                WWDEBUG_SAY(("Invalid texture size, scaling required. Texture:
%s, size: %d x %d -> %d x %d", Texture->Get_Full_Path(), ow, oh, width,
height));
        }
        ...
        // changed because format was being read from previous loading task?!
KJM Format=dest_format;
        //if (Format == WW3D_FORMAT_UNKNOWN) {
...
void TextureLoadTaskClass::Lock_Surfaces() {
  ...
  for (unsigned int i = 0; i < MipLevelCount; ++i) {
    D3DLOCKED_RECT locked_rect;
    DX8_ErrorCode(Peek_D3D_Texture()->LockRect(i, &locked_rect, nullptr, 0));
...
void TextureLoadTaskClass::Unlock_Surfaces() {
  for (unsigned int i = 0; i < MipLevelCount; ++i) {
    if (LockedSurfacePtr[i]) {
      WWASSERT(ThreadClass::_Get_Current_Thread_ID() ==
               DX8Wrapper::_Get_Main_Thread_ID());
  ...
  IDirect3DTexture8 *tex = DX8Wrapper::_Create_DX8_Texture(
      Width, Height, Format, Texture->MipLevelCount, D3DPOOL_DEFAULT);
...
bool TextureLoadTaskClass::Load_Compressed_Mipmap() {
  ...
  for (unsigned int level = 0; level < Get_Mip_Level_Count(); ++level) {
    ...
    dds_file.Copy_Level_To_Surface(level, Get_Format(), width, height,
                                   Get_Locked_Surface_Ptr(level),
                                   Get_Locked_Surface_Pitch(level), HSVShift);
...
bool TextureLoadTaskClass::Load_Uncompressed_Mipmap() {
  ...
  Targa targa;
  if (TARGA_ERROR_HANDLER(targa.Open(Texture->Get_Full_Path(), TGA_READMODE),
  ...
  dest_format = Get_Format(); // Texture can be requested in different format
                              // than the most obvious from the TGA
  ...
  // NOTE: We load the palette but we do not yet support paletted textures!
  if (TARGA_ERROR_HANDLER(
          targa.Load(Texture->Get_Full_Path(), TGAF_IMAGE, false),
          Texture->Get_Full_Path())) {
    return false;
  }
  ...
  if (src_format == WW3D_FORMAT_A1R5G5B5 || src_format == WW3D_FORMAT_R5G6B5 ||
    ...
    BitmapHandlerClass::Copy_Image(
        converted_surface, width, height, width * 4,
        WW3D_FORMAT_A8R8G8B8, // dest_format,
  ...
  if (Reduction) { // texture needs to be reduced so allocate storage for
                   // full-sized version.
    ...
    for (unsigned int level = 0; level < Reduction; ++level) {
      BitmapHandlerClass::Copy_Image(
          (unsigned char *)destination_surface, width, height, src_pitch,
          Get_Format(), src_surface, src_width, src_height, src_pitch,
          src_format, nullptr, 0, true, hsv_shift);
  ...
  for (unsigned int level = 0; level < Get_Mip_Level_Count(); ++level) {
    ...
    BitmapHandlerClass::Copy_Image(
        Get_Locked_Surface_Ptr(level), width, height,
        Get_Locked_Surface_Pitch(level), Get_Format(), src_surface, src_width,
        src_height, src_pitch, src_format, nullptr, 0, true, hsv_shift);
    hsv_shift = Vector3(0.0f, 0.0f, 0.0f);
...
unsigned char *
TextureLoadTaskClass::Get_Locked_Surface_Ptr(unsigned int level) {
...
unsigned int
TextureLoadTaskClass::Get_Locked_Surface_Pitch(unsigned int level) const {
  WWASSERT(level < MipLevelCount);
  WWASSERT(LockedSurfacePtr[level]);
...
CubeTextureLoadTaskClass::CubeTextureLoadTaskClass() : TextureLoadTaskClass() {
  // because texture load tasks are pooled, the constructor and destructor
  // don't need to do much. The work of attaching a task to a texture is
  // is done by Init() and Deinit().
...
void CubeTextureLoadTaskClass::Init(TextureBaseClass *tc, TaskType type,
                                    PriorityType priority) {
  WWASSERT(tc);
...
void CubeTextureLoadTaskClass::Lock_Surfaces() {
  for (unsigned int f = 0; f < 6; f++) {
    for (unsigned int i = 0; i < MipLevelCount; i++) {
      D3DLOCKED_RECT locked_rect;
      DX8_ErrorCode(Peek_D3D_Cube_Texture()->LockRect(
...
bool CubeTextureLoadTaskClass::Begin_Compressed_Load() {
  ...
  if (!mip_level_count) {
    mip_level_count = orig_mip_count; // dds_file.Get_Mip_Level_Count();
  } else if (mip_level_count >
             orig_mip_count) {        // dds_file.Get_Mip_Level_Count()) {
    mip_level_count = orig_mip_count; // dds_file.Get_Mip_Level_Count();
  }
  ...
  D3DTexture = DX8Wrapper::_Create_DX8_Cube_Texture(
      Width, Height, Format, (MipCountType)mip_level_count,
#ifdef USE_MANAGED_TEXTURES
      D3DPOOL_MANAGED
#else
      D3DPOOL_SYSTEMMEM
...
bool CubeTextureLoadTaskClass::Begin_Uncompressed_Load() {
  ...
  D3DTexture = DX8Wrapper::_Create_DX8_Cube_Texture(Width, Height, Format,
                                                    Texture->MipLevelCount,
#ifdef USE_MANAGED_TEXTURES
                                                    D3DPOOL_MANAGED
#else
  ...
  for (unsigned int face = 0; face < 6; face++) {
    ...
    for (unsigned int level = 0; level < Get_Mip_Level_Count(); level++) {
      ...
      dds_file.Copy_CubeMap_Level_To_Surface(
          face, level, Get_Format(), width, height,
          Get_Locked_CubeMap_Surface_Pointer(face, level),
          Get_Locked_CubeMap_Surface_Pitch(face, level), HSVShift);
...
unsigned char *CubeTextureLoadTaskClass::Get_Locked_CubeMap_Surface_Pointer(
...
unsigned int CubeTextureLoadTaskClass::Get_Locked_CubeMap_Surface_Pitch(
    unsigned int face, unsigned int level) const {
  WWASSERT(face < 6 && level < MipLevelCount);
...
void VolumeTextureLoadTaskClass::Init(TextureBaseClass *tc, TaskType type,
                                      PriorityType priority) {
  WWASSERT(tc);
...
void VolumeTextureLoadTaskClass::Lock_Surfaces() {
  for (unsigned int i = 0; i < MipLevelCount; i++) {
    ...
    DX8_ErrorCode(
        Peek_D3D_Volume_Texture()->LockBox(i, &locked_box, nullptr, 0));
...
void VolumeTextureLoadTaskClass::Unlock_Surfaces() {
  for (unsigned int i = 0; i < MipLevelCount; ++i) {
    if (LockedSurfacePtr[i]) {
      WWASSERT(ThreadClass::_Get_Current_Thread_ID() ==
               DX8Wrapper::_Get_Main_Thread_ID());
      DX8_ErrorCode(Peek_D3D_Volume_Texture()->UnlockBox(i));
    }
    LockedSurfacePtr[i] = nullptr;
  }

#ifndef USE_MANAGED_TEXTURES
  IDirect3DTexture8 *tex = DX8Wrapper::_Create_DX8_Volume_Texture(
      Width, Height, Depth, Format, Texture->MipLevelCount, D3DPOOL_DEFAULT);
  DX8CALL(UpdateTexture(Peek_D3D_Volume_Texture(), tex));
  Peek_D3D_Volume_Texture()->Release();
  D3DTexture = tex;
  WWDEBUG_SAY(("Created non-managed texture (%s)", Texture->Get_Full_Path()));
...
bool VolumeTextureLoadTaskClass::Begin_Compressed_Load() {
  ...
  D3DTexture = DX8Wrapper::_Create_DX8_Volume_Texture(
      Width, Height, Depth, Format, (MipCountType)mip_level_count,
#ifdef USE_MANAGED_TEXTURES
      D3DPOOL_MANAGED
#else
      D3DPOOL_SYSTEMMEM
...
bool VolumeTextureLoadTaskClass::Begin_Uncompressed_Load() {
  ...
  D3DTexture = DX8Wrapper::_Create_DX8_Volume_Texture(
      Width, Height, Depth, Format, Texture->MipLevelCount,
#ifdef USE_MANAGED_TEXTURES
      D3DPOOL_MANAGED
#else
      D3DPOOL_SYSTEMMEM
...
bool VolumeTextureLoadTaskClass::Load_Compressed_Mipmap() {
  ...
  for (unsigned int level = 0; level < Get_Mip_Level_Count(); level++) {
    ...
    dds_file.Copy_Volume_Level_To_Surface(
        level, depth, Get_Format(), width, height,
        Get_Locked_Volume_Pointer(level), Get_Locked_Volume_Row_Pitch(level),
        Get_Locked_Volume_Slice_Pitch(level), HSVShift);
...
unsigned char *
VolumeTextureLoadTaskClass::Get_Locked_Volume_Pointer(unsigned int level) {
  WWASSERT(level < MipLevelCount);
  WWASSERT(LockedSurfacePtr[level]);
  return LockedSurfacePtr[level];
}
...
unsigned int
VolumeTextureLoadTaskClass::Get_Locked_Volume_Row_Pitch(unsigned int level) {
  WWASSERT(level < MipLevelCount);
  WWASSERT(LockedSurfacePtr[level]);
  return LockedSurfacePitch[level];
}
...
unsigned int
VolumeTextureLoadTaskClass::Get_Locked_Volume_Slice_Pitch(unsigned int level) {
  WWASSERT(level < MipLevelCount);
  WWASSERT(LockedSurfacePtr[level]);
