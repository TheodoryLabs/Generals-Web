...
// -----------------------------------------------------
/**
        UnicodeString is the fundamental double-byte string type used in the
   Generals code base, and should be preferred over all other string
   constructions (e.g., array of WideChar, STL string<>, WWVegas StringClass,
   etc.)

        Of course, other string setups may be used when necessary or
   appropriate!
        ...
        This is done fairly efficiently, so that no new memory allocation is
   done unless the string is actually modified.
        ...
        UnicodeStrings are suitable for use as automatic, member, or static
   variables.
*/
...
protected:
  ...
  WideChar *peek() const;
  void releaseBuffer();
  void ensureUniqueBufferOfSize(int numCharsNeeded, Bool preserveData,
...
public:
  ...
  enum {
    MAX_FORMAT_BUF_LEN =
        2048,       ///< max total len of string created by format/format_va
  ...
  */
  ...
  /**
          Return the length, in characters, of the string up to the first zero
     or null terminator.
  */
  int getLength() const;

  /**
          Return the number of bytes used by the string up to the first zero or
     null terminator.
  */
  ...
  */
  Bool isEmpty() const;
  /**
          Make the string empty. Equivalent to (str = "") but slightly more
     efficient.
  */
  void clear();
  ...
  */
  ...
  /**
          Remove the final charCount characters in the string. If the string is
     empty, do nothing.
  */
  void truncateBy(const Int charCount);

  /**
          Truncate the string to a length of maxLength characters, not including
     null termination, by removing from the end. If the string is empty or
     shorter than maxLength, do nothing.
  */
  ...
  */
  ...
  Bool startsWith(const UnicodeString &stringSrc) const {
    return startsWith(stringSrc.str());
  }
  ...
  */
  ...
  Bool startsWithNoCase(const UnicodeString &stringSrc) const {
    return startsWithNoCase(stringSrc.str());
  }
  ...
  */
  ...
  Bool endsWith(const UnicodeString &stringSrc) const {
    return endsWith(stringSrc.str());
  }
  ...
  */
  ...
  Bool endsWithNoCase(const UnicodeString &stringSrc) const {
    return endsWithNoCase(stringSrc.str());
  }
  ...
  /**
          ...
          the token off!)
  */
  Bool nextToken(UnicodeString *token,
  ...
  UnicodeString &
  operator=(const UnicodeString &stringSrc);   ///< the same as set()
...
// -----------------------------------------------------
inline UnicodeString::UnicodeString() : m_data(0) { validate(); }
...
inline WideChar UnicodeString::getCharAt(int index) const {
  DEBUG_ASSERTCRASH(index >= 0 && index < getLength(),
                    ("bad index in getCharAt"));
  validate();
