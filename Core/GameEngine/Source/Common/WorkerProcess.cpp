...
#if defined(_WIN32)
// We need Job-related functions, but these aren't defined in the
// Windows-headers that VC6 uses. So we define them here and load them
// dynamically.
...
typedef BOOL(WINAPI *PFN_SetInformationJobObject)(HANDLE, JOBOBJECTINFOCLASS,
                                                  LPVOID, DWORD);
typedef BOOL(WINAPI *PFN_AssignProcessToJobObject)(HANDLE, HANDLE);

static PFN_CreateJobObjectW CreateJobObjectW =
    (PFN_CreateJobObjectW)GetProcAddress(GetModuleHandleW(L"kernel32.dll"),
                                         "CreateJobObjectW");
static PFN_SetInformationJobObject SetInformationJobObject =
    (PFN_SetInformationJobObject)GetProcAddress(
        GetModuleHandleW(L"kernel32.dll"), "SetInformationJobObject");
static PFN_AssignProcessToJobObject AssignProcessToJobObject =
    (PFN_AssignProcessToJobObject)GetProcAddress(
        GetModuleHandleW(L"kernel32.dll"), "AssignProcessToJobObject");
#endif
...
bool WorkerProcess::startProcess(UnicodeString command) {
  ...
  if (!CreateProcessW(nullptr, (LPWSTR)command.str(), nullptr, nullptr,
  ...
  // We want to make sure that when our process is killed, our workers
  // automatically terminate as well. In Windows, the way to do this is to
  // attach the worker to a job we own.
  m_jobHandle = CreateJobObjectW != nullptr ? CreateJobObjectW(nullptr, nullptr)
                                            : nullptr;
  if (m_jobHandle != nullptr) {
    ...
    SetInformationJobObject(
        m_jobHandle, (JOBOBJECTINFOCLASS)JobObjectExtendedLimitInformation,
...
bool WorkerProcess::isRunning() const { return m_processHandle != nullptr; }
...
bool WorkerProcess::fetchStdOutput() {
  while (true) {
    ...
    BOOL success = PeekNamedPipe(m_readHandle, nullptr, 0, nullptr,
    ...
    DWORD readBytes = 0;
    char buffer[1024];
    success = ReadFile(m_readHandle, buffer, ARRAY_SIZE(buffer) - 1, &readBytes,
                       nullptr);
    if (!success)
      return true;
...
void WorkerProcess::update() {
  ...
  // Pipe broke, that means the process already exited. But we call this just to
  // make sure
