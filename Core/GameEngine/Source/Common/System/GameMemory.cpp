...
#include "PreRTS.h"
#include <stdio.h>
 // This must go first in EVERY cpp file in the GameEngine
...
/**
        define MPSB_DLINK to add a backlink to MemoryPoolSingleBlock; this makes
   it faster to free raw DMA blocks.
...
/**
        if you define MEMORYPOOL_INTENSE_VERIFY, we do intensive verifications
   after nearly every memory operation. this is OFF by default, since it slows
   down things a lot, but is worth turning on for really obscure memory
   corruption issues.
*/
...
/**
        if you define MEMORYPOOL_CHECK_BLOCK_OWNERSHIP, we do lots of calls to
   verify that a block actually belongs to the pool it is called with. this is
   great for debugging, but can be realllly slow, so is off by default.
*/
...
enum { IGNORE_LEAKS = 0x0001 };

// in debug mode (but not internal), save stacktraces too
#if !defined(MEMORYPOOL_CHECKPOINTING) && defined(MEMORYPOOL_STACKTRACE) &&    \
    defined(RTS_DEBUG)
...
static UnsignedInt s_initFillerValue =
    0xf00dcafe; // will be replaced, should never be this value at runtime
static void calcFillerValue(Int index) {
  ...
  // DEBUG_LOG(("Setting MemoryPool initFillerValue to %08x (index
  // %d)",s_initFillerValue,index));
}
#endif
...
#define WALLCOUNT                                                              \
  (2) // default setting of 8 requires 4*4*2==32 extra bytes PER BLOCK
...
#define MEMORYPOOL_STACKTRACE_SIZE_BYTES                                       \
  (MEMORYPOOL_STACKTRACE_SIZE * sizeof(void *))
...
/** the max number levels to dump in the stacktrace. a variable rather than
        ...
        get shorter or longer dumps. (you can't go longer than
   MEMORYPOOL_STACKTRACE_SIZE in any event. */
...
//-----------------------------------------------------------------------------
/**
        this is the low-level allocator that we use to request memory from the
   OS. all (repeat, all) memory allocations in this module should ultimately go
   thru this routine (or sysAllocate).
...
//-----------------------------------------------------------------------------
/**
        the counterpart to sysAllocate / sysAllocateDoNotZero; used to free
   blocks allocated by them. it is OK to pass null here (it will just be
   ignored).
*/
static void sysFree(void *p) {
  if (p) {
...
// ----------------------------------------------------------------------------
/**
        fills memory with a 32-bit value (note: assumes the ptr is
   4-byte-aligned)
*/
static void memset32(void *ptr, Int value, Int bytesToFill) {
...
// ----------------------------------------------------------------------------
/**
        This is just a convenience routine that dumps output from the StackDump
   module to our normal debug log file, with a little massaging for formatting.
*/
static void doStackDumpOutput(const char *m) {
  ...
  if (*m == 0) {
    ...
    // Note - I am moving the prepend to the end, as this allows double clicking
    // in the output window to open the file in VisualStudio.  jba.
...
/**
        This is a auxiliary record that we allocate in debug modes (actually,
   checkpoint modes) to retain extra information about blocks; there is a
   one-to-one correspondence between this record and a block allocation. The
   interesting bit about this record is that we don't deallocate it when the
   corresponding block is freed; we retain it so we can later provide
   information about what blocks were freed when, etc. Yes, this does chew up a
   lot of memory! That's why it's debug-mode only; the presumption is that
   developers machines have boatloads of RAM. (Note that we *do* free these when
   resetCheckpoints() is called.) Note also that we directly allocate/free these
   with sysAllocate/sysFree, so ctors/dtors are never executed, nor would
   virtual functions work -- I know, it's a little evil.
*/
...
#ifdef MEMORYPOOL_STACKTRACE
  void *m_stacktrace[MEMORYPOOL_STACKTRACE_SIZE]; ///< stacktrace of when block
                                                  ///< was allocated
#endif
...
public:
  BlockCheckpointInfo *
  getNext(); ///< return next checkpointinfo for this pool/dma

  void debugSetFreepoint(
      Int f); ///< set the checkpoint at which the block was freed.

#ifdef MEMORYPOOL_STACKTRACE
  void **
  getStacktraceInfo(); ///< return a ptr to the allocation stacktrace info.
#endif

  static BlockCheckpointInfo *addToList(BlockCheckpointInfo **pHead,
                                        const char *debugLiteralTagString,
                                        Int allocCheckpoint, Int blockSize);

  static void freeList(BlockCheckpointInfo **pHead);

  Bool shouldBeInReport(Int flags, Int startCheckpoint, Int endCheckpoint);

  static void doBlockCheckpointReport(BlockCheckpointInfo *bi,
                                      const char *poolName, Int flags,
...
// ----------------------------------------------------------------------------
/**
        This is the fundamental allocation unit; when you allocate via (say)
   MemoryPool::allocateBlock, this is what is being allocated for you. (Of
   course, you don't see the private fields.) For the most part, we allocate big
   chunks of these in a monolithic Blob and subdivide from there. (However, we
   occasionally allocate these individually, for large blocks.)

        Note also that we directly allocate/free these with sysAllocate/sysFree,
...
private:
  ...
  MemoryPoolSingleBlock
      *m_nextBlock; ///< if m_owningBlob is nonnull, this points to next free
                    ///< (unallocated) block in the blob; if m_owningBlob is
                    ///< null, this points to the next used (allocated) raw
                    ///< block in the pool.
#ifdef MPSB_DLINK
  MemoryPoolSingleBlock
      *m_prevBlock; ///< if m_owningBlob is nonnull, this points to prev free
                    ///< (unallocated) block in the blob; if m_owningBlob is
                    ///< null, this points to the prev used (allocated) raw
                    ///< block in the pool.
#endif
#ifdef MEMORYPOOL_CHECKPOINTING
  BlockCheckpointInfo
      *m_checkpointInfo; ///< ptr to the checkpointinfo for this block
#endif
#ifdef MEMORYPOOL_BOUNDINGWALL
  Int m_wallPattern; ///< unique seed value for the bounding-walls for this
                     ///< block
#endif
#ifdef MEMORYPOOL_DEBUG
  const char *m_debugLiteralTagString; ///< ptr to the tagstring for this block.
  Int m_logicalSize; ///< logical size of block (not including overhead, walls,
                     ///< etc.)
  Int m_wastedSize;  ///< if allocated via DMA, the "wasted" bytes
  Short m_magicCookie; ///< magic value used to verify that the block is one of
                       ///< ours (as opposed to random pointer)
  Short m_debugFlags;  ///< misc flags
#ifdef MEMORYPOOL_SINGLEBLOCK_GETS_STACKTRACE
  void *m_stacktrace[MEMORYPOOL_STACKTRACE_SIZE]; ///< stacktrace of when block
                                                  ///< was allocated (if not
                                                  ///< checkpointing)
#endif
#endif
...
private:
  ...
  MemoryPoolSingleBlock
      *m_firstFreeBlock;   ///< ptr to first available block in this blob
  Int m_usedBlocksInBlob;  ///< total allocated blocks in this blob
  Int m_totalBlocksInBlob; ///< total blocks in this blob (allocated +
                           ///< available)
...
// ----------------------------------------------------------------------------
/**
        return a ptr to the user-data area of the block (ie, the part the
   enduser can deal with). this call does NO debug verification and is for
   internal use of class MemoryPoolSingleBlock only.
*/
...
/**
        return a ptr to the user-data area of the block (ie, the part the
   enduser can deal with). this call verifies that the block is valid in debug
   mode.
*/
inline void *MemoryPoolSingleBlock::getUserData() {
// yes, verify the block in this case for plain debug mode (not intense-verify
// mode)
#ifdef MEMORYPOOL_DEBUG
  debugVerifyBlock();
#endif
...
/**
        given a desired logical block size, calculate the physical size needed
   for each MemoryPoolSingleBlock (including overhead, etc.)
*/
...
/**
        return the next free block in this blob. this call assumes that the
   block in question belongs to a blob, and will assert if not.
*/
...
/**
        ...
        in question belongs to a blob, but will NOT assert if not, since it may
   be called when the blocks are in an inconsistent state.
*/
inline void MemoryPoolSingleBlock::setNextFreeBlock(MemoryPoolSingleBlock *b) {
  // DEBUG_ASSERTCRASH(m_owningBlob != nullptr && b->m_owningBlob != nullptr,
  // ("must be called on blob block"));
  //  don't check the 'b' block -- we need to call this before 'b' is fully
  //  initialized.
  DEBUG_ASSERTCRASH(m_owningBlob != nullptr, ("must be called on blob block"));
  this->m_nextBlock = b;
#ifdef MPSB_DLINK
...
inline void MemoryPoolSingleBlock::setNextRawBlock(MemoryPoolSingleBlock *b) {
  DEBUG_ASSERTCRASH(m_owningBlob == nullptr &&
                        (!b || b->m_owningBlob == nullptr),
                    ("must be called on raw block"));
  m_nextBlock = b;
#ifdef MPSB_DLINK
  if (b)
    b->m_prevBlock = this;
...
*/
inline void
MemoryPoolSingleBlock::debugSetCheckpointInfo(BlockCheckpointInfo *bi) {
...
#ifdef MEMORYPOOL_CHECKPOINTING
/**
        sets the checkpointinfo to null, but does NOT free it... the
   checkpointinfo is expected to be freed elsewhere.
*/
...
//-----------------------------------------------------------------------------
#ifdef MEMORYPOOL_CHECKPOINTING
/// return true iff this checkpointinfo should be included in a checkpointreport
/// with the given parameters.
Bool BlockCheckpointInfo::shouldBeInReport(Int flags, Int startCheckpoint,
                                           Int endCheckpoint) {
  Bool allocFlagsOK = false;
  Bool freedFlagsOK = false;

  if (m_allocCheckpoint < startCheckpoint &&
      (flags & _REPORT_CP_ALLOCATED_BEFORE))
    allocFlagsOK = true;
  if (m_allocCheckpoint >= startCheckpoint &&
      m_allocCheckpoint < endCheckpoint &&
      (flags & _REPORT_CP_ALLOCATED_BETWEEN))
    allocFlagsOK = true;
  ...
  if (m_freeCheckpoint == -1) {
    ...
    if (m_freeCheckpoint < startCheckpoint && (flags & _REPORT_CP_FREED_BEFORE))
      freedFlagsOK = true;
    if (m_freeCheckpoint >= startCheckpoint &&
        m_freeCheckpoint < endCheckpoint && (flags & _REPORT_CP_FREED_BETWEEN))
      freedFlagsOK = true;
  }
...
//-----------------------------------------------------------------------------
#ifdef MEMORYPOOL_CHECKPOINTING
/// print a checkpointreport for the given checkpointinfo. if checkpointinfo is
/// null, print column headers.
/*static*/ void BlockCheckpointInfo::doBlockCheckpointReport(
    BlockCheckpointInfo *bi, const char *poolName, Int flags,
    Int startCheckpoint, Int endCheckpoint) {
  const char *PREPEND = "BLOCKINFO"; // allows grepping more easily

  if (!bi) {
    DEBUG_LOG(("%s,%32s,%6s,%6s,%6s,%s", PREPEND, "POOLNAME", "BLKSZ", "ALLOC",
               "FREED", "BLOCKNAME"));
  } else {
    DEBUG_ASSERTCRASH(startCheckpoint >= 0 && startCheckpoint <= endCheckpoint,
                      ("bad checkpoints"));
    DEBUG_ASSERTCRASH((flags & _REPORT_CP_ALLOCATED_DONTCARE) != 0,
                      ("bad flags: must set at least one alloc flag"));
    DEBUG_ASSERTCRASH((flags & _REPORT_CP_FREED_DONTCARE) != 0,
                      ("bad flags: must set at least one freed flag"));

    if (bi->shouldBeInReport(flags, startCheckpoint, endCheckpoint)) {
      DEBUG_LOG(("%s,%32s,%6d,%6d,%6d,%s", PREPEND, poolName, bi->m_blockSize,
                 bi->m_allocCheckpoint, bi->m_freeCheckpoint,
                 bi->m_debugLiteralTagString));
#ifdef MEMORYPOOL_STACKTRACE
      if (flags & REPORT_CP_STACKTRACE) {
        ::doStackDump(bi->m_stacktrace,
...
#ifdef MEMORYPOOL_CHECKPOINTING
/**
        allocate a new checkpointinfo with the given tag/checkpoint/size, add it
   to the linked list, and return the checkpointinfo. (note that this will NOT
   throw an exception; if there is not enough memory to allocate a new
   checkpointinfo, it will quietly return null.)
*/
/*static*/ BlockCheckpointInfo *
BlockCheckpointInfo::addToList(BlockCheckpointInfo **pHead,
                               const char *debugLiteralTagString,
                               Int allocCheckpoint, Int blockSize) {
  DEBUG_ASSERTCRASH(debugLiteralTagString != FREE_SINGLEBLOCK_TAG_STRING,
                    ("bad tag string"));

  BlockCheckpointInfo *freed = nullptr;
  try {
    freed = (BlockCheckpointInfo *)::sysAllocateDoNotZero(
        sizeof(BlockCheckpointInfo));
  } catch (...) {
    freed = nullptr;
  }
  if (freed) {
    DEBUG_ASSERTCRASH(debugLiteralTagString != nullptr,
                      ("null tagstrings are not allowed"));
...
//-----------------------------------------------------------------------------
/**
        fill in a block's fields. this is usually done only just after a block
   is allocated, but might also be done at other points in debug mode.
*/
void MemoryPoolSingleBlock::initBlock(
    Int logicalSize, MemoryPoolBlob *owningBlob,
    MemoryPoolFactory *owningFactory DECLARE_LITERALSTRING_ARG2) {
...
#ifdef MEMORYPOOL_SINGLEBLOCK_GETS_STACKTRACE
    if (theStackTraceDepth > 0 &&
        (!TheGlobalData || TheGlobalData->m_checkForLeaks)) {
      memset(m_stacktrace, 0, MEMORYPOOL_STACKTRACE_SIZE_BYTES);
      ::FillStackAddresses(m_stacktrace,
                           min(MEMORYPOOL_STACKTRACE_SIZE, theStackTraceDepth),
...
//-----------------------------------------------------------------------------
/**
        given a 'public' ptr to a block (ie, the ptr returned by the
   MemoryPool::allocateBlock), recover the ptr to the MemoryPoolSingleBlock, so
   we can access the hidden fields.
*/
/* static */ MemoryPoolSingleBlock *
...
//-----------------------------------------------------------------------------
/**
        allocate and initialize a single block. this should only used by
   DynamicMemoryAllocator when allocating an extraordinarily large block.
*/
/*static*/ MemoryPoolSingleBlock *MemoryPoolSingleBlock::rawAllocateSingleBlock(
    MemoryPoolSingleBlock **pRawListHead, Int logicalSize,
    MemoryPoolFactory *owningFactory DECLARE_LITERALSTRING_ARG2) {
  MemoryPoolSingleBlock *block =
      (MemoryPoolSingleBlock *)::sysAllocateDoNotZero(
          calcRawBlockSize(logicalSize));
...
//-----------------------------------------------------------------------------
/**
        remove the block from the list, which is presumed to be a list of raw
   blocks. generally, only the DynamicMemoryAllocator should call this.
*/
void MemoryPoolSingleBlock::removeBlockFromList(MemoryPoolSingleBlock **pHead) {
  DEBUG_ASSERTCRASH(this->m_owningBlob == nullptr,
                    ("this function should only be used on raw blocks"));
#ifdef MPSB_DLINK
  DEBUG_ASSERTCRASH(this->m_nextBlock == nullptr ||
                        this->m_nextBlock->m_owningBlob == nullptr,
                    ("this function should only be used on raw blocks"));
  if (this->m_prevBlock) {
    DEBUG_ASSERTCRASH(this->m_prevBlock->m_owningBlob == nullptr,
                      ("this function should only be used on raw blocks"));
  ...
  if (this->m_nextBlock) {
    DEBUG_ASSERTCRASH(this->m_nextBlock->m_owningBlob == nullptr,
                      ("this function should only be used on raw blocks"));
  ...
  for (MemoryPoolSingleBlock *cur = *pHead; cur; cur = cur->m_nextBlock) {
    DEBUG_ASSERTCRASH(cur->m_owningBlob == nullptr,
                      ("this function should only be used on raw blocks"));
    if (cur == this) {
      if (prev) {
...
Int MemoryPoolSingleBlock::debugSingleBlockReportLeak(const char *owner) {
  ...
  if (strcmp(m_debugLiteralTagString,
             "STR_AsciiString::ensureUniqueBufferOfSize") == 0) {
    /** @todo srj -- we leak a bunch of these for some reason (probably due to
       leaking Win32LocalFile) so just ignore 'em for now... figure out later.
     */
  } else if (strstr(m_debugLiteralTagString, "Win32LocalFileSystem.cpp") !=
             nullptr) {
...
void MemoryPoolSingleBlock::debugVerifyBlock() {
  ...
  DEBUG_ASSERTCRASH(m_magicCookie == SINGLEBLOCK_MAGIC_COOKIE,
                    ("wrong cookie"));
  DEBUG_ASSERTCRASH(m_debugLiteralTagString != nullptr, ("bad tagstring"));
  /// @todo Put this check back in after the AI memory usage is under control
  /// (MSB)
  // DEBUG_ASSERTCRASH(m_logicalSize>0 && m_logicalSize < 0x00ffffff, ("unlikely
  // value for m_logicalSize"));
  DEBUG_ASSERTCRASH(m_nextBlock == nullptr ||
                        memcmp(&m_nextBlock->m_owningBlob, &s_initFillerValue,
                               sizeof(s_initFillerValue)) == 0 ||
                        m_nextBlock->m_owningBlob == m_owningBlob,
                    ("owning blob mismatch..."));
#ifdef MPSB_DLINK
  DEBUG_ASSERTCRASH(m_prevBlock == nullptr ||
                        memcmp(&m_prevBlock->m_owningBlob, &s_initFillerValue,
                               sizeof(s_initFillerValue)) == 0 ||
...
#ifdef MEMORYPOOL_DEBUG
/**
        Fill block with bogus values and mark other internal fields for
   debugging purposes.
*/
...
MemoryPoolBlob::MemoryPoolBlob()
    : m_owningPool(nullptr), m_nextBlob(nullptr), m_prevBlob(nullptr),
      m_firstFreeBlock(nullptr), m_usedBlocksInBlob(0), m_totalBlocksInBlob(0),
      m_blockData(nullptr) {}
...
*/
MemoryPoolBlob::~MemoryPoolBlob() { ::sysFree((void *)m_blockData); }

//-----------------------------------------------------------------------------
/**
        initialize a Blob; this is called just after the blob is allocated.
        allocate space for the blocks in this blob and initialize all those
   blocks.
*/
void MemoryPoolBlob::initBlob(MemoryPool *owningPool, Int allocationCount) {
  ...
  Int rawBlockSize = MemoryPoolSingleBlock::calcRawBlockSize(
      m_owningPool->getAllocationSize());
  m_blockData = (char *)::sysAllocateDoNotZero(
      rawBlockSize * m_totalBlocksInBlob); // throws on failure
...
#ifdef MEMORYPOOL_DEBUG
    block->initBlock(m_owningPool->getAllocationSize(), this,
                     owningPool->getOwningFactory(),
                     FREE_SINGLEBLOCK_TAG_STRING);
#else
    block->initBlock(m_owningPool->getAllocationSize(), this,
                     owningPool->getOwningFactory());
#endif
...
*/
MemoryPoolSingleBlock *
MemoryPoolBlob::allocateSingleBlock(DECLARE_LITERALSTRING_ARG1) {
  DEBUG_ASSERTCRASH(m_firstFreeBlock,
                    ("no free blocks available in MemoryPoolBlob"));

  MemoryPoolSingleBlock *block = m_firstFreeBlock;
  m_firstFreeBlock = block->getNextFreeBlock();
  ++m_usedBlocksInBlob;

  // Mark block as in-use with a sentinel so freeSingleBlock can detect
  // double-frees.  0xDEADF00D is used because it is an invalid (odd, low)
  // pointer on WASM32, so it can never appear as a legitimate free-list link.
  block->setNextFreeBlock((MemoryPoolSingleBlock *)0xDEADF00DUL);

#if defined(__EMSCRIPTEN__)
  // Per-block alloc trace for dmaPool_32: lets us reconstruct the
  // alloc/free timeline from the browser log and correlate with the
  // POOL-DOUBLE-FREE callstack from the matching freeSingleBlock.
  if (m_owningPool && strcmp(m_owningPool->getPoolName(), "dmaPool_32") == 0 && g_isDma32TracingEnabledNow) {
    char callstack_buf[4096];
    emscripten_get_callstack(EM_LOG_C_STACK | EM_LOG_JS_STACK | EM_LOG_DEMANGLE, callstack_buf, sizeof(callstack_buf));
    fprintf(stderr, "POOL-ALLOC-32 block=%p stack:\n%s\n", block->getUserData(), callstack_buf);
    fflush(stderr);
  }
#endif
...
#endif
#ifdef MEMORYPOOL_DEBUG
  // this is debug-only because it only serves to update the
  // debugLiteralTagString.
  block->initBlock(m_owningPool->getAllocationSize(), this,
                   m_owningPool->getOwningFactory(), debugLiteralTagString);
#endif
#ifdef MEMORYPOOL_INTENSE_VERIFY
  debugMemoryVerifyBlob();
#endif
...
//-----------------------------------------------------------------------------
/**
        make this block available for future allocations. it is assumed that the
   block belongs to this blob, and is not already free.
*/
...
#if defined(__EMSCRIPTEN__)
  // Per-block free trace for dmaPool_32: paired with POOL-ALLOC-32 lines
  // above so we can find the double-free pair in the browser log.
  if (m_owningPool && strcmp(m_owningPool->getPoolName(), "dmaPool_32") == 0 && g_isDma32TracingEnabledNow) {
    char callstack_buf[4096];
    emscripten_get_callstack(EM_LOG_C_STACK | EM_LOG_JS_STACK | EM_LOG_DEMANGLE, callstack_buf, sizeof(callstack_buf));
    fprintf(stderr, "POOL-FREE-32 block=%p stack:\n%s\n", block->getUserData(), callstack_buf);
    fflush(stderr);
  }
#endif
...
void MemoryPoolBlob::debugMemoryVerifyBlob() {
  ...
  DEBUG_ASSERTCRASH(m_usedBlocksInBlob >= 0 &&
                        m_usedBlocksInBlob <= m_totalBlocksInBlob,
                    ("unlikely m_usedBlocksInBlob"));
  DEBUG_ASSERTCRASH(m_totalBlocksInBlob > 0, ("unlikely m_totalBlocksInBlob"));

  Int rawBlockSize = MemoryPoolSingleBlock::calcRawBlockSize(
      m_owningPool->getAllocationSize());
  char *blockData = m_blockData;
  for (Int i = m_totalBlocksInBlob - 1; i >= 0;
       i--, blockData += rawBlockSize) {
...
Bool MemoryPoolBlob::debugIsBlockInBlob(void *pBlockPtr) {
  ...
  MemoryPoolSingleBlock *block =
      MemoryPoolSingleBlock::recoverBlockFromUserData(pBlockPtr);
  Int rawBlockSize = MemoryPoolSingleBlock::calcRawBlockSize(
      m_owningPool->getAllocationSize());
  char *blockData = m_blockData;
  for (Int i = m_totalBlocksInBlob - 1; i >= 0; i--) {
...
/**
        ...
        this does NOT free the checkpointinfos; that is presumed to happen
   elsewhere.
*/
...
Checkpointable::Checkpointable()
    : m_firstCheckpointInfo(nullptr), m_cpiEverFailed(false) {}
#endif
...
/**
        ...
        enough memory, and sets a flag to indicate our checkpointinfo is not
   complete.
*/
BlockCheckpointInfo *
Checkpointable::debugAddCheckpointInfo(const char *debugLiteralTagString,
                                       Int allocCheckpoint, Int blockSize) {

  BlockCheckpointInfo *bi = BlockCheckpointInfo::addToList(
      &m_firstCheckpointInfo, debugLiteralTagString, allocCheckpoint,
      blockSize);

  if (bi) {
#ifdef MEMORYPOOL_STACKTRACE
    void **stacktrace = bi->getStacktraceInfo();
    if (theStackTraceDepth > 0 && !TheGlobalData ||
        TheGlobalData->m_checkForLeaks) {
...
*/
void Checkpointable::debugCheckpointReport(Int flags, Int startCheckpoint,
                                           Int endCheckpoint,
                                           const char *poolName) {
  DEBUG_ASSERTCRASH(startCheckpoint >= 0 && startCheckpoint <= endCheckpoint,
                    ("bad checkpoints"));
  ...
  if (m_cpiEverFailed) {
    DEBUG_LOG(("  *** WARNING *** info on freed blocks may be inaccurate or "
               "incomplete!"));
  }

  for (BlockCheckpointInfo *bi = m_firstCheckpointInfo; bi;
       bi = bi->getNext()) {
    BlockCheckpointInfo::doBlockCheckpointReport(
        bi, poolName, flags, startCheckpoint, endCheckpoint);
  }
}
#endif
...
#ifdef MEMORYPOOL_CHECKPOINTING
/**
        throw away all the checkpointinfos. this frees the memory, but blocks
   might still refer to the discarded infos; you must zero those elsewhere.
*/
...
MemoryPool::MemoryPool()
    : m_factory(nullptr), m_nextPoolInFactory(nullptr), m_poolName(""),
      m_allocationSize(0), m_initialAllocationCount(0),
      m_overflowAllocationCount(0), m_usedBlocksInPool(0),
      m_totalBlocksInPool(0), m_peakUsedBlocksInPool(0), m_firstBlob(nullptr),
      m_lastBlob(nullptr), m_firstBlobWithFreeBlocks(nullptr) {}

//-----------------------------------------------------------------------------
/**
        initialize the memory pool with the given parameters. allocate the
   initial set of blocks.
*/
void MemoryPool::init(MemoryPoolFactory *factory, const char *poolName,
                      Int allocationSize, Int initialAllocationCount,
                      Int overflowAllocationCount) {
  m_factory = factory;
  m_poolName = poolName;
  m_allocationSize =
      ::roundUpMemBound(allocationSize); // round up to four-byte boundary
...
//-----------------------------------------------------------------------------
/**
        create a new blob for this pool. if you set up good values for
   initialAllocationCount, this should rarely (if ever) be called (though during
   development it will be called frequently).
*/
MemoryPoolBlob *MemoryPool::createBlob(Int allocationCount) {
  DEBUG_ASSERTCRASH(
      allocationCount > 0 && allocationCount % MEM_BOUND_ALIGNMENT == 0,
      ("bad allocationCount (must be >0 and evenly divisible by %d)",
       MEM_BOUND_ALIGNMENT));

  MemoryPoolBlob *blob = new (::sysAllocateDoNotZero(sizeof(MemoryPoolBlob)))
      MemoryPoolBlob; // will throw on failure

  blob->initBlob(this, allocationCount); // will throw on failure

  blob->addBlobToList(&m_firstBlob, &m_lastBlob);

  DEBUG_ASSERTCRASH(m_firstBlobWithFreeBlocks == nullptr,
                    ("DO NOT IGNORE. Please call John McD - x36872 "
                     "(m_firstBlobWithFreeBlocks != nullptr)"));
...
//-----------------------------------------------------------------------------
/**
        throw away a given blob, and all its blocks. it's assumed that the blob
   belongs to this pool.
*/
Int MemoryPool::freeBlob(MemoryPoolBlob *blob) {
  DEBUG_ASSERTCRASH(blob, ("null blob"));
  DEBUG_ASSERTCRASH(blob->getOwningPool() == this,
                    ("blob does not belong to this pool"));

  // save these for later...
  Int totalBlocksInBlob = blob->getTotalBlockCount();
  Int usedBlocksInBlob = blob->getUsedBlockCount();
  DEBUG_ASSERTCRASH(usedBlocksInBlob == 0,
                    ("freeing a nonempty blob (%d)", usedBlocksInBlob));

  // this is really just an estimate... will be too small in debug mode.
  Int amtFreed =
      totalBlocksInBlob * getAllocationSize() + sizeof(MemoryPoolBlob);
  ...
  // this is evil... since there is no 'placement delete' we must do this the
  // hard way and call the dtor directly. ordinarily this is heinous, but in
...
#ifdef MEMORYPOOL_DEBUG
  m_factory->adjustTotals("", -usedBlocksInBlob * getAllocationSize(),
                          -totalBlocksInBlob * getAllocationSize());
#endif
...
*/
void *
MemoryPool::allocateBlockDoNotZeroImplementation(DECLARE_LITERALSTRING_ARG1) {
  ScopedCriticalSection scopedCriticalSection(TheMemoryPoolCriticalSection);

  if (m_firstBlobWithFreeBlocks != nullptr &&
      !m_firstBlobWithFreeBlocks->hasAnyFreeBlocks()) {
    // hmm... the current 'free' blob has nothing available. look and see if
    // there are any other existing blobs with freespace.
  ...
  DEBUG_ASSERTCRASH(
      blob, ("no blob with free blocks available in MemoryPool::allocate"));

  MemoryPoolSingleBlock *block =
      blob->allocateSingleBlock(PASS_LITERALSTRING_ARG1);
  DEBUG_ASSERTCRASH(block, ("should not fail here"));

#ifdef MEMORYPOOL_CHECKPOINTING
  BlockCheckpointInfo *bi = debugAddCheckpointInfo(
      block->debugGetLiteralTagString(), m_factory->getCurCheckpoint(),
      getAllocationSize());
  if (bi)
...
void *MemoryPool::allocateBlockImplementation(DECLARE_LITERALSTRING_ARG1) {
  void *p = allocateBlockDoNotZeroImplementation(
      PASS_LITERALSTRING_ARG1); // throws on failure
...
void MemoryPool::freeBlock(void *pBlockPtr) {
  ...
  DEBUG_ASSERTCRASH(blob && blob->getOwningPool() == this,
  ...
  // normally we don't bother, but just in case this is ever desired, here's how
  // you'd do it...
  //
...
//-----------------------------------------------------------------------------
/**
        if the pool has any blobs that are completely unused, they are released
   back to the operating system. this will rarely, if ever, be called, but may
   be useful in odd situations.
*/
...
void MemoryPool::reset() {
  ...
  init(m_factory, m_poolName, m_allocationSize, m_initialAllocationCount,
       m_overflowAllocationCount); // will throw on failure
}
...
/*static*/ void MemoryPool::debugPoolInfoReport(MemoryPool *pool, FILE *fp) {
  ...
  if (!pool) {
    DEBUG_LOG(("%s,%32s,%6s,%6s,%6s,%6s,%6s,%6s", PREPEND, "POOLNAME", "BLKSZ",
               "INIT", "OVRFL", "USED", "TOTAL", "PEAK"));
    if (fp)
      fprintf(fp, "%s,%32s,%6s,%6s,%6s,%6s,%6s,%6s\n", PREPEND, "POOLNAME",
              "BLKSZ", "INIT", "OVRFL", "USED", "TOTAL", "PEAK");
  } else {
    DEBUG_LOG(("%s,%32s,%6d,%6d,%6d,%6d,%6d,%6d", PREPEND, pool->m_poolName,
               pool->m_allocationSize, pool->m_initialAllocationCount,
               pool->m_overflowAllocationCount, pool->m_usedBlocksInPool,
               pool->m_totalBlocksInPool, pool->m_peakUsedBlocksInPool));
    if (fp) {
      fprintf(fp, "%s,%32s,%6d,%6d,%6d,%6d,%6d,%6d\n", PREPEND,
              pool->m_poolName, pool->m_allocationSize,
              pool->m_initialAllocationCount, pool->m_overflowAllocationCount,
              pool->m_usedBlocksInPool, pool->m_totalBlocksInPool,
...
void MemoryPool::debugMemoryVerifyPool() {
  ...
  DEBUG_ASSERTCRASH(m_usedBlocksInPool == used,
                    ("used mismatch %d %d", m_usedBlocksInPool, used));
  DEBUG_ASSERTCRASH(m_totalBlocksInPool == total,
                    ("total mismatch %d %d", m_totalBlocksInPool, total));
}
#endif
...
Bool MemoryPool::debugIsBlockInPool(void *pBlockPtr) {
  ...
  DEBUG_ASSERTCRASH(check1 == check2,
                    ("mismatch checks in debugIsBlockInPool"));
...
/**
        ...
        the block is null or invalid, "FREE_SINGLEBLOCK_TAG_STRING" will be
   returned. it is assumed that the block was allocated by this pool.
*/
...
#ifdef MEMORYPOOL_CHECKPOINTING
/**
        free all checkpointinfo for this pool, and reset all ptrs to
   checkpointinfo.
*/
...
DynamicMemoryAllocator::DynamicMemoryAllocator()
    : m_factory(nullptr), m_nextDmaInFactory(nullptr), m_numPools(0),
      m_usedBlocksInDma(0), m_rawBlocks(nullptr) {
...
*/
void DynamicMemoryAllocator::init(MemoryPoolFactory *factory, Int numSubPools,
                                  const PoolInitRec pParms[]) {
  const PoolInitRec defaultDMA[7] = {
      {"dmaPool_16", 16, 64, 64},    {"dmaPool_32", 32, 64, 64},
  ...
  for (Int i = 0; i < m_numPools; i++) {
    DEBUG_ASSERTCRASH(i == 0 || pParms[i].allocationSize >
                                    pParms[i - 1].allocationSize,
...
void DynamicMemoryAllocator::removeFromList(DynamicMemoryAllocator **pHead) {
  ...
  for (DynamicMemoryAllocator *cur = *pHead; cur;
       cur = cur->m_nextDmaInFactory) {
    if (cur == this) {
      if (prev) {
...
#ifdef MEMORYPOOL_DEBUG
    DEBUG_ASSERTCRASH(findPoolForSize(block->debugGetLogicalSize()) ==
                          block->getOwningBlob()->getOwningPool(),
                      ("pool mismatch"));
#endif
    block->debugIgnoreLeaksForThisBlock();
  } else {
    DEBUG_CRASH((
        "cannot currently ignore leaks for raw blocks (allocation too large)"));
  }
}
#endif
...
//-----------------------------------------------------------------------------
/**
        allocate a chunk-o-bytes from this DMA and return it, but don't bother
  zeroing out the block. if unable to allocate, throw ERROR_OUT_OF_MEMORY. this
        function will never return null.

  added code to make sure we're on a DWord boundary, throw exception if not
*/
void *DynamicMemoryAllocator::allocateBytesDoNotZeroImplementation(
    Int numBytes DECLARE_LITERALSTRING_ARG2) {
  ...
  if (pool != nullptr) {
    result =
        pool->allocateBlockDoNotZeroImplementation(PASS_LITERALSTRING_ARG1);
#ifdef MEMORYPOOL_DEBUG
    {
      USE_PERF_TIMER(MemoryPoolDebugging)
      waste = pool->getAllocationSize() - numBytes;
      MemoryPoolSingleBlock *wblock =
          MemoryPoolSingleBlock::recoverBlockFromUserData(result);
    ...
    MemoryPoolSingleBlock *block =
        MemoryPoolSingleBlock::rawAllocateSingleBlock(
            &m_rawBlocks, numBytes, m_factory PASS_LITERALSTRING_ARG2);

#ifdef MEMORYPOOL_CHECKPOINTING
    BlockCheckpointInfo *bi =
        debugAddCheckpointInfo(block->debugGetLiteralTagString(),
                               m_factory->getCurCheckpoint(), numBytes);
    if (bi)
      block->debugSetCheckpointInfo(bi);
#endif
  ...
  DEBUG_ASSERTCRASH(m_usedBlocksInDma >= 0,
                    ("negative count for m_usedBlocksInDma"));
#ifdef MEMORYPOOL_DEBUG
#ifdef USE_FILLER_VALUE
  {
    USE_PERF_TIMER(MemoryPoolInitFilling)
...
//-----------------------------------------------------------------------------
/**
        allocate a chunk-o-bytes from this DMA and return it, and zero out the
   contents first. if unable to allocate, throw ERROR_OUT_OF_MEMORY. this
   function will never return null.
*/
void *DynamicMemoryAllocator::allocateBytesImplementation(
    Int numBytes DECLARE_LITERALSTRING_ARG2) {
  void *p = allocateBytesDoNotZeroImplementation(
      numBytes PASS_LITERALSTRING_ARG2); // throws on failure
  memset(p, 0, numBytes);
  return p;
}
...
void DynamicMemoryAllocator::freeBytes(void *pBlockPtr) {
  ...
  if (block->getOwningBlob()) {
    ...
    {
      USE_PERF_TIMER(MemoryPoolDebugging)
      DEBUG_ASSERTCRASH(findPoolForSize(used) ==
                            block->getOwningBlob()->getOwningPool(),
                        ("pool mismatch"));
#ifdef INTENSE_DMA_BOOKKEEPING
      if (doingIntenseDMA == 0)
...
Bool DynamicMemoryAllocator::debugIsBlockInDma(void *pBlockPtr) {
  ...
  if (block->getOwningBlob()) {
    ...
    return pool && pool->debugIsBlockInPool(pBlockPtr) &&
           debugIsPoolInDma(pool);
  } else {
...
/**
        return the tagstring for the block. this will never return null; if
        the block is null or invalid, "FREE_SINGLEBLOCK_TAG_STRING" will be
   returned. it is assumed that the block was allocated by this dma.
*/
...
#ifdef MEMORYPOOL_CHECKPOINTING
/**
        free all checkpointinfo for this dma, and reset all ptrs to
   checkpointinfo.
*/
...
void DynamicMemoryAllocator::debugDmaInfoReport(FILE *fp) {
  ...
  DEBUG_LOG(("%s,Average Raw Block Size = %d", PREPEND,
             numBlocks ? bytes / numBlocks : 0));
...
MemoryPoolFactory::MemoryPoolFactory()
    : m_firstPoolInFactory(nullptr), m_firstDmaInFactory(nullptr)
#ifdef MEMORYPOOL_CHECKPOINTING
      ,
      m_curCheckpoint(0)
#endif
#ifdef MEMORYPOOL_DEBUG
      ,
      m_usedBytes(0), m_physBytes(0), m_peakUsedBytes(0), m_peakPhysBytes(0)
#endif
{
#ifdef MEMORYPOOL_DEBUG
...
MemoryPool *MemoryPoolFactory::createMemoryPool(const PoolInitRec *parms) {
  return createMemoryPool(parms->poolName, parms->allocationSize,
...
MemoryPool *MemoryPoolFactory::createMemoryPool(const char *poolName,
  ...
  if (pool) {
    DEBUG_ASSERTCRASH(allocationSize == pool->getAllocationSize(),
                      ("pool size mismatch"));
    return pool;
  }

  userMemoryAdjustPoolSize(poolName, initialAllocationCount,
                           overflowAllocationCount);

  if (initialAllocationCount <= 0 || overflowAllocationCount < 0) {
    DEBUG_CRASH(("illegal pool size: %d %d", initialAllocationCount,
                 overflowAllocationCount));
    throw ERROR_OUT_OF_MEMORY;
  }

  pool = new (::sysAllocateDoNotZero(sizeof(MemoryPool)))
      MemoryPool; // will throw on failure
  pool->init(this, poolName, allocationSize, initialAllocationCount,
             overflowAllocationCount); // will throw on failure
...
//-----------------------------------------------------------------------------
/**
        find a memory pool with the given name; return null if no such pool
   exists, return null. note that this function isn't particularly fast (it just
   does a linear search), so you should probably cache the result.
*/
MemoryPool *MemoryPoolFactory::findMemoryPool(const char *poolName) {
  for (MemoryPool *pool = m_firstPoolInFactory; pool;
       pool = pool->getNextPoolInList()) {
    if (strcmp(poolName, pool->getPoolName()) == 0) {
      DEBUG_ASSERTCRASH(poolName == pool->getPoolName(),
                        ("hmm, ptrs should probably match here"));
      return pool;
    }
  }
  return nullptr;
...
//-----------------------------------------------------------------------------
/**
        destroy the given memory pool. you normally will never need to call this
   directly.
*/
void MemoryPoolFactory::destroyMemoryPool(MemoryPool *pMemoryPool) {
  ...
  DEBUG_ASSERTCRASH(pMemoryPool->getUsedBlockCount() == 0,
...
//-----------------------------------------------------------------------------
/**
        create a new dynamicMemoryAllocator. You normally will never need to
   call this directly.
*/
DynamicMemoryAllocator *
MemoryPoolFactory::createDynamicMemoryAllocator(Int numSubPools,
                                                const PoolInitRec pParms[]) {
  DynamicMemoryAllocator *dma;

  dma = new (::sysAllocateDoNotZero(sizeof(DynamicMemoryAllocator)))
      DynamicMemoryAllocator;           // will throw on failure
...
//-----------------------------------------------------------------------------
/**
        destroy the given dynamicMemoryAllocator. You normally will never need
   to call this directly.
*/
void MemoryPoolFactory::destroyDynamicMemoryAllocator(
    DynamicMemoryAllocator *dma) {
  if (!dma)
    return;
...
//-----------------------------------------------------------------------------
/**
        throw away everything in all pools/dmas owned by this factory, but keep
   the factory and pools/dmas themselves valid.
*/
void MemoryPoolFactory::reset() {
  ...
  for (DynamicMemoryAllocator *dma = m_firstDmaInFactory; dma;
       dma = dma->getNextDmaInList()) {
    dma->reset();
  }
...
*/
void MemoryPoolFactory::adjustTotals(const char *tagString, Int usedDelta,
                                     Int physDelta) {
  USE_PERF_TIMER(MemoryPoolDebugging)
  ...
  for (int i = 1; i < MAX_SPECIAL_USED; ++i) // start at 1, not zero
  {
    ...
    if (strncmp(tagString, s_specialPrefixes[i],
                strlen(s_specialPrefixes[i])) == 0) {
      found = i;
      break;
    }
  }
...
/**
        ...
        the block is null or invalid, "FREE_SINGLEBLOCK_TAG_STRING" will be
   returned. it is assumed that the block was allocated by this factory.
*/
...
#ifdef MEMORYPOOL_CHECKPOINTING
/**
        set a new checkpoint for all future blocks allocated/freed by this
   factory's pools/dmas.
*/
Int MemoryPoolFactory::debugSetCheckpoint() { return ++m_curCheckpoint; }
#endif
...
void MemoryPoolFactory::memoryPoolUsageReport(const char *filename,
  ...
  if (perfStatsFile == nullptr) {
    DEBUG_CRASH(
        ("could not open/create perf file %s -- is it open in another app?",
         filename));
    return;
  }
  ...
  for (;;) {
    ...
    if (pool) {
      if (lineIdx == 0) {
        fprintf(perfStatsFile, "%s,%d", "Unpooled Large Blocks (>1024 bytes)",
                thePeakLargeBlocks / 1024);
      } else {
        ...
        if (waste < 0)
          waste = 0;
        fprintf(perfStatsFile, "%s,%d,%d", pool->getPoolName(), peak / 1024,
                waste / 1024);
    ...
    if (it != TheUsedNPeakMap.end()) {
      Int wastepct = (it->second.peakwaste * 100) /
                     (it->second.peak + it->second.peakwaste);
      fprintf(perfStatsFile, ",,,,%s,%d,%d,%d", it->first,
              it->second.peak / 1024, it->second.peakwaste / 1024, wastepct);
      ++it;
      keepGoing = true;
    } else {
      fprintf(perfStatsFile, ",,,,,,,");
    ...
    if (lineIdx < MAX_SPECIAL_USED && s_specialPrefixes[lineIdx] != nullptr) {
      fprintf(perfStatsFile, ",,,%s,%d", s_specialPrefixes[lineIdx],
              m_usedBytesSpecialPeak[lineIdx] / 1024);
      keepGoing = true;
    }
...
*/
void MemoryPoolFactory::debugMemoryReport(Int flags, Int startCheckpoint,
                                          Int endCheckpoint, FILE *fp) {
  // USE_PERF_TIMER(MemoryPoolDebugging) skip end-of-run reporting stuff
...
#ifdef MEMORYPOOL_CHECKPOINTING
  Bool doBlockReport = (flags & _REPORT_CP_ALLOCATED_DONTCARE) != 0 &&
                       (flags & _REPORT_CP_FREED_DONTCARE) != 0;
  DEBUG_ASSERTCRASH(startCheckpoint >= 0 && startCheckpoint <= endCheckpoint &&
                        endCheckpoint <= m_curCheckpoint,
                    ("bad checkpoints"));
  DEBUG_ASSERTCRASH(((flags & _REPORT_CP_ALLOCATED_DONTCARE) != 0) ==
                        ((flags & _REPORT_CP_FREED_DONTCARE) != 0),
  ...
  if (flags & REPORT_POOL_OVERFLOW) {
    ...
    for (; pool; pool = pool->getNextPoolInList()) {
      if (pool->getPeakBlockCount() > pool->getInitialBlockCount()) {
        DEBUG_LOG(("*** Pool %s overflowed initial allocation of %d (peak "
                   "allocation was %d)",
                   pool->getPoolName(), pool->getInitialBlockCount(),
    ...
    for (pool = m_firstPoolInFactory; pool; pool = pool->getNextPoolInList()) {
      ...
      if (peak < initial / 2 && (initial - peak) > 4096) {
        DEBUG_LOG(("*** Pool %s used less than half its initial allocation of "
                   "%d (peak allocation was %d, wasted %dk)",
                   pool->getPoolName(), pool->getInitialBlockCount(),
                   pool->getPeakBlockCount(), (initial - peak) / 1024));
      }
    }
    DEBUG_LOG(("------------------------------------------"));
  ...
  if (flags & REPORT_SIMPLE_LEAKS) {
    ...
    DEBUG_ASSERTCRASH(!any,
                      ("There were %d memory leaks. Please fix them.", any));
    DEBUG_LOG(("------------------------------------------"));
...
#ifdef MEMORYPOOL_CHECKPOINTING
  if (doBlockReport) {
    const char *nm = (this == TheMemoryPoolFactory)
                         ? "TheMemoryPoolFactory"
                         : "*** UNKNOWN *** MemoryPoolFactory";
    ...
    for (MemoryPool *pool = m_firstPoolInFactory; pool;
         pool = pool->getNextPoolInList()) {
      pool->debugCheckpointReport(flags, startCheckpoint, endCheckpoint,
                                  pool->getPoolName());
    }
    for (DynamicMemoryAllocator *dma = m_firstDmaInFactory; dma;
         dma = dma->getNextDmaInList()) {
      dma->debugCheckpointReport(flags, startCheckpoint, endCheckpoint,
                                 "(Oversized)");
    }
...
void *STLSpecialAlloc::allocate(size_t __n) {
  ...
  DEBUG_ASSERTCRASH(
      ...
      ("must init memory manager before calling global operator new"));
...
//-----------------------------------------------------------------------------
/**
        overload for global operator new; send requests to
...
void *operator new[](size_t size) {
  ...
  DEBUG_ASSERTCRASH(
      TheDynamicMemoryAllocator != nullptr,
      ("must init memory manager before calling global operator new"));
  return TheDynamicMemoryAllocator->allocateBytes(size,
                                                  "global operator new[]");
}

//-----------------------------------------------------------------------------
/**
        overload for global operator delete; send requests to
   TheDynamicMemoryAllocator.
*/
...
//-----------------------------------------------------------------------------
/**
        overload for global operator new (MFC debug version); send requests to
   TheDynamicMemoryAllocator.
*/
...
//-----------------------------------------------------------------------------
/**
        overload for global operator delete (MFC debug version); send requests
   to TheDynamicMemoryAllocator.
*/
...
void *calloc(size_t a, size_t b) {
  ++theLinkTester;
  preMainInitMemoryManager();
  DEBUG_ASSERTCRASH(TheDynamicMemoryAllocator != nullptr,
                    ("must init memory manager"));
...
//-----------------------------------------------------------------------------
/**
        Initialize the memory manager, and create TheMemoryPoolFactory and
   TheDynamicMemoryAllocator.
*/
void initMemoryManager() {
  if (TheMemoryPoolFactory == nullptr) {
    ...
    TheMemoryPoolFactory =
        new (::sysAllocateDoNotZero(sizeof(MemoryPoolFactory)))
            MemoryPoolFactory;    // will throw on failure
    TheMemoryPoolFactory->init(); // will throw on failure
    TheDynamicMemoryAllocator =
        TheMemoryPoolFactory->createDynamicMemoryAllocator(
            numSubPools, pParms); // will throw on failure
...
#endif
  {
    // LinkTester fails on Emscripten because of dead-code elimination.
...
//-----------------------------------------------------------------------------
Bool isMemoryManagerOfficiallyInited() { return theMainInitFlag; }

//-----------------------------------------------------------------------------
/**
        Initialize the memory manager, and create TheMemoryPoolFactory and
   TheDynamicMemoryAllocator. This is only called if memory is allocated prior
   to the normal call to initMemoryManager (generally via a static C++ ctor).
*/
static void preMainInitMemoryManager() {
  if (TheMemoryPoolFactory == nullptr) {
...
//-----------------------------------------------------------------------------
/**
        shutdown the memory manager and discard all memory. Note: if
   preMainInitMemoryManager() was called prior to initMemoryManager(), this call
   will do nothing.
*/
...
#ifdef MEMORYPOOL_DEBUG
    DEBUG_LOG(
        ("*** Memory Manager was inited prior to main -- skipping shutdown!"));
#endif
  } else {
    if (TheDynamicMemoryAllocator) {
      DEBUG_ASSERTCRASH(TheMemoryPoolFactory, ("hmm, no factory"));
      if (TheMemoryPoolFactory)
        TheMemoryPoolFactory->destroyDynamicMemoryAllocator(
            TheDynamicMemoryAllocator);
      TheDynamicMemoryAllocator = nullptr;
    }

    if (TheMemoryPoolFactory) {
      // this is evil... since there is no 'placement delete' we must do this
      // the hard way and call the dtor directly. ordinarily this is heinous,
...
void *createW3DMemPool(const char *poolName, int allocationSize) {
  ...
  preMainInitMemoryManager();
  MemoryPool *pool =
      TheMemoryPoolFactory->createMemoryPool(poolName, allocationSize, 0, 0);
  DEBUG_ASSERTCRASH(pool && pool->getAllocationSize() == allocationSize,
                    ("bad w3d pool"));
  return pool;
}
...
//-----------------------------------------------------------------------------
void *allocateFromW3DMemPool(void *pool, int allocationSize, const char *msg,
                             int unused) {
  DEBUG_ASSERTCRASH(pool, ("pool is null"));
  DEBUG_ASSERTCRASH(
      pool && ((MemoryPool *)pool)->getAllocationSize() == allocationSize,
