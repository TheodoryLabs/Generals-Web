...
WebBrowser::WebBrowser() : mRefCount(1) {
...
STDMETHODIMP WebBrowser::QueryInterface(REFIID iid,
                                        void **ppv) IUNKNOWN_NOEXCEPT {
  *ppv = nullptr;
...
ULONG STDMETHODCALLTYPE WebBrowser::Release(void) IUNKNOWN_NOEXCEPT {
  ...
  if (mRefCount == 0) {
    DEBUG_LOG((
        "WebBrowser::Release - all references released, deleting the object."));
    if (this == TheWebBrowser) {
      TheWebBrowser = nullptr;
    }
    delete this;
