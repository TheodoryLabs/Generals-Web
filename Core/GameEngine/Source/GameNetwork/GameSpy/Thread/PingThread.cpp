...
AsciiString Pinger::getPingString(Int timeout) {
  ...
  AsciiString tmp;
  for (std::map<std::string, Int>::const_iterator it = m_pingMap.begin();
       it != m_pingMap.end(); ++it) {
    Int ping = it->second;
...
Int PingThreadClass::doPing(UnsignedInt IP, Int timeout) {
  ...
  DWORD(WINAPI * lpfnIcmpSendEcho)(HANDLE, DWORD, LPVOID, WORD, LPVOID, LPVOID,
                                   DWORD, DWORD) = nullptr;
   ...
   */
  lpfnIcmpCreateFile = (void *(__stdcall *)())GetProcAddress(
      (HINSTANCE)hICMP_DLL, "IcmpCreateFile");
  lpfnIcmpCloseHandle = (int(__stdcall *)(void *))GetProcAddress(
      (HINSTANCE)hICMP_DLL, "IcmpCloseHandle");
  lpfnIcmpSendEcho = (unsigned long(__stdcall *)(
      void *, unsigned long, void *, unsigned short, void *, void *,
      unsigned long, unsigned long))GetProcAddress((HINSTANCE)hICMP_DLL,
                                                   "IcmpSendEcho");
   ...
   *                   and read the Reply
   */
  dwReplyCount = lpfnIcmpSendEcho(hICMP, IP, achReqData, nDataLen, lpstIPInfo,
