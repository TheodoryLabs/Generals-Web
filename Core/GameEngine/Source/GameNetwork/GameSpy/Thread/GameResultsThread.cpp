...
private:
  Int sendGameResults(UnsignedInt IP, UnsignedShort port,
...
GameResultsQueue::~GameResultsQueue() { endThreads(); }
    ...
    while (running) {
      ...
      if (TheGameResultsQueue && TheGameResultsQueue->getRequest(req)) {
        ...
        if (isdigit(hostnameBuffer[0])) {
          ...
          in_addr hostNode;
          hostNode.s_addr = IP;
          DEBUG_LOG(("sending game results to %s - IP = %s", hostnameBuffer,
                     inet_ntoa(hostNode)));
        } else {
          HOSTENT *hostStruct;
          in_addr *hostNode;
...
Int GameResultsThreadClass::sendGameResults(UnsignedInt IP, UnsignedShort port,
                                            const std::string &results) {
  int error = 0;
  ...
  if (sock < 0) {
    DEBUG_LOG(
        ("GameResultsThreadClass::sendGameResults() - socket() returned %d(%s)",
  ...
  if (connect(sock, (struct sockaddr *)&sockAddr, sizeof(sockAddr)) == -1) {
    ...
    DEBUG_LOG((
        "GameResultsThreadClass::sendGameResults() - connect() returned %d(%s)",
  ...
  if (send(sock, results.c_str(), results.length(), 0) == SOCKET_ERROR) {
    error = WSAGetLastError();
    DEBUG_LOG(
        ("GameResultsThreadClass::sendGameResults() - send() returned %d(%s)",
         error, getWSAErrorString(error)));
    closesocket(sock);
