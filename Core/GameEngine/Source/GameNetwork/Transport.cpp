...
Bool Transport::update() {
  Bool retval = TRUE;
  if (doRecv() == FALSE && m_udpsock &&
      m_udpsock->GetStatus() == UDP::ADDRNOTAVAIL) {
    retval = FALSE;
  }
  DEBUG_ASSERTLOG(
      retval, ("WSA error is %s", GetWSAErrorString(WSAGetLastError()).str()));
  if (doSend() == FALSE && m_udpsock &&
      m_udpsock->GetStatus() == UDP::ADDRNOTAVAIL) {
    retval = FALSE;
  }
  DEBUG_ASSERTLOG(
...
Bool Transport::doSend() {
  ...
  if (m_lastSecond + 1000 < now) {
    m_lastSecond = now;
    m_statisticsSlot =
        (m_statisticsSlot + 1) % MAX_TRANSPORT_STATISTICS_SECONDS;
  ...
  for (i = 0; i < MAX_MESSAGES; ++i) {
    if (m_outBuffer[i].length != 0) {
      int bytesSent = 0;
      // TheSuperHackers @info The handling of data sizing of the payload within
      // a UDP packet is confusing due to the current networking implementation
      // The max game packet size needs to be smaller than max udp payload by
      // sizeof(TransportMessageHeader) But the max network message size needs
      // to include the bytes of the transport message header and equal the max
      // udp payload Therefore, transmitted data needs to add the extra bytes of
      // the network header to the payloads length
      int bytesToSend = m_outBuffer[i].length + sizeof(TransportMessageHeader);
      // Send this message
      if ((bytesSent = m_udpsock->Write((unsigned char *)(&m_outBuffer[i]),
                                        bytesToSend, m_outBuffer[i].addr,
                                        m_outBuffer[i].port)) > 0) {
        // DEBUG_LOG(("Sending %d bytes to %d.%d.%d.%d:%d", bytesToSend,
        ...
        if (bytesSent != bytesToSend) {
          DEBUG_LOG(("Transport::doSend - wanted to send %d bytes, only sent "
  ...
  if (m_useLatency) {
    for (i = 0; i < MAX_MESSAGES; ++i) {
      if (m_delayedInBuffer[i].message.length != 0 &&
          m_delayedInBuffer[i].deliveryTime <= now) {
        for (int j = 0; j < MAX_MESSAGES; ++j) {
          if (m_inBuffer[j].length == 0) {
            // Empty slot; use it
            memcpy(&m_inBuffer[j], &m_delayedInBuffer[i].message,
...
Bool Transport::doRecv() {
  ...
  UnsignedInt now = timeGetTime();
#endif
  // TheSuperHackers @info The handling of data sizing of the payload within a
  // UDP packet is confusing due to the current networking implementation The
  // max game packet size needs to be smaller than max udp payload by
  // sizeof(TransportMessageHeader) But the max network message size needs to
  // include the bytes of the transport message header and equal the max udp
  // payload Therefore, when receiving data we use the max udp payload size to
  // receive the game packet payload and network header
  ...
  while ((len = m_udpsock->Read(buf, MAX_NETWORK_MESSAGE_LEN, &from)) > 0) {
    ...
    if (len <= sizeof(TransportMessageHeader) ||
        !isGeneralsPacket(&incomingMessage)) {
    ...
    //		DEBUG_LOG(("Saw %d bytes from %d:%d", len,
      ...
      if (m_useLatency) {
        if (m_delayedInBuffer[i].message.length == 0) {
          ...
          m_delayedInBuffer[i].deliveryTime =
              ...
              (Int)(TheGlobalData->m_latencyAmplitude *
                    sin(now * TheGlobalData->m_latencyPeriod)) +
              GameClientRandomValue(-TheGlobalData->m_latencyNoise,
                                    TheGlobalData->m_latencyNoise);
  ...
  for (i = 0; i < MAX_MESSAGES; ++i) {
    if (m_outBuffer[i].length == 0) {
      ...
      crc.computeCRC((unsigned char *)(&(m_outBuffer[i].header.magic)),
                     m_outBuffer[i].length + sizeof(TransportMessageHeader) -
                         sizeof(UnsignedInt));
      //			DEBUG_LOG(("About to assign the CRC for the
      //packet"));
      m_outBuffer[i].header.crc = crc.get();

      // Encrypt packet
      //			DEBUG_LOG(("buffer: "));
      encryptBuf((unsigned char *)&m_outBuffer[i],
                 len + sizeof(TransportMessageHeader));
      //			DEBUG_LOG((""));
...
Bool Transport::isGeneralsPacket(TransportMessage *msg) {
  ...
  crc.computeCRC((unsigned char *)(&(msg->header.magic)),
                 msg->length + sizeof(TransportMessageHeader) -
                     sizeof(UnsignedInt));
