...
public:
  ...
  virtual Bool
  isAttachedTo(GameWindow *window); ///< Is the manager attached toa window
  virtual GameWindow *
  getWindow(); ///< Returns the window we are currently attached to
  virtual Bool
  isComposing(); ///< Manager is currently composing new input string
  virtual void getCompositionString(
      UnicodeString &string); ///< Return the current composition string
  virtual Int
  getCompositionCursorPosition(); ///< Returns the composition cursor position
  ...
  getCandidate(Int index);                 ///< Returns the candidate string
  virtual Int getSelectedCandidateIndex(); ///< Returns the indexed of the
                                           ///< currently selected candidate
  virtual Int
  getCandidatePageSize(); ///< Returns the page size for the candidates list
  virtual Int
  getCandidatePageStart(); ///< Returns the index of the first visibel candidate
...
protected:
  ...
  Int m_compositionCharsDisplayed;  ///< number of temporary composition
                                    ///< characters displayed that need to be
                                    ///< replaced with result string.

  WideChar
  convertCharToWide(WPARAM mbchar); ///< Convert multibyte character to wide
  void updateCompositionString();   ///< Update the context of the composition
                                    ///< string from the IMM
  ...
  void updateListBox(
      CANDIDATELIST *candidateList); ///< Update candidate list box gadget
  ...
  void buildFlagsString(IMEManager::MessageInfo *msgTable, Int value,
                        AsciiString &string);
...
void IMEManager::buildFlagsString(IMEManager::MessageInfo *msgTable, Int value,
                                  AsciiString &string) {
  string.clear();
  Bool first = TRUE;
...
void IMEManager::printMessageInfo(Int message, Int wParam, Int lParam) {
  ...
  case WM_IME_NOTIFY: {
    ...
    if (notifyName == nullptr)
      notifyName = "unknown";
    DEBUG_LOG(("IMM: %s(0x%04x) - %s(0x%04x) - 0x%08x", messageText, message,
               notifyName, wParam, lParam));
    break;
  }
  case WM_IME_CONTROL: {
  ...
  case WM_IME_SETCONTEXT: {
    ...
    DEBUG_LOG(("IMM: %s(0x%04x) - 0x%08x - %s(0x%04x)", messageText, message,
               wParam, flags.str(), lParam));
    break;
  }
  default:
    if (messageText) {
      DEBUG_LOG(("IMM: %s(0x%04x) - 0x%08x - 0x%08x", messageText, message,
                 wParam, lParam));
    }
    break;
  }
...
IMEManagerInterface *CreateIMEManagerInterface() { return NEW IMEManager; }
...
IMEManager::IMEManager()
    : m_window(nullptr), m_context(nullptr), m_candidateWindow(nullptr),
      m_statusWindow(nullptr), m_candidateCount(0), m_candidateString(nullptr),
      m_compositionStringLength(0), m_composing(FALSE), m_disabled(0),
      m_result(0), m_indexBase(1),

      m_compositionCharsDisplayed(0), m_candidateDownArrow(nullptr),
      m_candidateTextArea(nullptr), m_candidateUpArrow(nullptr),
      m_compositionCursorPos(0), m_pageSize(0), m_pageStart(0),
      m_selectedIndex(0), m_unicodeIME(FALSE) {}
...
void IMEManager::init() {
  ...
  m_disabled = 0;
  m_candidateWindow =
      TheWindowManager->winCreateFromScript("IMECandidateWindow.wnd");
  ...
  if (m_candidateWindow) {
    ...
    NameKeyType id =
        TheNameKeyGenerator->nameToKey("IMECandidateWindow.wnd:TextArea");
    m_candidateTextArea =
        TheWindowManager->winGetWindowFromId(m_candidateWindow, id);
...
Bool IMEManager::serviceIMEMessage(void *windowsHandle, UnsignedInt message,
                                   Int wParam, Int lParam) {

  DEBUG_ASSERTCRASH(windowsHandle == ApplicationHWnd,
                    ("Unexpected window handle for IMEManager"));
#ifdef DEBUG_IME
  ...
  case WM_IME_ENDCOMPOSITION: {
    ...
    m_composing = FALSE;
    while (m_compositionCharsDisplayed >
           0) { // if cursor has moved since start of composition, we need to
                // move it back using backspace message
      TheWindowManager->winSendInputMsg(m_window, GWM_CHAR, KEY_BACKSPACE,
                                        KEY_STATE_DOWN);
  ...
  case WM_IME_COMPOSITION: {
    ...
    if (lParam & GCS_RESULTSTR) // added to show instant updates as soon as a
                                // character is translated. -MW
    {
      if (m_window) {
        m_composing = FALSE; // reset this flag before calling GWM_IME_CHAR

        while (m_compositionCharsDisplayed >
               0) { // if cursor has moved since start of composition, we need
                    // to move it back using backspace message
          TheWindowManager->winSendInputMsg(m_window, GWM_CHAR, KEY_BACKSPACE,
                                            KEY_STATE_DOWN);
          m_compositionCharsDisplayed--;
        }
      ...
      m_compositionCharsDisplayed = 0;
               ...
               lParam &
                   CS_NOMOVECARET) { // we are supposed to display the
                                     // composition character without moving the
                                     // cursor. (it's a candidate).
      if (m_window) {
        m_composing = FALSE; // reset this flag before calling GWM_IME_CHAR
    ...
    } else if (lParam & GCS_COMPSTR) { // we are supposed to display the
                                       // composition character without moving
                                       // the cursor. (it's a candidate).
      if (m_window) {
        m_composing = FALSE; // reset this flag before calling GWM_IME_CHAR
  ...
  case WM_IME_NOTIFY: {
    ...
    case IMN_GUIDELINE: // This message is sent when an IME is about to show an
    ...
    case IMN_SETCONVERSIONMODE: // This message is sent when the conversion mode
                                // of the input context is updated.
    {
#ifdef DEBUG_IME
    ...
    case IMN_SETSENTENCEMODE: // This message is sent when the sentence mode of
                              // the input context is updated.
    {
#ifdef DEBUG_IME
      printSentenceStatus();
#endif
    ...
    /*
                                            // pass the reset on through to the
       installed IME case IMN_SETCANDIDATEPOS:        //This message is sent
       when the IME is about to move the candidate window. case
       IMN_SETSTATUSWINDOWPOS:     //This message is sent when the status window
       position in the input context is updated. return FALSE; case
       IMN_OPENSTATUSWINDOW:       //This message is sent when an IME is about
       to create the status window. DEBUG_LOG(("Open Status Window")); return
       FALSE; case IMN_CLOSESTATUSWINDOW:      //This message is sent to the
       application when an input method editor (IME) is about to close the
       status window. DEBUG_LOG(("Close Status Window")); return FALSE; case
       IMN_SETOPENSTATUS:          //This message is sent when the open status
       of the input context is updated. case IMN_SETCOMPOSITIONFONT:     //This
       message is sent when the font of the input context is updated. case
       IMN_SETCOMPOSITIONWINDOW:   //This message is sent when the style or
       position of the composition window is updated. case IMN_PRIVATE: //This
       message is for your own use, it seems.
    */
    default:
      m_result = 1;
      return TRUE;
...
Int IMEManager::result() { return m_result; }
...
Bool IMEManager::isEnabled() { return m_context != nullptr && m_disabled == 0; }

//============================================================================
// IMEManager::isAttached
//============================================================================

Bool IMEManager::isAttachedTo(GameWindow *window) { return m_window == window; }

//============================================================================
// IMEManager::getWindow
//============================================================================

GameWindow *IMEManager::getWindow() { return m_window; }
...
WideChar IMEManager::convertCharToWide(WPARAM wParam) {
  ...
  if (MultiByteToWideChar(CP_ACP, 0, dcbsString, strlen(dcbsString), uniString,
                          1) == 1) {
    return uniString[0];
  }
...
Bool IMEManager::isComposing() { return m_composing; }
...
void IMEManager::updateCompositionString() {
  ...
  if (m_context) {
    ...
    LONG result = ImmGetCompositionStringW(
        m_context, GCS_COMPSTR, m_compositionString, MAX_COMPSTRINGLEN);

    if (result >= 0) {
      m_compositionStringLength = result / 2;
      m_compositionCursorPos =
          (ImmGetCompositionStringW(m_context, GCS_CURSORPOS, nullptr, 0) &
           0xffff);
    } else {

      // read MBCS instead
      result = ImmGetCompositionStringA(m_context, GCS_COMPSTR, tempBuf,
                                        MAX_COMPSTRINGLEN * 2);

      if (result > 0) {
        tempBuf[result] = '\0';

        int convRes = MultiByteToWideChar(
            CP_ACP, 0, tempBuf, -1, m_compositionString, MAX_COMPSTRINGLEN);
        ...
        if (convRes < 0) {
          ...
          m_compositionCursorPos =
              (ImmGetCompositionString(m_context, GCS_CURSORPOS, nullptr, 0) &
               0xffff);
          convRes = wcslen(m_compositionString);
        }

        // m_compositionCursorPos is in DBCS characters, need to convert it to
        // Wide characters

        // msg_assert ( (int)strlen(tempBuf) >= convRes ,("bad DBCS string: DBCS
        // = %d chars, Wide = %d chars", strlen(tempBuf), convRes));
        m_compositionCursorPos =
            _mbsnccnt((unsigned char *)tempBuf, m_compositionCursorPos);
  ...
  DEBUG_ASSERTCRASH(m_compositionStringLength < MAX_COMPSTRINGLEN,
                    ("composition string too large"));
...
void IMEManager::getResultsString() {
  ...
  if (m_context) {
    // try reading unicode directly
    LONG result = ImmGetCompositionStringW(m_context, GCS_RESULTSTR,
                                           m_resultsString, MAX_COMPSTRINGLEN);
    ...
    if (result >= 0) {
      ...
      // read MBCS instead
      result = ImmGetCompositionStringA(m_context, GCS_RESULTSTR, tempBuf,
                                        MAX_COMPSTRINGLEN * 2);

      if (result > 0) {
        tempBuf[result] = '\0';

        int convRes = MultiByteToWideChar(CP_ACP, 0, tempBuf, strlen(tempBuf),
                                          m_resultsString, MAX_COMPSTRINGLEN);
  ...
  DEBUG_ASSERTCRASH(stringLen < MAX_COMPSTRINGLEN,
                    ("results string too large"));
...
void IMEManager::updateCandidateList(Int candidateFlags) {
  ...
  if (m_candidateWindow == nullptr || m_context == nullptr ||
      candidateFlags == 0) {
    return;
  }
  ...
  for (Int i = 0, candidate = 1; i < 32; i++, candidate = candidate << 1) {
    if (candidateFlags & candidate) {
      ...
      if (unicode) {
        bytesCopied =
            ImmGetCandidateListW(m_context, i, (CANDIDATELIST *)clist, size);
      } else {
        bytesCopied =
            ImmGetCandidateListA(m_context, i, (CANDIDATELIST *)clist, size);
      }
      ...
      if (ok && clist->dwStyle != IME_CAND_UNKNOWN &&
          clist->dwStyle != IME_CAND_CODE) {
...
#ifdef DEBUG_IME
        DEBUG_LOG(("IME: Candidate Update: Candidates = %d, pageSize = %d "
                   "pageStart = %d, selected = %d",
        ...
        if (m_candidateDownArrow) {
          m_candidateDownArrow->winHide(m_candidateCount - m_pageStart <=
                                        m_pageSize);
        }
        ...
        if (m_candidateCount > 0) {
          ...
          if (m_candidateString) {
            ...
            for (i = 0; i < m_candidateCount; i++) {
              Char *string = (Char *)((UnsignedInt)clist +
                                      (UnsignedInt)clist->dwOffset[i]);
              if (unicode) {
...
Int IMEManager::getIndexBase() { return m_indexBase; }
...
Int IMEManager::getCandidateCount() { return m_candidateCount; }
...
Int IMEManager::getSelectedCandidateIndex() { return m_selectedIndex; }

//============================================================================
// IMEManager::getCandidatePageSize
//============================================================================

Int IMEManager::getCandidatePageSize() { return m_pageSize; }

//============================================================================
// IMEManager::getStartCandidateIndex
//============================================================================

Int IMEManager::getCandidatePageStart() { return m_pageStart; }
...
public:
  ...
  virtual Bool isAttachedTo(GameWindow *window) { return FALSE; }
  virtual GameWindow *getWindow() { return nullptr; }
  virtual Bool isComposing() { return FALSE; }
  virtual void getCompositionString(UnicodeString &string) {}
  virtual Int getCompositionCursorPosition() { return 0; }
  virtual Int getIndexBase() { return 0; }

  virtual Int getCandidateCount() { return 0; }
  virtual const UnicodeString *getCandidate(Int index) { return nullptr; }
