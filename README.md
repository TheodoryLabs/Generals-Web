...
- **IMPORTANT — current stable Chrome can crash the tab when a match starts.** Chrome 149 stable (before 149.0.7827.201) and Chrome 150 stable up to 150.0.7871.47 ship a V8 garbage-collector bug (a GC-evacuation fault, fixed upstream in V8 14.9.207.37 / 15.0.245.14) that can kill the renderer of any large WebAssembly app. It is **probabilistic** — the same build can play fine one launch and crash the next, and heavy allocation (starting a skirmish) is the most likely moment. This is a browser bug, not a bug in this port: the identical binary plays flawlessly on a browser whose V8 carries the fix. What to do, in order of preference:
  1. **Use a fixed browser**: run `./scripts/play-web.sh` next to the release files — it downloads a known-good Chrome for Testing build and launches the game in it (isolated profile, your normal browser untouched). Or use Chrome Beta/Dev, or wait for the next Chrome 150 stable respin and update.
  2. **Partial mitigation on affected Chrome**: fully quit Chrome (⌘Q / all windows) and relaunch with `--js-flags=--no-compact`. This disables only one of the two vulnerable GC paths — it reduces crashes but does **not** eliminate them. The flag applies per launch and is silently ignored if Chrome was already running.
  3. If the tab crashes: just reload — nothing is lost but the session, and the odds reset.

  Full investigation write-up: [postmortem](docs/web-port/POSTMORTEM-2026-07-07-gc-crash.md).
