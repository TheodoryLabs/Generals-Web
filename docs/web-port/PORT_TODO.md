> **Historical tracker** — statuses below reflect May 2026; the custom memory pool has since been **re-enabled** (see `cmake/web.cmake`), and live project status is in the [root README](../../README.md).
...
+ release artifacts since source diverged from the build outputs in
March/April.
...
- [x] wasm-opt optimization pass — Session 11 (May 5 2026): `link_mac.sh`
      ...
      writes to `GeneralsZH-rel.{html,js,wasm}` so debug + release artifacts
